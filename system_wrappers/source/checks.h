...
#define DEFINE_CHECK_OP_IMPL(name, op) \
  ...
  inline std::string* Check##name##Impl(const t1& v1, const t2& v2, \
    ...
    else return cloopenwebrtc::MakeCheckOpString(v1, v2, names); \
  } \
...
class FatalMessage {
 public:
  explicit FatalMessage(const char* file, int line);
  // Used for CHECK_EQ(), etc. Takes ownership of the given string.
  explicit FatalMessage(const char* file, int line, std::string* result);
