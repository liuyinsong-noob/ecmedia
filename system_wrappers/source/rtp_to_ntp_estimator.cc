...
bool UpdateRtcpList(uint32_t ntp_secs,
  ...
  *new_rtcp_sr = false;
  ...
  for (RtcpList::iterator it = rtcp_list->begin();
       it != rtcp_list->end(); ++it) {
    if (ntp_secs == (*it).ntp_time.seconds() &&
        ntp_frac == (*it).ntp_time.fractions()) {
      // This RTCP has already been added to the list.
      return true;
    }
  }

  RtpToNtpEstimator::RtcpMeasurement measurement(ntp_secs, ntp_frac, rtp_timestamp);
...
bool RtpToNtpMs(int64_t rtp_timestamp,
  ...
  assert(rtcp.size() == 2);
  int64_t rtcp_ntp_ms_new = Clock::NtpToMs(rtcp.front().ntp_time.seconds(),
                                           rtcp.front().ntp_time.fractions());
  int64_t rtcp_ntp_ms_old = Clock::NtpToMs(rtcp.back().ntp_time.seconds(),
                                           rtcp.back().ntp_time.fractions());
