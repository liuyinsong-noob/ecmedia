...
class SendTimeHistory {
 public:
  ...
  bool MaybeCleanOutstandingData(int64_t current_ms, uint32_t tims);
 ...
 private:
  ...
  using Flight_bytes = std::pair<RemoteAndLocalNetworkId, size_t>;
  std::map<int64_t, Flight_bytes> time_in_flight_bytes;
