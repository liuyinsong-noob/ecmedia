...
//extern void PrintConsole(char * fmt,...);
...
rtp_session_init (RtpSession * session, int mode)
{
    ...
    session->VoEBase=NULL;
    session->ViENetwork=NULL;
...
void * rtp_session_get_voeBase(const RtpSession *session)
{
    return session->VoEBase;
}

void * rtp_session_get_vieNetwork(const RtpSession *session)
{
    return session->ViENetwork;
}
...
//
//			//PrintConsole("hubin rtpsession check OS support for qwave.lib: %i %i %i\n",
