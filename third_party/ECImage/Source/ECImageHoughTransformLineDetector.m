...
@interface ECImageHoughTransformLineDetector()

- (void)extractLineParametersFromImageAtFrameTime:(CMTime)frameTime;

@end

@implementation ECImageHoughTransformLineDetector
...
//    if ([ECImageContext deviceSupportsFramebufferReads])
//    if ([ECImageContext deviceSupportsFramebufferReads])
//    {
//        thresholdEdgeDetectionFilter = [[ECImageThresholdEdgeDetectionFilter alloc] init];
//        thresholdEdgeDetectionFilter = [[ECImageSobelEdgeDetectionFilter alloc] init];
//        [(ECImageThresholdEdgeDetectionFilter *)thresholdEdgeDetectionFilter setThreshold:0.07];
//        [(ECImageThresholdEdgeDetectionFilter *)thresholdEdgeDetectionFilter setEdgeStrength:0.25];
//        [(ECImageThresholdEdgeDetectionFilter *)thresholdEdgeDetectionFilter setEdgeStrength:1.0];
//        thresholdEdgeDetectionFilter = [[ECImageCannyEdgeDetectionFilter alloc] init];
//    }
//    else
//    {
        thresholdEdgeDetectionFilter = [[ECImageCannyEdgeDetectionFilter alloc] init];
//    }
    ...
    parallelCoordinateLineTransformFilter = [[ECImageParallelCoordinateLineTransformFilter alloc] init];
    [self addFilter:parallelCoordinateLineTransformFilter];
    
    // Third pass: apply non-maximum suppression
    if ([ECImageContext deviceSupportsFramebufferReads])
    {
        nonMaximumSuppressionFilter = [[ECImageThresholdedNonMaximumSuppressionFilter alloc] initWithPackedColorspace:YES];
    }
    else
    {
        nonMaximumSuppressionFilter = [[ECImageThresholdedNonMaximumSuppressionFilter alloc] initWithPackedColorspace:NO];
    }
    [self addFilter:nonMaximumSuppressionFilter];
    
    __unsafe_unretained ECImageHoughTransformLineDetector *weakSelf = self;
...
//    __unsafe_unretained ECImageOutput<ECImageInput> *weakEdgeDetectionFilter = thresholdEdgeDetectionFilter;
//    [thresholdEdgeDetectionFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
...
//    }];
//
//    __unsafe_unretained ECImageOutput<ECImageInput> *weakParallelCoordinateLineTransformFilter = parallelCoordinateLineTransformFilter;
//    [parallelCoordinateLineTransformFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
//        UIImage *intermediateImage = [weakParallelCoordinateLineTransformFilter imageFromCurrentFramebuffer];
//        [weakIntermediateImages addObject:intermediateImage];
//    }];

    __unsafe_unretained ECImageOutput<ECImageInput> *weakNonMaximumSuppressionFilter = nonMaximumSuppressionFilter;
    [nonMaximumSuppressionFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
...
//{
//    [(ECImageCannyEdgeDetectionFilter *)thresholdEdgeDetectionFilter setThreshold:newValue];
//}
//
//- (CGFloat)edgeThreshold;
//{
//    return [(ECImageCannyEdgeDetectionFilter *)thresholdEdgeDetectionFilter threshold];
//}
