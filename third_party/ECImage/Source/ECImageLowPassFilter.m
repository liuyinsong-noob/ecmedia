...
@implementation ECImageLowPassFilter
...
- (id)init;
{
    ...
    dissolveBlendFilter = [[ECImageDissolveBlendFilter alloc] init];
    [self addFilter:dissolveBlendFilter];
    
    // Buffer the result to be fed back into the blend
    bufferFilter = [[ECImageBuffer alloc] init];
...
- (void)addTarget:(id<ECImageInput>)newTarget atTextureLocation:(NSInteger)textureLocation;
{
    ...
    //if use ECImagePipline,will cause self.termainlFilter removeAllTargets,so need add bufferFilter back
