...
NSString *const kECImageColorAveragingVertexShaderString = SHADER_STRING
(
...
NSString *const kECImageColorAveragingFragmentShaderString = SHADER_STRING
(
...
@implementation ECImageAverageColor
...
- (id)init;
{
    if (!(self = [super initWithVertexShaderFromString:kECImageColorAveragingVertexShaderString fragmentShaderFromString:kECImageColorAveragingFragmentShaderString]))
    {
        return nil;
    }
    ...
    __unsafe_unretained ECImageAverageColor *weakSelf = self;
    [self setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime) {
...
- (void)renderToTextureWithVertices:(const GLfloat *)vertices textureCoordinates:(const GLfloat *)textureCoordinates;
{
    ...
    for (NSUInteger currentReduction = 0; currentReduction < reductionsToHitSideLimit; currentReduction++)
    {
        ...
        outputFramebuffer = [[ECImageContext sharedFramebufferCache] fetchFramebufferForSize:currentStageSize textureOptions:self.outputTextureOptions onlyTexture:NO];
