...
NSString *const kECImageColorSwizzlingFragmentShaderString = SHADER_STRING
(
...
@interface ECImageMovieWriter ()
{
    GLuint movieFramebuffer, movieRenderbuffer;
    
    ECGLProgram *colorSwizzlingProgram;
...
- (void)renderAtInternalSizeUsingFramebuffer:(ECImageFramebuffer *)inputFramebufferToUse;

@end

@implementation ECImageMovieWriter
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    ec_runSynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        {
            colorSwizzlingProgram = [_movieWriterContext programForVertexShaderString:kECImageVertexShaderString fragmentShaderString:kECImagePassthroughFragmentShaderString];
        }
        else
        {
            colorSwizzlingProgram = [_movieWriterContext programForVertexShaderString:kECImageVertexShaderString fragmentShaderString:kECImageColorSwizzlingFragmentShaderString];
        }
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        ...
        videoQueue = dispatch_queue_create("com.sunsetlakesoftware.ECImage.videoReadingQueue", NULL);
    ...
    if (audioInputReadyCallback != NULL)
    {
        audioQueue = dispatch_queue_create("com.sunsetlakesoftware.ECImage.audioReadingQueue", NULL);
...
- (void)renderAtInternalSizeUsingFramebuffer:(ECImageFramebuffer *)inputFramebufferToUse;
{
    ...
    const GLfloat *textureCoordinates = [ECImageFilter textureCoordinatesForRotation:inputRotation];
