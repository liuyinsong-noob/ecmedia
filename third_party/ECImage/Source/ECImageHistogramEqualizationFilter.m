//
//  ECImageHistogramEqualizationFilter.m
//  FilterShowcase
//
...
NSString *const kECImageRedHistogramEqualizationFragmentShaderString = SHADER_STRING
(
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
NSString *const kECImageGreenHistogramEqualizationFragmentShaderString = SHADER_STRING
(
 varying highp vec2 textureCoordinate;
 uniform sampler2D inputImageTexture;
 uniform sampler2D inputImageTexture2;
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
NSString *const kECImageBlueHistogramEqualizationFragmentShaderString = SHADER_STRING
(
 varying highp vec2 textureCoordinate;
 uniform sampler2D inputImageTexture;
 uniform sampler2D inputImageTexture2;
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
NSString *const kECImageRGBHistogramEqualizationFragmentShaderString = SHADER_STRING
(
 varying highp vec2 textureCoordinate; 
 uniform sampler2D inputImageTexture;
 uniform sampler2D inputImageTexture2;
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
NSString *const kECImageLuminanceHistogramEqualizationFragmentShaderString = SHADER_STRING
(
 varying highp vec2 textureCoordinate;
 uniform sampler2D inputImageTexture;
 uniform sampler2D inputImageTexture2;
...
@implementation ECImageHistogramEqualizationFilter
...
- (id)initWithHistogramType:(ECImageHistogramType)newHistogramType
{
    ...
    histogramFilter = [[ECImageHistogramFilter alloc] initWithHistogramType:newHistogramType];
    [self addFilter:histogramFilter];
    
    GLubyte dummyInput[4 * 256]; // NB: No way to initialise ECImageRawDataInput without providing bytes
    rawDataInputFilter = [[ECImageRawDataInput alloc] initWithBytes:dummyInput size:CGSizeMake(256.0, 1.0) pixelFormat:GPUPixelFormatBGRA type:GPUPixelTypeUByte];
    rawDataOutputFilter = [[ECImageRawDataOutput alloc] initWithImageSize:CGSizeMake(256.0, 3.0) resultsInBGRAFormat:YES];
    
    __unsafe_unretained ECImageRawDataOutput *_rawDataOutputFilter = rawDataOutputFilter;
    __unsafe_unretained ECImageRawDataInput *_rawDataInputFilter = rawDataInputFilter;
    ...
    switch (newHistogramType) {
        case kECImageHistogramRed:
            fragmentShader = kECImageRedHistogramEqualizationFragmentShaderString;
            break;
        case kECImageHistogramGreen:
            fragmentShader = kECImageGreenHistogramEqualizationFragmentShaderString;
            break;
        case kECImageHistogramBlue:
            fragmentShader = kECImageBlueHistogramEqualizationFragmentShaderString;
            break;
        default:
        case kECImageHistogramRGB:
            fragmentShader = kECImageRGBHistogramEqualizationFragmentShaderString;
            break;
        case kECImageHistogramLuminance:
            fragmentShader = kECImageLuminanceHistogramEqualizationFragmentShaderString;
            break;
    }
    ECImageFilter *equalizationFilter = [[ECImageTwoInputFilter alloc] initWithFragmentShaderFromString:fragmentShader];
