...
#else
	if (dispatch_get_specific([ECImageContext contextKey]))
#endif
	{
		block();
	}else
...
{
    ECImagePicture *stillImageSource = [[ECImagePicture alloc] initWithCGImage:imageToFilter];
    
    [self useNextFrameForImageCapture];
    [stillImageSource addTarget:(id<ECImageInput>)self];
    [stillImageSource processImage];
    
    CGImageRef processedImage = [self newCGImageFromCurrentlyProcessedOutput];
    
    [stillImageSource removeTarget:(id<ECImageInput>)self];
