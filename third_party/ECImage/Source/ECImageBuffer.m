...
@interface ECImageBuffer()

@end

@implementation ECImageBuffer
...
- (void)dealloc
{
    for (ECImageFramebuffer *currentFramebuffer in bufferedFramebuffers)
    {
...
- (void)setBufferSize:(NSUInteger)newValue;
{
    ...
    else
    {
        ...
        {
            ECImageFramebuffer *lastFramebuffer = [bufferedFramebuffers lastObject];
