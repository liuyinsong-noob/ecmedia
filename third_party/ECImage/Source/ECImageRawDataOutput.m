...
@interface ECImageRawDataOutput ()
{
    ECImageFramebuffer *firstInputFramebuffer, *outputFramebuffer, *retainedFramebuffer;
    
    BOOL hasReadFromTheCurrentFrame;
    
    ECGLProgram *dataProgram;
...
@implementation ECImageRawDataOutput
...
- (id)initWithImageSize:(CGSize)newImageSize resultsInBGRAFormat:(BOOL)resultsInBGRAFormat;
{
    ...
    if ( (outputBGRA && ![ECImageContext supportsFastTextureUpload]) || (!outputBGRA && [ECImageContext supportsFastTextureUpload]) )
    {
        dataProgram = [[ECImageContext sharedImageProcessingContext] programForVertexShaderString:kECImageVertexShaderString fragmentShaderString:kECImageColorSwizzlingFragmentShaderString];
    }
    else
    {
        dataProgram = [[ECImageContext sharedImageProcessingContext] programForVertexShaderString:kECImageVertexShaderString fragmentShaderString:kECImagePassthroughFragmentShaderString];
    }
...
- (void)dealloc
{
    if (_rawBytesForImage != NULL && (![ECImageContext supportsFastTextureUpload]))
    {
...
- (void)renderAtInternalSize;
{
    [ECImageContext setActiveShaderProgram:dataProgram];

    outputFramebuffer = [[ECImageContext sharedFramebufferCache] fetchFramebufferForSize:imageSize onlyTexture:NO];
