...
- (void)capturePhotoProcessedUpToFilter:(ECImageOutput<ECImageInput> *)finalFilterInChain withImageOnGPUHandler:(void (^)(NSError *error))block;

@end

@implementation ECImageStillCamera {
...
{
//    reportAvailableMemoryForECImage(@"Before Capture");
    ...
    [self capturePhotoProcessedUpToFilter:finalFilterInChain withImageOnGPUHandler:^(NSError *error) {
        ...
        if(!error){
            @autoreleasepool {
                ...
                dispatch_semaphore_signal(frameRenderingSemaphore);
//                reportAvailableMemoryForECImage(@"After UIImage generation");

                dataForJPEGFile = UIImageJPEGRepresentation(filteredPhoto,self.jpegCompressionQuality);
//                reportAvailableMemoryForECImage(@"After JPEG generation");
            }

//            reportAvailableMemoryForECImage(@"After autorelease pool");
        }else{
            dispatch_semaphore_signal(frameRenderingSemaphore);
        }
...
- (void)capturePhotoAsJPEGProcessedUpToFilter:(ECImageOutput<ECImageInput> *)finalFilterInChain withOrientation:(UIImageOrientation)orientation withCompletionHandler:(void (^)(NSData *processedImage, NSError *error))block {
    [self capturePhotoProcessedUpToFilter:finalFilterInChain withImageOnGPUHandler:^(NSError *error) {
        NSData *dataForJPEGFile = nil;
...
- (void)capturePhotoProcessedUpToFilter:(ECImageOutput<ECImageInput> *)finalFilterInChain withImageOnGPUHandler:(void (^)(NSError *error))block
{
    ...
    [photoOutput captureStillImageAsynchronouslyFromConnection:[[photoOutput connections] objectAtIndex:0] completionHandler:^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
        ...
        CGSize scaledImageSizeToFitOnGPU = [ECImageContext sizeThatFitsWithinATextureForSize:sizeOfPhoto];
        if (!CGSizeEqualToSize(sizeOfPhoto, scaledImageSizeToFitOnGPU))
        {
            ...
            {
                ECImageCreateResizedSampleBuffer(cameraFrame, scaledImageSizeToFitOnGPU, &sampleBuffer);
            }
        ...
        else
        {
            ...
            if ( (currentCameraPosition != AVCaptureDevicePositionFront) || (![ECImageContext supportsFastTextureUpload]) || !requiresFrontCameraTextureCacheCorruptionWorkaround)
            {
                dispatch_semaphore_signal(frameRenderingSemaphore);
                [finalFilterInChain useNextFrameForImageCapture];
