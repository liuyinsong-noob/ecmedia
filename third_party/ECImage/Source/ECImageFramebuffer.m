...
void ec_dataProviderReleaseCallback(void *info, const void *data, size_t size);
void ec_dataProviderUnlockCallback(void *info, const void *data, size_t size);

@implementation ECImageFramebuffer
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            CVOpenGLESTextureCacheRef coreVideoTextureCache = [[ECImageContext sharedImageProcessingContext] coreVideoTextureCache];
...
- (void)destroyFramebuffer;
{
    ec_runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([ECImageContext supportsFastTextureUpload] && (!_missingFramebuffer)) {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            if (renderTarget) {
...
- (void)unlock;
{
    ...
    {
        [[ECImageContext sharedFramebufferCache] returnFramebufferToCache:self];
    }
}
...
void ec_dataProviderUnlockCallback(void *info, const void *data, size_t size)
{
    ECImageFramebuffer *framebuffer = (__bridge_transfer ECImageFramebuffer*)info;
    
    [framebuffer restoreRenderTarget];
    [framebuffer unlock];
    [[ECImageContext sharedFramebufferCache] removeFramebufferFromActiveImageCaptureList:framebuffer];
}
...
- (CGImageRef)newCGImageFromFramebufferContents;
{
    ...
    ec_runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([ECImageContext supportsFastTextureUpload]) {
            ...
            dataProvider = CGDataProviderCreateWithData((__bridge_retained void *) self, rawImagePixels, paddedBytesForImage, ec_dataProviderUnlockCallback);
            [[ECImageContext sharedFramebufferCache] addFramebufferToActiveImageCaptureList:self]; // In case the framebuffer is swapped out on the filter, need to have a strong reference to it somewhere for it to hang on while the image is in existence
#else
#endif
        } else {
            ...
            dataProvider = CGDataProviderCreateWithData(NULL, rawImagePixels, totalBytesForImage, ec_dataProviderReleaseCallback);
...
- (void)unlockAfterReading
{
    ...
    if ([ECImageContext supportsFastTextureUpload])
    {
        NSAssert(readLockCount > 0, @"Unbalanced call to -[ECImageFramebuffer unlockAfterReading]");
        readLockCount--;
        if (readLockCount == 0)
        {
