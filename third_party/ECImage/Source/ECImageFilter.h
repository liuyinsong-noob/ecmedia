...
#define ECImageHashIdentifier #
#define ECImageWrappedLabel(x) x
#define ECImageEscapedHashIdentifier(a) ECImageWrappedLabel(ECImageHashIdentifier)a

extern NSString *const kECImageVertexShaderString;
extern NSString *const kECImagePassthroughFragmentShaderString;
...
/** ECImage's base filter class
 
 Filters and other subsequent elements in the chain conform to the ECImageInput protocol, which lets them take in the supplied or processed texture from the previous link in the chain and do something with it. Objects one step further down the chain are considered targets, and processing can be branched by adding multiple targets to a single output or filter.
 */
@interface ECImageFilter : ECImageOutput <ECImageInput>
{
    ECImageFramebuffer *firstInputFramebuffer;
    
    ECGLProgram *filterProgram;
