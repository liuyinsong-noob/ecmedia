...
@interface ECImageHarrisCornerDetectionFilter()
...
@implementation ECImageHarrisCornerDetectionFilter

#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
NSString *const kECImageHarrisCornerDetectionFragmentShaderString = SHADER_STRING
( 
...
- (id)init;
{
    if (!(self = [self initWithCornerDetectionFragmentShader:kECImageHarrisCornerDetectionFragmentShaderString]))
    {
        return nil;
    }
...
- (id)initWithCornerDetectionFragmentShader:(NSString *)cornerDetectionFragmentShader;
{
    ...
    derivativeFilter = [[ECImageXYDerivativeFilter alloc] init];
    [self addFilter:derivativeFilter];

#ifdef DEBUGFEATUREDETECTION
    __unsafe_unretained NSMutableArray *weakIntermediateImages = _intermediateImages;
    __unsafe_unretained ECImageFilter *weakFilter = derivativeFilter;
    [derivativeFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
    ...
    blurFilter = [[ECImageGaussianBlurFilter alloc] init];
    [self addFilter:blurFilter];
    
#ifdef DEBUGFEATUREDETECTION
    weakFilter = blurFilter;
    [blurFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
        UIImage *intermediateImage = [weakFilter imageFromCurrentlyProcessedOutput];
        [weakIntermediateImages addObject:intermediateImage];
    }];
#endif
    ...
    harrisCornerDetectionFilter = [[ECImageFilter alloc] initWithFragmentShaderFromString:cornerDetectionFragmentShader];
    [self addFilter:harrisCornerDetectionFilter];

#ifdef DEBUGFEATUREDETECTION
    weakFilter = harrisCornerDetectionFilter;
    [harrisCornerDetectionFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
        UIImage *intermediateImage = [weakFilter imageFromCurrentlyProcessedOutput];
        [weakIntermediateImages addObject:intermediateImage];
    }];
#endif
    ...
    nonMaximumSuppressionFilter = [[ECImageThresholdedNonMaximumSuppressionFilter alloc] init];
    [self addFilter:nonMaximumSuppressionFilter];

    __unsafe_unretained ECImageHarrisCornerDetectionFilter *weakSelf = self;
#ifdef DEBUGFEATUREDETECTION
...
//    colorPackingFilter = [[ECImageColorPackingFilter alloc] init];
...
//    [colorPackingFilter setFrameProcessingCompletionBlock:^(ECImageOutput *filter, CMTime frameTime){
