...
@implementation ECImageBoxBlurFilter

+ (NSString *)vertexShaderForOptimizedBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    if (blurRadius < 1)
    {
        return kECImageVertexShaderString;
    }
...
+ (NSString *)fragmentShaderForOptimizedBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    if (blurRadius < 1)
    {
        return kECImagePassthroughFragmentShaderString;
    }
...
- (id)init;
{
    //    NSString *currentGaussianBlurVertexShader = [ECImageGaussianBlurFilter vertexShaderForStandardGaussianOfRadius:4 sigma:2.0];
    //    NSString *currentGaussianBlurFragmentShader = [ECImageGaussianBlurFilter fragmentShaderForStandardGaussianOfRadius:4 sigma:2.0];
