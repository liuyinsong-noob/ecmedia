...
#define ECMEDIA_VERSION "ecmedia_version: v2.3.2.20"
...
int ECMedia_setECMedia_ConferenceParticipantCallbackTimeInterVal(int channelid, int timeInterVal) {
    PrintConsole("[ECMEDIA INFO] %s begins... and channelid: %d, timeInterVal: %d", __FUNCTION__, channelid, timeInterVal);
    AUDIO_ENGINE_UN_INITIAL_ERROR(ERR_ENGINE_UN_INIT);
    ...
    if (base) {
        ...
        base->Release();
        if (ret != 0) {
            PrintConsole("[ECMEDIA ERROR] %s failed to set conference participant timeInterVal", __FUNCTION__);
        }
        PrintConsole("[ECMEDIA INFO] %s ends... with code: %d ", __FUNCTION__, ret);
        return 0;
    }
