...
public class ViESurfaceRenderer implements Callback {
    ...
    public ViESurfaceRenderer(String account) {
        ...
        if(surfaceHolder == null) {
        	if(DEBUG) {
        		Log4Util.w(TAG, "Remote SurfaceView is null...");
        	}
            return;
        }
        
        if(DEBUG) {
        	Log4Util.w(TAG, "Remote SurfaceView is coming...");
        }
...
//        dstRect.right = 300;
    ...
    public void surfaceChanged(SurfaceHolder holder, int format,
    	...
    	if(DEBUG) {
    		Log4Util.d(TAG, "ViESurfaceRender::surfaceChanged");
    	}
    ...
    public void surfaceCreated(SurfaceHolder holder) {
    	if(DEBUG) {
    		Log4Util.d(TAG, "ViESurfaceRender::surfaceCreated" );
    	}
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
    	if(DEBUG) {
    		Log4Util.d(TAG, "ViESurfaceRenderer::surfaceDestroyed");
    	}
        //bitmap = null;
       //byteBuffer = null;
    }
    ...
    public Bitmap CreateBitmap(int width, int height) {
    	if(DEBUG) {
    		Log4Util.d(TAG, "CreateByteBitmap " + width + ":" + height);
    	}
        if (bitmap == null) {
            try {
    ...
    public ByteBuffer CreateByteBuffer(int width, int height) {
    	if(DEBUG) {
    		Log4Util.d(TAG, "CreateByteBuffer " + width + ":" + height);
    	}
       // if (bitmap == null) {
    ...
    public void SetCoordinates(float left, float top,
    	...
    	if(DEBUG) {
    		Log4Util.d(TAG, "SetCoordinates " + left + "," + top + ":" +
    				right + "," + bottom);
    	}
        dstLeftScale = left;
        dstTopScale = top;
    ...
    public void DrawByteBuffer() {
        if(byteBuffer == null){
        	if(DEBUG) {
        		Log4Util.w(TAG, "DrawByteBuffer is null...");
        	}
            return;
        }
        if(DEBUG) {
    		Log4Util.w(TAG, "DrawByteBuffer is coming...");
    	}
        byteBuffer.rewind();
    ...
    public void DrawBitmap() {
    	if(DEBUG) {
    		...
    		Log4Util.w(TAG, "DrawBitmap is coming...");
    	}
        if(bitmap == null){
        	if(DEBUG) {
        		
        		Log4Util.w(TAG, "bitmap is null...");
        	}
            return;
        }
        ...
        if(canvas != null) {
        	...
        	if(DEBUG) {
        		...
        		Log4Util.w(TAG, "srcRect: " + srcRect + ", dstRect: " + dstRect);
        	}
