...
class TaskQueue::MultimediaTimer {
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(MultimediaTimer);
};
...
TaskQueue::TaskQueue(const char* queue_name, Priority priority /*= NORMAL*/)
  ...
  CHECK(thread_.QueueAPC(&InitializeQueueThread,
...
TaskQueue::~TaskQueue() {
  ...
  while (!::PostThreadMessage(thread_.GetThreadRef(), WM_QUIT, 0, 0)) {
    CHECK_EQ(ERROR_NOT_ENOUGH_QUOTA, ::GetLastError());
    Sleep(1);
  }
  thread_.Stop();
}
...
void TaskQueue::ThreadMain(void* context) {
  ...
  while (true) {
    DCHECK(mm_timers.size() <= arraysize(timer_handles));
    DWORD count = 0;
    ...
    if (result == (WAIT_OBJECT_0 + count)) {
      ...
      DCHECK(!mm_timers[result].is_active());
      ...
      DCHECK_EQ(WAIT_IO_COMPLETION, result);
    }
  }
}
    ...
    if (!msg.hwnd) {
      switch (msg.message) {
        ...
        case WM_TIMER: {
          ...
          DCHECK(found != delayed_tasks->end());
          if (!found->second->Run())
