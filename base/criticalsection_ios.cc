...
#else
#if (defined(WEBRTC_MAC)  || defined(WEBRTC_IOS)) && !USE_NATIVE_MUTEX_ON_MAC
  lock_queue_ = 0;
  owning_thread_ = 0;
  recursion_ = 0;
...
void CriticalSection::Enter() const EXCLUSIVE_LOCK_FUNCTION() {
  ...
  if (!have_lock && AtomicOps::Increment(&lock_queue_) > 1) {
    ...
    // have succeeded.
    DCHECK(!IsThreadRefEqual(owning_thread_, self));
    // Wait for the lock to become available.
    dispatch_semaphore_wait(semaphore_, DISPATCH_TIME_FOREVER);
    DCHECK(owning_thread_ == 0);
    DCHECK(!recursion_);
  }
...
#endif
#if (defined(WEBRTC_MAC) || defined(WEBRTC_IOS))&& !USE_NATIVE_MUTEX_ON_MAC
  DCHECK(IsThreadRefEqual(owning_thread_, CurrentThreadRef()));
  DCHECK_GE(recursion_, 0);
  --recursion_;
  if (!recursion_)
    owning_thread_ = 0;
...
#if !defined(WEBRTC_WIN) && ((!defined(WEBRTC_MAC)&& !defined(WEBRTC_IOS)) || USE_NATIVE_MUTEX_ON_MAC)
...
#if defined(WEBRTC_WIN)
    ::Sleep(0);
#elif (defined(WEBRTC_MAC) || defined(WEBRTC_IOS)) && !USE_NATIVE_MUTEX_ON_MAC
    sched_yield();
#else
