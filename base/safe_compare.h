...
#define SAFECMP_MAKE_OP(name, op)      \
  struct name {                            \
    template <typename T1, typename T2>    \
...
SAFECMP_MAKE_OP(EqOp, ==)
SAFECMP_MAKE_OP(NeOp, !=)
SAFECMP_MAKE_OP(LtOp, <)
SAFECMP_MAKE_OP(LeOp, <=)
SAFECMP_MAKE_OP(GtOp, >)
SAFECMP_MAKE_OP(GeOp, >=)
#undef SAFECMP_MAKE_OP
...
#if 0 //TODO:this code issue build error on vs2015, temporary remove this code 
