...
void EventAssign(struct event* ev,
  ...
  CHECK_EQ(0, event_assign(ev, base, fd, events, callback, arg));
#else
  event_set(ev, fd, events, callback, arg);
  CHECK_EQ(0, event_base_set(base, ev));
#endif
}
...
void TaskQueue::OnWakeup(int socket, short flags, void* context) {  // NOLINT
  QueueContext* ctx =
      static_cast<QueueContext*>(pthread_getspecific(GetQueuePtrTls()));
  DCHECK(ctx->queue->wakeup_pipe_out_ == socket);
  char buf;
  ...
  switch (buf) {
    ...
    case kRunTask: {
      std::unique_ptr<QueuedTask> task;
      {
        ...
        DCHECK(!ctx->queue->pending_.empty());
