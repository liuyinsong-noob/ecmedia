...
int PhysicalSocket::Send(const void* pv, size_t cb) {
  ...
  DCHECK(sent <= static_cast<int>(cb));
...
int PhysicalSocket::SendTo(const void* buffer,
  ...
  MaybeRemapSendError();
  // We have seen minidumps where this may be false.
  DCHECK(sent <= static_cast<int>(length));
...
class EventDispatcher : public Dispatcher {
 public:
  ...
  void OnEvent(uint32_t ff, int err) override { NOTREACHED(); }
...
class PosixSignalHandler {
 public:
  ...
  bool IsSignalSet(int signum) const {
    DCHECK(signum < static_cast<int>(arraysize(received_signal_)));
