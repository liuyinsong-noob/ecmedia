...
void VideoDenoiser::DenoiserReset(const I420VideoFrame& frame,
	I420VideoFrame* denoised_frame,
	I420VideoFrame* denoised_frame_prev) {
  width_ = frame.width();
...
//                               stride_u_, stride_v_, kVideoRotation_0);
  {
	  int size_y = frame.height() * frame.stride(kYPlane);
	  int size_u = frame.stride(kUPlane) * ((frame.height() + 1) / 2);
	  int size_v = frame.stride(kVPlane) * ((frame.height() + 1) / 2);

	  denoised_frame->CreateFrame(size_y,
								 frame.buffer(kYPlane),
								  size_u,
								frame.buffer(kUPlane),
								  size_v,
									frame.buffer(kVPlane),
								  frame.width(),
								  frame.height(),
...
void VideoDenoiser::DenoiseFrame(const I420VideoFrame& frame,
	I420VideoFrame* denoised_frame,
	I420VideoFrame* denoised_frame_prev,
