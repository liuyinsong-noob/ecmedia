...
RtpRtcp::Configuration::Configuration()
    : id(-1),
	  ...
	  receive_rtcp_packettype_count_observer(NULL),
...
ModuleRtpRtcpImpl::ModuleRtpRtcpImpl(const Configuration& configuration)
    : rtp_sender_(configuration.id,
      ...
      last_packet_timeout_process_time_(configuration.clock->TimeInMilliseconds()),
      ...
      simulcast_(false),
      key_frame_req_method_(kKeyFrameReqFirRtcp),
  ...
  rtcp_receiver_.RegisterReceiveRtcpPacketTypeCounterObserver(configuration.receive_rtcp_packettype_count_observer);
...
int32_t ModuleRtpRtcpImpl::Process() {
  ...
  const int64_t kRtpRtcpBitrateProcessTimeMs = 10; //10ms?Æ†??
  if (now >= last_bitrate_process_time_ + kRtpRtcpBitrateProcessTimeMs) {
    rtp_sender_.ProcessBitrate(); //???ò˜?¢¿’br?total£®nack£®fec£®video
...
ModuleRtpRtcpImpl::SetRTPKeepaliveStatus(bool enable, WebRtc_Word8 unknownPayloadType, WebRtc_UWord16 deltaTransmitTimeMS)
{
    if (enable)
    {
        LOG(LS_INFO) << "SetRTPKeepaliveStatus(enable, payloadType:" << unknownPayloadType << "deltaTransmitTimeMS:" << deltaTransmitTimeMS;
        // check the transmit keepalive delta time [1,60]
        if (deltaTransmitTimeMS < 1000 || deltaTransmitTimeMS > 60000)
        {
            LOG(LS_ERROR) << "\tinvalid deltaTransmitTimeSeconds (" << deltaTransmitTimeMS <<")";
            return (-1);
        }
        ...
        {
            LOG(LS_ERROR) << "\tinvalid unknownPayloadType (" << unknownPayloadType <<")";
            return (-1);
        }
        
        // enable RTP keepalive mechanism
        return rtp_sender_.EnableRTPKeepalive(unknownPayloadType, deltaTransmitTimeMS);
        
    }else
    {
        LOG(LS_INFO) << "SetRTPKeepaliveStatus(disable)";
