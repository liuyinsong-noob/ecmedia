...
Packet::Packet(const ExtensionManager* extensions, size_t capacity)
    : buffer_(capacity) {
  DCHECK_GE(capacity, kFixedHeaderSize);
  Clear();
  if (extensions) {
...
bool Packet::Marker() const {
  DCHECK_EQ(marker_, (data()[1] & 0x80) != 0);
  return marker_;
}

uint8_t Packet::PayloadType() const {
  DCHECK_EQ(payload_type_, data()[1] & 0x7f);
  return payload_type_;
}

uint16_t Packet::SequenceNumber() const {
  DCHECK_EQ(sequence_number_,
                ByteReader<uint16_t>::ReadBigEndian(data() + 2));
  return sequence_number_;
}

uint32_t Packet::Timestamp() const {
  DCHECK_EQ(timestamp_, ByteReader<uint32_t>::ReadBigEndian(data() + 4));
  return timestamp_;
}

uint32_t Packet::Ssrc() const {
  DCHECK_EQ(ssrc_, ByteReader<uint32_t>::ReadBigEndian(data() + 8));
  return ssrc_;
}

std::vector<uint32_t> Packet::Csrcs() const {
  size_t num_csrc = data()[0] & 0x0F;
  DCHECK_GE(capacity(), kFixedHeaderSize + num_csrc * 4);
...
void Packet::CopyHeaderFrom(const Packet& packet) {
  DCHECK_GE(capacity(), packet.headers_size());
...
void Packet::SetCsrcs(const std::vector<uint32_t>& csrcs) {
  DCHECK_EQ(extensions_size_, 0);
  DCHECK_EQ(payload_size_, 0);
  DCHECK_EQ(padding_size_, 0);
  DCHECK_LE(csrcs.size(), 0x0fu);
  DCHECK_LE(kFixedHeaderSize + 4 * csrcs.size(), capacity());
...
void Packet::SetPayloadSize(size_t size_bytes) {
  DCHECK_EQ(padding_size_, 0);
  DCHECK_LE(size_bytes, payload_size_);
  payload_size_ = size_bytes;
  buffer_.SetSize(payload_offset_ + payload_size_);
}
...
bool Packet::AllocateExtension(ExtensionType type,
  ...
  if (extensions_size_ == 0) {
    DCHECK_EQ(payload_offset_, kFixedHeaderSize + (num_csrc * 4));
