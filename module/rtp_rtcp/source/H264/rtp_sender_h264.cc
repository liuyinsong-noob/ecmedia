                                  ...
                                  const WebRtc_UWord32 ssrc)
{
    ...
    if (fillerLength > IP_PACKET_SIZE - 12 - 1)
    {
        return 0;
    }
                                      ...
                                      const bool higestLayer)
{
    ...
    if(higestLayer && rtpHeader->type.Video.codecHeader.H264SVC.relayE)
    {
        // set marker bit
        dataBuffer[1] |= kRtpMarkerBitMask;
                                    ...
                                    const bool sendSVCPACSI)
{
    ...
    const WebRtc_UWord16 maxPayloadLength = IP_PACKET_SIZE - _rtpSender.PacketOverHead() - FECPacketOverhead() - rtpHeaderLength;
    WebRtc_UWord8 dataBuffer[IP_PACKET_SIZE];
    bool isBaseLayer=false;
                                     ...
                                     const bool lastPacketInNALU)
{
	...
	WebRtc_UWord16 decodingOrderNumber = 0;
