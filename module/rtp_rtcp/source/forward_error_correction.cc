...
#include "../module/rtp_rtcp/source/flexfec_header_reader_writer.h"
...
int ForwardErrorCorrection::EncodeFec(const PacketList& media_packets,
  ...
  DCHECK_GT(num_media_packets, 0);
  DCHECK_GE(num_important_packets, 0);
  DCHECK_LE(num_important_packets, num_media_packets);
  DCHECK(fec_packets->empty());
...
int ForwardErrorCorrection::NumFecPackets(int num_media_packets,
  ...
  DCHECK_LE(num_fec_packets, num_media_packets);
...
void ForwardErrorCorrection::InsertFecPacket(
  ...
  if (fec_packet->protected_packets.empty()) {
    ...
    DCHECK_LE(received_fec_packets_.size(), max_fec_packets);
  }
}
...
void ForwardErrorCorrection::XorPayloads(const Packet& src,
                                         ...
                                         Packet* dst) {
  // XOR the payload.
  DCHECK_LE(kRtpHeaderSize + payload_length, sizeof(src.data));
  DCHECK_LE(dst_offset + payload_length, sizeof(dst->data));
...
void ForwardErrorCorrection::DiscardOldRecoveredPackets(
  ...
  DCHECK_LE(recovered_packets->size(), max_media_packets);
}
