...
void RTPSenderVideo::SendVideoPacketAsRedMaybeWithUlpfec(
  ...
  for (const auto& fec_packet : fec_packets) {
    ...
    std::unique_ptr<RtpPacketToSend> rtp_packet(
        new RtpPacketToSend(*media_packet));
    CHECK(rtp_packet->Parse(fec_packet->data(), fec_packet->length()));
...
void RTPSenderVideo::SetUlpfecConfig(int red_payload_type,
  ...
  // turned off.
  DCHECK_GE(red_payload_type, -1);
  DCHECK_LE(red_payload_type, 127);
  ...
  DCHECK(red_enabled() || !ulpfec_enabled());
...
bool RTPSenderVideo::SendVideo(RtpVideoCodecTypes video_type,
  ...
  DCHECK_LE(packet_capacity, rtp_header->capacity());
  DCHECK_GT(packet_capacity, rtp_header->headers_size());
  ...
  while (!last) {
    ...
    if (!packetizer->NextPacket(packet.get(), &last))
      return false;
    DCHECK_LE(packet->payload_size(), max_data_payload_length);
