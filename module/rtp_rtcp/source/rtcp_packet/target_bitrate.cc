...
bool TargetBitrate::Parse(const uint8_t* block, uint16_t block_length) {
  ...
  DCHECK_EQ(block[0], kBlockType);
  DCHECK_EQ(block_length, ByteReader<uint16_t>::ReadBigEndian(&block[2]));
...
void TargetBitrate::AddTargetBitrate(uint8_t spatial_layer,
                                     uint8_t temporal_layer,
                                     uint32_t target_bitrate_kbps) {
  DCHECK_LE(spatial_layer, 0x0F);
  DCHECK_LE(temporal_layer, 0x0F);
  DCHECK_LE(target_bitrate_kbps, 0x00FFFFFFU);
  bitrates_.push_back(
