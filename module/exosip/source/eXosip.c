...
#ifndef WIN32
int eXosip_trylock(void){
	return pthread_mutex_trylock((pthread_mutex_t*)eXosip.j_mutexlock);
}
						...
						int *retry)
{
	...
	ji = (jinfo_t *)osip_transaction_get_your_instance(out_tr);
						...
						int *retry)
{
	...
	if (out_tr != NULL && out_tr->last_response != NULL
		...
		while (!osip_list_eol(&msg->headers, pos)) {
			osip_header_t *head = (osip_header_t *)osip_list_get(&msg->headers, pos);
...
static int _eXosip_retry_register_with_auth(eXosip_event_t * je)
{
	...
	//upper level has control the retry times
	if (jr /*&& jr->r_retry < 3*/) {
		//jr->r_retry++;
		osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,"[Debug] rebuild authen register\n");
		if(je->request && osip_message_get_contact(je->request,0, &contact) == 0)
		{
			osip_message_header_get_byname(je->request, "expires", 0, &exp);
			if( exp)
				i= eXosip_register_build_register(jr->r_id,atoi(exp->hvalue),&reg);
			osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,
					"[Debug] registration build return (%d)\n",i);

			osip_from_to_str(contact,&str);
			osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,"[Debug] replace contact with orig requet\n");
			osip_message_replace_contact(reg,str);
			osip_free(str);
		}		
		osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,"[Debug] send authen register\n");
		return eXosip_register_send_register(jr->r_id, reg);
	}
	osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,"can not get register info\n");
	return OSIP_UNDEFINED_ERROR;
}
...
static int _eXosip_sesssio_expires_invite(eXosip_event_t * je)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
...
int eXosip_default_action(eXosip_event_t * je)
{
	...
	else if (je->response->status_code == 422)
		return _eXosip_sesssio_expires_invite(je);
	else
		return 1;
}
	...
	for (js = eXosip.j_subscribes; js != NULL; js = js->next) {
		for (jd = js->s_dialogs; jd != NULL; jd = jd->next) {
			if (jd->d_dialog != NULL && (jd->d_id >= 1)) {	/* finished call */
				...
				out_tr = (osip_transaction_t *)osip_list_get(jd->d_out_trs, 0);
				if (out_tr == NULL)
					out_tr = js->s_out_tr;
	...
	for (jr = eXosip.j_reg; jr != NULL; jr = jr->next) {
		if (jr->r_id >= 1 && jr->r_last_tr != NULL) {
			if (jr->r_reg_period == 0) {
				/* skip refresh! */
		    //hubin 14.10.23, when change system time backward, refresh
			//} else if (now - jr->r_last_tr->birth_time > 900) {
			} else if (now - jr->r_last_tr->birth_time > 900 || now - jr->r_last_tr->birth_time < 0) {
				/* automatic refresh */
//                OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                      OSIP_ERROR, NULL,"seansean eXosip_automatic_refresh 111 jr addr:%p, jr->r_registrar:%s, jr->r_last_tr:%p,jr->r_id:%d\n",jr,jr->r_registrar,jr->r_last_tr,jr->r_id));
...
//                OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                      OSIP_ERROR, NULL,"seansean eXosip_automatic_refresh 222 jr addr:%p, jr->r_registrar:%s, jr->r_last_tr:%p,jr->r_id:%d\n",jr,jr->r_registrar,jr->r_last_tr,jr->r_id));
				eXosip_register_send_register(jr->r_id, NULL);
			} else if (now - jr->r_last_tr->birth_time > 60 &&
					   (jr->r_last_tr->last_response == NULL
						|| (!MSG_IS_STATUS_2XX(jr->r_last_tr->last_response)))) {
				/* automatic refresh */
//                           OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                                 OSIP_ERROR, NULL,"seansean eXosip_automatic_refresh 333 jr addr:%p, jr->r_registrar:%s, jr->r_last_tr:%p,jr->r_id:%d\n",jr,jr->r_registrar,jr->r_last_tr,jr->r_id));
				eXosip_register_send_register(jr->r_id, NULL);
			}
		}
	}
...
int eXosip_stop_specified_keepalive (const char *ip, int port)
{
    return eXtl_tcp.tl_stop_keepalive(ip, port);
}
