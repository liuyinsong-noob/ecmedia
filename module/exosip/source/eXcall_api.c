...
int eXosip_call_build_answer(int tid, int status, osip_message_t ** answer)
{
	...
	*answer = NULL;
	...
	if (0 == osip_strcasecmp(tr->orig_request->sip_method, "INVITE")) {
		i = _eXosip_answer_invite_123456xx(jc, jd, status, answer, 0, NULL, -1);
	} else {
...
int eXosip_call_send_answer(int tid, int status, osip_message_t * answer, const char *key, int isAkey)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
	...
	if (answer == NULL) {
		if (0 == osip_strcasecmp(tr->orig_request->sip_method, "INVITE")) {
			...
			int ret = _eXosip_answer_invite_123456xx(jc, jd, status, &response, 1, key, isAkey);
            return ret;
		}
		osip_message_free(answer);
		return OSIP_BADPARAMETER;
						...
						if (exp_h == NULL) {
							...
							if (exp_h->element == NULL) {
								...
								osip_generic_param_get_byname(&exp_h->gen_params,
															  "refresher", &param);
								if (param == NULL) {
									tmp1=osip_strdup("refresher");
									tmp2=osip_strdup("uas");
									osip_generic_param_add(&exp_h->gen_params,
														   tmp1, tmp2);
//									osip_generic_param_add(&exp_h->gen_params,
//														   osip_strdup("refresher"),
//														   osip_strdup("uas"));
									osip_free(cp->hvalue);
									cp->hvalue = NULL;
...
int eXosip_call_terminate(int cid, int did)
{
	...
	if (tr == NULL) {
		...
		if (tr != NULL && tr->last_response != NULL && MSG_IS_STATUS_1XX(tr->last_response)) {	/* answer with 603 */
			...
			i = eXosip_call_send_answer(tr->transactionid, 603, NULL, NULL, 0);
	...
	if (MSG_IS_STATUS_3XX(out_tr->last_response)) {
		...
		/* support for diversions headers/draft! */
		{
			...
			while (!osip_list_eol(&out_tr->last_response->headers, pos)) {
				...
				osip_header_t *head = (osip_header_t *)
