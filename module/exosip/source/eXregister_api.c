...
#ifndef WIN32
extern int g_RegisterErrorCount;
#endif
...
//    if (expires == 0) {
//        printf("seansean who calls unregister\n");
//    }
...
int eXosip_register_send_register(int rid, osip_message_t * reg)
{
    //OSIP_TRACE(osip_trace
    //           (__FILE__, __LINE__, OSIP_ERROR, NULL,
    //            "eXosip: eXosip register send register enters!"));
	...
	if (jr == NULL) {
        ...
        OSIP_TRACE(osip_trace
                   (__FILE__, __LINE__, OSIP_ERROR, NULL,
                    "eXosip: eXosip register send register OSIP_NOTFOUND! Return"));
		return OSIP_NOTFOUND;
	}
	...
	if (jr->r_last_tr != NULL) {
		if (jr->r_last_tr->state != NICT_TERMINATED
            ...
            OSIP_TRACE(osip_trace
                       (__FILE__, __LINE__, OSIP_ERROR, NULL,
                        "eXosip: eXosip register send register OSIP_WRONG_STATE! Return"));
#ifndef WIN32
	...
	if (i != 0) {
        ...
        OSIP_TRACE(osip_trace
                   (__FILE__, __LINE__, OSIP_ERROR, NULL,
                    "eXosip: eXosip register send register! Return i = %d",i));
		return i;
	}
    ...
    OSIP_TRACE(osip_trace
               (__FILE__, __LINE__, OSIP_ERROR, NULL,
                "eXosip: eXosip register send register begins"));
