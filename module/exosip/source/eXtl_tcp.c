...
static int tcp_tl_open(void)
{
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
		...
		OSIP_TRACE(osip_trace
			(__FILE__, __LINE__, OSIP_ERROR, NULL,
			"bind sock=%d node:%s family:%d %s, func:%s\n",
			sock, eXtl_tcp.proto_ifs, curinfo->ai_family,
			strerror(ex_errno),__FUNCTION__));
...
static char *buffer_find(const char *haystack, size_t haystack_len, const char *needle)
{
	...
	while (search < end &&
		   (p = (char *)memchr(search, *needle, end - search)) != NULL) {
		if (p + len > end)
			break;
...
static int handle_messages(struct _tcp_sockets *sockinfo)
{
	...
	//服务器心跳包回的是两个字节处理---张俊良20140814, begin
	if(2==buflen&&(end_headers = buffer_find(buf, buflen, END_HEADERS_STR1)) != NULL)
	{	
		if (buf == end_headers)
		{
		/* skip tcp standard keep-alive */
			OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
									"socket %s:%i: standard keep alive received (CRLF)\n",
									sockinfo->remote_ip, sockinfo->remote_port));
			consumed = 2;
			sockinfo->send_keepalive_count = 0;
			return consumed;
		}
	}
	////////////////////////////////////张俊良20140814, end
...
static int _tcp_tl_connect_socket(char *host, int port)
{
	...
	struct addrinfo *bindaddrinfo = NULL;
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
...
#else
		{
			...
			val = 10;		/* 30 sec before starting probes */
...

		{
			...
			laddr.sin_addr.s_addr= inet_addr(eXtl_tcp.proto_ifs);
			if( bind(sock,(struct sockaddr*)&laddr,sizeof(laddr)) < 0) {
				OSIP_TRACE(osip_trace
					(__FILE__, __LINE__, OSIP_INFO2, NULL,
					"Cannot bind local ip:%s \n",
					eXtl_tcp.proto_ifs));
			}
		}
					...
					int port, int out_socket)
{
	...
	OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
						  ...
						  host, port, message));
	OSIP_MESSAGE_HOOK(osip_message_hook(0,host,port,message));
	i = _tcp_tl_send(out_socket, (const char *)message, length);
	osip_free(message);
	return i;
}
...
static int tcp_tl_keepalive(void)
{
	...
	if (tcp_socket <= 0)
    {
        OSIP_TRACE(osip_trace
                   (__FILE__, __LINE__, OSIP_ERROR, NULL,
                    "[DEBUG] sean %s error happens, tcp_socket:%d\n",__FUNCTION__,tcp_socket));
		return OSIP_UNDEFINED_ERROR;
		...
		if (tcp_socket_tab[pos].socket > 0) {
			...
			if (i > 0) {
				...
				if(eXosip.keep_alive > 0 &&
					++tcp_socket_tab[pos].send_keepalive_count > 3) {
					_tcp_tl_close_sockinfo(&tcp_socket_tab[pos]);
					close_socket_count++;
				}
				continue;
				...
				if( eXosip.keep_alive > 0 && 
					...
					OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_ERROR, NULL,
						   "Keep Alive no response more than three times,disconnect\n"));
					close_socket_count++;
					continue;
				}
			...
			if (eXosip.keep_alive > 0) {
				...
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_INFO2, NULL,
							"tcp_tl_keepalive send ret =%d\n",i));
			}
		}
	}
    OSIP_TRACE(osip_trace
               (__FILE__, __LINE__, OSIP_ERROR, NULL,
                "[DEBUG] sean %s ends...,close_socket_count:%d\n",__FUNCTION__,close_socket_count));
	return close_socket_count;
}
...
static int tcp_tl_stop_keepalive(const char *ip, int port)
{
    int socPos = _tcp_tl_find_socket((char *)ip, port);
    if (socPos >= 0)
    {
        struct _tcp_sockets *socinfo = _tcp_tl_find_sockinfo(tcp_socket_tab[socPos].socket);
        if (socinfo)
        {
            _tcp_tl_close_sockinfo(socinfo);
        }
        else
