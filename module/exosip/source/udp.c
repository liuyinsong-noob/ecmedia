	...
	for (jc = eXosip.j_calls; jc != NULL; jc = jc->next) {
		...
		for (jd = jc->c_dialogs; jd != NULL; jd = jd->next) {
			...
			while (!osip_list_eol(jd->d_inc_trs, pos)) {
				tr = (osip_transaction_t *)osip_list_get(jd->d_inc_trs, pos);
	...
	if (jd != NULL && transaction != NULL && evt->sip != NULL
		...
		if (jd->d_dialog != NULL && jd->d_dialog->remote_cseq > 0) {
			...
			if (cseq < jd->d_dialog->remote_cseq) {
				...
				return;*/
				sprintf(buf, "Wrong Lower CSeq is %d %d\n",cseq,jd->d_dialog->remote_cseq);
				OSIP_MESSAGE_HOOK(osip_message_hook(1,"local",0,buf));
			}
			if (cseq == jd->d_dialog->remote_cseq) {
								...
								char *host, int port)
{
	...
	OSIP_MESSAGE_HOOK(osip_message_hook(1,host,port,buf));
	buf[length]=tmp;
...
int eXosip_read_message(int max_message_nb, int sec_max, int usec_max)
{
	...
	while (max_message_nb != 0 && eXosip.j_stop_ua == 0) {
		...
		if ((i == -1) && (errno == EINTR || errno == EAGAIN)) {
			OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"select error, continue to select \n"));
			continue;
		}
#endif
#ifdef OSIP_MT
...
eXosip_release_finished_transactions(eXosip_call_t * jc, eXosip_dialog_t * jd)
{
	...
	if (jd != NULL) {
		...
		pos = 1;
		while (!osip_list_eol(jd->d_inc_trs, pos)) {
			inc_tr = (osip_transaction_t *)osip_list_get(jd->d_inc_trs, pos);
		...
		while (!osip_list_eol(jd->d_out_trs, pos)) {
			out_tr = (osip_transaction_t *)osip_list_get(jd->d_out_trs, pos);
	...
	for (jc = eXosip.j_calls; jc != NULL;) {
		...
		if (jc->c_dialogs == NULL) {
			if (jc->c_inc_tr != NULL
				 ...
				 */
				/*OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
					"eXosip:wait here for user action to delete the call ");
					   ...
					   && jc->c_out_tr->state == ICT_TERMINATED
					   && jc->c_out_tr->completed_time + 10 > now 
					   &&jc->c_out_tr->last_response != NULL
					   &&jc->c_out_tr->last_response->status_code == 407) {
						   /*in the case we need to wait the UA send the invite again
						   before we release the call.
						   */
			}
