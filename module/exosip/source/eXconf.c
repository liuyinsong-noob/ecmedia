...
#ifndef WIN32
extern int g_RegisterTimerFlag;
extern time_t g_RegisterTimerB;
#endif
int ipv6_enable = 0;
...
void eXosip_quit(void)
{
...

	{
		...
		for (ev = (eXosip_event_t *)osip_fifo_tryget(eXosip.j_events); ev != NULL;
			 ev = (eXosip_event_t *)osip_fifo_tryget(eXosip.j_events))
...
int eXosip_check_sip_active()
{
#ifndef WIN32
    int pthread_kill_err;
    
    pthread_kill_err = pthread_kill(*(pthread_t *)eXosip.j_thread,0);
    
//    OSIP_TRACE(osip_trace
//               (__FILE__, __LINE__, OSIP_ERROR, NULL,
//                "seansean eXosip_check_sip_active eXosip.j_thread : %p\n",eXosip.j_thread));
    if(pthread_kill_err == ESRCH)
    {
        OSIP_TRACE(osip_trace
                   (__FILE__, __LINE__, OSIP_ERROR, NULL,
                    "eXosip: 0x%x thread not exists or abort\n",(unsigned int)eXosip.j_thread));
        return -1;
    }
...
int eXosip_reinit_network(void)
{
	eXosip.restart_network = 1;
	__eXosip_wakeup();
	return OSIP_SUCCESS;
}
...
int eXosip_execute(void)
{
	...
	int i;
	int keep_alive_gap =(int) (eXosip.last_keep_alive_time + eXosip.keep_alive / 1000- time(NULL));
	if( keep_alive_gap <= 0)
		keep_alive_gap = 0;

#ifdef OSIP_MT
	osip_timers_gettimeout(eXosip.j_osip, &lower_tv);
	if( lower_tv.tv_sec > keep_alive_gap ) {
		lower_tv.tv_sec = keep_alive_gap;
	}
	if (lower_tv.tv_sec > 10) {
		...
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_INFO4, NULL,
					"eXosip: Reseting timer to sec:%i usec:%i!\n",
	...
	if (i == -2000) {
		OSIP_TRACE(osip_trace(__FILE__, __LINE__, 
				OSIP_ERROR, NULL,"eXosip: read message error\n"));
		return -2000;
	}
	if( eXosip.restart_network ) {
		eXosip_reg_t *jr;
		OSIP_TRACE(osip_trace(__FILE__, __LINE__,
			OSIP_ERROR, NULL,"eXosip: try to restart network\n"));

		eXosip.restart_network = 0;		
		if(eXosip.eXtl) {
			eXosip.eXtl->tl_free();
			eXosip.eXtl->tl_open();
			...
			eXosip_lock();
//            OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                  OSIP_ERROR, NULL,"seansean eXosip_execute before enter for circle 111\n"));
			for (jr = eXosip.j_reg; jr != NULL; jr = jr->next) {
				if (jr->r_id >= 1 && jr->r_last_tr != NULL) {
//                    OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                          OSIP_ERROR, NULL,"seansean eXosip_execute in for circle jr addr:%p, jr->r_registrar:%s, jr->r_last_tr:%p,jr->r_id:%d\n",jr,jr->r_registrar,jr->r_last_tr,jr->r_id));
...
static void _eXosip_keep_alive(void)
{
	...
	int remain_time =   eXosip.last_keep_alive_time 
			+ eXosip.keep_alive /1000 - time(NULL);
    
    if (remain_time <= 0 || remain_time > eXosip.keep_alive/1000) {
        OSIP_TRACE(osip_trace
                   (__FILE__, __LINE__, OSIP_INFO4, NULL,
                    "keep alive:\n"));
        
        eXosip.last_keep_alive_time = time(NULL);
        
        eXtl_udp.tl_keepalive();
        //some socket was closed duo to keepalive no response
        //so we need to resend register
        if( strcasecmp(eXosip.transport,"TCP") == 0 && eXtl_tcp.tl_keepalive()!= 0 )
        {
            eXosip_reg_t *jr;
#ifndef WIN32
            ...
            eXosip_lock();
//            OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                  OSIP_ERROR, NULL,"seansean _eXosip_keep_alive before enter for circle 111\n"));
            for (jr = eXosip.j_reg; jr != NULL; jr = jr->next) {
                if (jr->r_id >= 1 && jr->r_last_tr != NULL) {
//                    OSIP_TRACE(osip_trace(__FILE__, __LINE__,
//                                          OSIP_ERROR, NULL,"seansean _eXosip_keep_alive in for circle jr addr:%p, jr->r_registrar:%s, jr->r_last_tr:%p,jr->r_id:%d\n",jr,jr->r_registrar,jr->r_last_tr,jr->r_id));
                    eXosip_register_send_register(jr->r_id, NULL);
                }
            }
            eXosip_unlock();
...
//        eXtl_tls.tl_keepalive();
        if( strcasecmp(eXosip.transport,"TLS") == 0 && eXtl_tls.tl_keepalive()!= 0 )
        {
            eXosip_reg_t *jr;
            eXosip_lock();
            for (jr = eXosip.j_reg; jr != NULL; jr = jr->next) {
		...
		OSIP_TRACE(osip_trace
                   (__FILE__, __LINE__, OSIP_INFO4, NULL,
                    "still need %ds to send keep alive\n",remain_time));
		return;
	}
	
}

void eXosip_send_keep_alive()
{
	eXosip.last_keep_alive_time = 0;
	__eXosip_wakeup();
...
void *_eXosip_thread(void *arg)
{
	...
	while (eXosip.j_stop_ua == 0) {
		...
		if (i == -2000) {
			OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"eXosip: _eXosip_thread exit due to execute error\n"));
			osip_thread_exit();
		}
	}
	osip_thread_exit();
