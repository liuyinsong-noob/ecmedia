...
extern char * ca_certificate_path;
...
ANDROID
	...
	/* Load the CAs we trust */
	{
...
#else
		{
			...
			if (stat(ca_certificate_path, &fileStat) != 0) {
				OSIP_TRACE(osip_trace
					(__FILE__, __LINE__, OSIP_ERROR, NULL,"stat of %s failed: %s\n",ca_certificate_path,strerror(errno)));
			} else {
				if (S_ISDIR(fileStat.st_mode)) {
					caFolder = ca_certificate_path;
				} else {
		...
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_INFO3, NULL,
					"eXosip: Trusted CA %s : '%s'\n", caFolder?"folder":"file", ca_certificate_path));
	...
	if (_tls_add_certificates(ctx) <= 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"1Cannot load certificates from Microsoft Certificate Store\n"));
	}
							   ...
							   const char *password, int transport)
{
	...
	if (_tls_add_certificates(ctx) <= 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"2Cannot load certificates from Microsoft Certificate Store\n"));
	}
					...
					int port, int out_socket)
{
	...
	OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
						  ...
						  host, port, message));
	OSIP_MESSAGE_HOOK(osip_message_hook(0,host,port,buf));
            ...
            if (i > 0) {
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_INFO4, NULL,
							"tls_tl_keepalive socket node:%s:%i, socket %d [pos=%d], in progress\n",
							tls_socket_tab[pos].remote_ip,
							tls_socket_tab[pos].remote_port,
							tls_socket_tab[pos].socket, pos));
				if(eXosip.keep_alive > 0 &&
                   ++tls_socket_tab[pos].send_keepalive_count > 3) {
					_tls_tl_close_sockinfo(&tls_socket_tab[pos]);
					close_socket_count++;
				}
				continue;
				...
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_INFO4, NULL,
							"tls_tl_keepalive socket node:%s:%i , socket %d [pos=%d], connected\n",
							tls_socket_tab[pos].remote_ip,
							tls_socket_tab[pos].remote_port,
							tls_socket_tab[pos].socket, pos));
				if( eXosip.keep_alive > 0 &&
				...
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_ERROR, NULL,
							"tls_tl_keepalive socket node:%s:%i, socket %d [pos=%d], socket error\n",
							tls_socket_tab[pos].remote_ip,
							tls_socket_tab[pos].remote_port,
							tls_socket_tab[pos].socket, pos));
				_tls_tl_close_sockinfo(&tls_socket_tab[pos]);
...
eXosip_tls_ctx_error eXosip_tls_verify_certificate(int
												   _tls_verify_client_certificate)
{
	return (eXosip_tls_ctx_error)-1; /* NOT IMPLEMENTED */
}
