...
int eXosip_event_init(eXosip_event_t ** je, int type)
{
	*je = (eXosip_event_t *) osip_malloc(sizeof(eXosip_event_t));
	...
	(*je)->type = (eXosip_event_type_t)type;
...
void eXosip_event_free(eXosip_event_t * je)
{
    ...
    if (je->type == EXOSIP_STUN_PACKET)
    {
        OrtpEventData *ed=ortp_event_get_data(je->stun_event);
        free(ed->packet);
        free(ed->ep);
		//hubin resolve memory leak
		ortp_free(je->stun_event->b_datap);
        ortp_free(je->stun_event);
    }
