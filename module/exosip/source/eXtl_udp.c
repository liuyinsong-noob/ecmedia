					...
					int port, int out_socket)
{
	...
	OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
						  ...
						  ipbuf, port, message));
	OSIP_MESSAGE_HOOK(osip_message_hook(0,ipbuf,port,message));
	...
	if (tr != NULL) {
		if (tr->ict_context != NULL){
			tmp1=osip_strdup(ipbuf);
			osip_ict_set_destination(tr->ict_context, tmp1, port);
//			osip_ict_set_destination(tr->ict_context, osip_strdup(ipbuf), port);
		}
		if (tr->nict_context != NULL){
			tmp1=osip_strdup(ipbuf);
			osip_nict_set_destination(tr->nict_context, tmp1, port);
//			osip_nict_set_destination(tr->nict_context, osip_strdup(ipbuf), port);
		}
	}
	if (0 >
		(ret =sendto(udp_socket, (const void *) message, length, 0,
