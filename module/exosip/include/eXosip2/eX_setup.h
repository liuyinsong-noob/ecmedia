 ...
 */
 ...
 /**
  *Re initialize all network socket duo to network config changed
  */
...
/**
 * Try to lock eXtended oSIP library (pthread only)
 * Returns -EBUSY if already locked, -ENOTSUPP if not supported, 0 if successful.
**/
...
/**
 * Check osip thread is active or not
 */
