...
WebRtc_Word32 SetAndroidAudioDeviceObjects(void* javaVM, void* env,
                                           void* context)
{
    WEBRTC_TRACE(kTraceStateInfo, kTraceAudioDevice, -1, "%s",
                 __FUNCTION__);
...
bool AudioDeviceAndroidJni::PlayThreadProcess()
{
    ...
    if (!_playing)
    {
        switch (_timeEventPlay.Wait(1000000))
        {
            case kEventSignaled:
...
bool AudioDeviceAndroidJni::RecThreadProcess()
{
    ...
    if (!_recording)
    {
        switch (_timeEventRec.Wait(1000000))
        {
            case kEventSignaled:
                WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice,
...
//                         "total delay is %d", msPlayDelay + _delayRecording);
            ...
            if(!_micphoneMuted)
                ...
                memset(_recBuffer, 0, 2*samplesToRec);
            }
