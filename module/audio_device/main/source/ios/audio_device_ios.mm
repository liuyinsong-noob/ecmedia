...
AudioDeviceIOS::AudioDeviceIOS(const int32_t id)
    ...
    WEBRTC_TRACE(kTraceMemory, kTraceAudioDevice, id,
        ...
        if ([deviceString hasPrefix:@"iPhone8"]) {
            _isIphone6s = true;
        }
...
void AudioDeviceIOS::AttachAudioBuffer(AudioDeviceBuffer* audioBuffer) {
    ...
    if (_isIphone6s) {
        _ptrAudioBuffer->SetRecordingSampleRate(ENGINE_REC_BUF_SIZE_IN_SAMPLES_6s);
        _ptrAudioBuffer->SetPlayoutSampleRate(ENGINE_PLAY_BUF_SIZE_IN_SAMPLES_6s);
    } else {
...
int32_t AudioDeviceIOS::SpeakerIsAvailable(bool& available) {
    WEBRTC_TRACE(kTraceModuleCall, kTraceAudioDevice, _id,
                 "%s", __FUNCTION__);
...
int32_t AudioDeviceIOS::MicrophoneIsAvailable(bool& available) {
    WEBRTC_TRACE(kTraceModuleCall, kTraceAudioDevice, _id,
                 "%s", __FUNCTION__);
...
int32_t AudioDeviceIOS::MicrophoneMuteIsAvailable(bool& available) {
 ...
 //   available = false;  // Mic mute not supported on iOS
    ...
    available = true; // use _micphoneMuted 
...
int32_t AudioDeviceIOS::InitPlayOrRecord() {
    ...
    if (result == kAudioSessionNotInitialized) {
        AudioSessionInitialize(NULL,NULL, NULL, NULL);
        result = AudioSessionSetActive(YES);
    }
    if (0 != result) {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
                     "  Could not set audio session active (result=%d)",
                     result);
        return -1;
    }
    ...
    if (playoutDesc.mSampleRate == 48000) {
        _adbSampFreq = 48000;
...
int32_t AudioDeviceIOS::ShutdownPlayOrRecord() {
    ...
    if (NULL != _auVoiceProcessing) {
        ...
        if (0 != result) {
            WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                         "  Error Uninitialize Audio Unit (result=%d)", result);
        }
        ...
        if (0 != result) {
            WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
                         "  Could not set audio session inactive (result=%d)",
                         result);
        }
        _auVoiceProcessing = NULL;
    }
