...
GLint compileShader(GLuint *shader, GLenum type, const char *sources)
{
	...
	if (!sources)
	{
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0,"Failed to load vertex shader");
		return 0;
	}
    ...
    *shader = glCreateShader(type);				// create shader
    ...
    if (logLength > 1)
    {
        ...
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0, "Shader compile log:\n%s", log);
        free(log);
    }
    ...
    if (status == GL_FALSE)
	{
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0,"Failed to compile shader\n");
	}
...
GLint linkProgram(GLuint prog)
{
    ...
    if (logLength > 1)
    {
        ...
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0, "Program link log:\n%s", log);
        free(log);
    }
    ...
    if (status == GL_FALSE)
    {
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0,"Failed to link program %d", prog);        
    }
...
GLint validateProgram(GLuint prog)
{
    ...
    if (logLength > 0)
    {
        char *log = (char *)malloc(logLength);
        glGetProgramInfoLog(prog, logLength, &logLength, log);
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0, "Program validate log:\n%s", log);
        free(log);
    }
    
    glGetProgramiv(prog, GL_VALIDATE_STATUS, &status);
    if (status == GL_FALSE)
		WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, 0, "Failed to validate program %d", prog);
