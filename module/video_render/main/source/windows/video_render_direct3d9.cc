...
int D3D9Channel::GetSurfaceWidth()
{
    return _width;
}

int D3D9Channel::GetSurfaceHeight()
{
    return _height;
}
...
int D3D9Channel::FrameSizeChange(int width, int height, int numberOfStreams)
{
    ...
    if (_pd3dDevice)
#ifdef RENDER_USE_SURFACE
		ret = _pd3dDevice->CreateOffscreenPlainSurface(_width, _height, D3DFMT_X8R8G8B8, 
														D3DPOOL_DEFAULT, &_pSurface, NULL);
          if (FAILED(ret))
    {
        _pSurface = NULL;
        return -1;
...
int D3D9Channel::DeliverFrame(const I420VideoFrame& videoFrame) {
  ...
  if (FAILED(_pSurface->LockRect(&lr, NULL, 0))) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
  ...
  if (FAILED(_pSurface->UnlockRect())) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
...
LPDIRECT3DSURFACE9 D3D9Channel::GetSurface()
{
    CriticalSectionScoped cs(_critSect);
    return _pSurface;
}
...
int D3D9Channel::RecreateSurface(LPDIRECT3DDEVICE9 pd3DDevice)
{
    CriticalSectionScoped cs(_critSect);
	...
	ret = _pd3dDevice->CreateOffscreenPlainSurface(_width, _height, D3DFMT_A8R8G8B8, 
		D3DPOOL_DEFAULT, &_pSurface, NULL);
                                            ...
                                            D3DPRESENT_PARAMETERS* pd3dpp)
{
    ...
    {
		WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
			"VideoRenderDirect3D9::InitializeD3D  Direct3DCreate9 fail [1]");
        return -1;
    }
                                   ...
                                   &_pd3dDevice)))
    {
        ...
        {
			WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
				"VideoRenderDirect3D9::InitializeD3D  CreateDevice fail [2], dwVertexProcessing=%d, hwnd=0x%x", dwVertexProcessing, hWnd);
            return -1;
        }
    }
...
int VideoRenderDirect3D9::ResetDevice()
{
    ...
    while (it != _d3dChannels.end())
    {
        ...
        {
#ifdef RENDER_USE_SURFACE
            it->second->RecreateSurface(_pd3dDevice);
#else
...
#ifdef RENDER_USE_SURFACE
	GetClientRect(_hWnd, &_originalHwndRect);
	{
		WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
			"VideoRenderDirect3D9::InitDevice info: top:%d,left=%d,bottom=%d,right=%d",
			_originalHwndRect.top, _originalHwndRect.left, 
			_originalHwndRect.bottom, _originalHwndRect.right);
	}
    ...
    if (SUCCEEDED(_pd3dDevice->BeginScene()))
    {
...
#ifdef RENDER_USE_SURFACE
        LPDIRECT3DSURFACE9 surfaceFromChannel = NULL;
        DWORD surfaceWidth, surfaceHeight;
#else
        _pd3dDevice->SetStreamSource(0, _pVB, 0, sizeof(CUSTOMVERTEX)); //将顶点缓冲区绑定到设备数据流。
        _pd3dDevice->SetFVF(D3DFVF_CUSTOMVERTEX); //Gets the fixed vertex function declaration.
        ...
        while (it != _d3dZorder.rend())
        {
            ...
            if (ddIt != _d3dChannels.end())
            {
                ...
                {
#ifdef RENDER_USE_SURFACE
                    surfaceFromChannel = channelObj->GetSurface();
                    surfaceWidth = channelObj->GetSurfaceWidth();
                    surfaceHeight = channelObj->GetSurfaceHeight();

					// Copy offscreen buffer to screen
					LPDIRECT3DSURFACE9 pScreenSurface = NULL;
					if (FAILED(_pd3dDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pScreenSurface)))
					{
						WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
							"Failed to GetBackBuffer.");
					}
					
					/*if (FAILED(_pd3dDevice->UpdateSurface(surfaceFromChannel, NULL, pScreenSurface, NULL)))
					{
						WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
							"Failed to UpdateSurface.");	
					}*/

					if (FAILED(_pd3dDevice->StretchRect(surfaceFromChannel, NULL, pScreenSurface, &_originalHwndRect, D3DTEXF_LINEAR)))
					{
						WEBRTC_TRACE(kTraceError, kTraceVideo, -1,
							"Failed to StretchRect.");
					}
#else
                    ...
                    _pd3dDevice->SetTexture(0, textureFromChannel); //????àí·?????éè±?ìùí?2?
