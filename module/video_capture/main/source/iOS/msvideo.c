...
MSPicture* copy_ycbcrbiplanar_to_true_yuv_with_rotation_and_down_scale_by_2(MSPicture *pict, uint8_t* y, uint8_t * cbcr, int rotation, int w, int h, int y_byte_per_row,int cbcr_byte_per_row, bool_t uFirstvSecond, bool_t down_scale) {
//	MSPicture *pict = (MSPicture *)malloc(sizeof(MSPicture));
	int uv_w;
	int uv_h;
	uint8_t* ysrc;
	uint8_t* ydst;
...
//    pict->ptr = (uint8_t *)malloc((w*h*3)/2);
//    pict->w=w;
//	pict->h=h;
//    pict->planes[0] = pict->ptr;
//    pict->planes[1] = pict->ptr + w*h;
//    pict->planes[2] = pict->planes[1] + w*h/4;
//    pict->planes[3] = 0;
    
    pict->strides[0]=w;
	pict->strides[1]=w/2;
	pict->strides[2]=pict->strides[1];
	pict->strides[3]=0;
    
//	if (down_scale && !hasNeon) {
//        printf("down scaling by two requires NEON, returning empty block");
//		return NULL;
//	}
		...
		if (rotation == 0) {
...
#endif
			{
                if (down_scale) {
                    ...
                    //plain copy with down_scale
                    uint8_t *ysrc_ptr = y;
                    uint8_t *ydest_ptr = pict->planes[0];
                    uint8_t *cbcrsrc_ptr = cbcr;
                    uint8_t *udest_ptr = u_dest;
                    uint8_t *vdest_ptr = v_dest;
                    int crcb_dest_offset=0;
                    for (yy = 0; yy<src_h; yy+=y_inc) {
                        for (x = 0; x<src_w; x+=x_inc) {
                            for (int kk = 0; kk < 16; kk++) {
                                memcpy(ydest_ptr++, ysrc_ptr++, 1);
                                ysrc_ptr++;
                            }
                            
                        }
                        ysrc_ptr = y + yy*y_byte_per_row;
                    }
                    
                    for (yy = 0; yy<src_h>>1; yy+=y_inc) {
                        for (x=0; x<src_w; x+=x_inc) {
                            for (int kk = 0; kk < 8; kk++) {
                                memcpy(udest_ptr++, cbcrsrc_ptr++, 1);
                                memcpy(vdest_ptr++, cbcrsrc_ptr++, 1);
                                cbcrsrc_ptr+=2;
                            }
                        }
                        cbcrsrc_ptr= cbcr +  yy * cbcr_byte_per_row;
                        crcb_dest_offset+=down_scale?(src_w>>2):(src_w>>1);
                        udest_ptr=u_dest + crcb_dest_offset;
                        vdest_ptr=v_dest + crcb_dest_offset;
                    }
                }else
                {
...
#endif
            {
                ...
                for(int j = h - 1; j >= 0; j--)
                {
                    for(int i = w; i > 0; i--)
                    {
                        ydst[n++] = y[y_byte_per_row*j + i];
                    }  
                }

				// 180° rotation + de-interlace u/v
                
                for (int i = uv_h - 1; i>=0; i--) {
                    for (int j = uv_w-1; j>=0; j--) {
                        *u_dest++ = cbcr[cbcr_byte_per_row*i+2*j];
                        *v_dest++ = cbcr[cbcr_byte_per_row*i+2*j+1];
                    }
                }
...
void ms_video_init_average_fps(MSAverageFPS* afps, const char* ctx) {
	...
	if (!ctx || strstr(ctx, "%f") == 0) {
		printf("Invalid MSAverageFPS context given '%s' (must be not null and must contain one occurence of '%%f'", ctx);
	}
}
