...
int32_t VideoCaptureDS::SetCaptureSettings(
	VideoCaptureCapability settings)
{
	if (_requestedCapability.width!=settings.width || _requestedCapability.height!=settings.height
		||_requestedCapability.maxFPS != settings.maxFPS)
	{
		DisconnectGraph();
		_requestedCapability = settings;
