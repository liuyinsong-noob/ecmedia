...
VideoCaptureModule* VideoCaptureImpl::Create(const int32_t id,
                                             const char* device_id, VideoCaptureCapability *settings) {
  if (device_id == NULL)
    return NULL;
  ...
  if (settings)
  {
	  capture->_requestedCapability.width = settings->width;
	  capture->_requestedCapability.height = settings->height;
	  capture->_requestedCapability.maxFPS = settings->maxFPS;
  }
