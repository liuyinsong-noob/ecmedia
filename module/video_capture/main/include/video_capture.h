...
class VideoCaptureModule: public RefCountedModule {
 public:
  ...
  virtual int32_t SetCaptureSettings(VideoCaptureCapability settings)=0;
  ...
  virtual void EnableNoPictureAlarm(const bool enable) = 0;
    //Set the local video preview window
    virtual WebRtc_Word32 SetPreviewWindow(void* window) = 0;
protected:
