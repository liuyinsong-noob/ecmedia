...
int16_t WebRtcOpus_EncoderCreate(OpusEncInst** inst, int32_t channels, int32_t freq) {
  OpusEncInst* state;
  if (inst != NULL) {
    ...
    if (state) {
      ...
      state->encoder = opus_encoder_create(freq, channels, application,
                                           &error);
      state->in_dtx_mode = 0;
...
int16_t WebRtcOpus_DecoderCreate(OpusDecInst** inst, int channels, int freq_samples) {
  int error;
  OpusDecInst* state;
  ...
  if (inst != NULL) {
    ...
    state->decoder = opus_decoder_create(freq_samples, channels, &error);
...
static int DecodeNative(OpusDecInst* inst, const uint8_t* encoded,
                        ...
                        int16_t* decoded, int16_t* audio_type, int decode_fec, int fs) {
  int res = opus_decode(inst->decoder, encoded, encoded_bytes,
                        (opus_int16*)decoded, frame_size, decode_fec, fs);
...
int16_t WebRtcOpus_Decode(OpusDecInst* inst, const uint8_t* encoded,
                          ...
                          int16_t* audio_type, int fs) {
  int decoded_samples;

  if (encoded_bytes == 0) {//不会走到
    *audio_type = DetermineAudioType(inst, encoded_bytes);
    decoded_samples = WebRtcOpus_DecodePlc(inst, decoded, 1, fs);
  } else {
...
int16_t WebRtcOpus_DecodePlc(OpusDecInst* inst, int16_t* decoded,
                             int16_t number_of_lost_frames, int fs) {
...
int16_t WebRtcOpus_DecodeFec(OpusDecInst* inst, const uint8_t* encoded,
                             int16_t encoded_bytes, int16_t* decoded,
                             int16_t* audio_type, int16_t fs) {
  int decoded_samples;
  int fec_samples;
//sean for mos test
//  if (WebRtcOpus_PacketHasFec(encoded, encoded_bytes, fs) != 1) {
//      return 0;
//  }

  fec_samples = opus_packet_get_samples_per_frame(encoded, fs);

  decoded_samples = DecodeNative(inst, encoded, encoded_bytes,
                                 fec_samples, decoded, audio_type, 1, fs);
  if (decoded_samples < 0) {//不会走到
    return -1;
  }
...
int WebRtcOpus_DurationEst(OpusDecInst* inst,
  ...
  samples = frames * opus_packet_get_samples_per_frame(payload, fs);
...
int WebRtcOpus_FecDurationEst(const uint8_t* payload,
                              ...
                              int fs) {
  int samples;
  if (WebRtcOpus_PacketHasFec(payload, payload_length_bytes, fs) != 1) {
    return 0;
  }

  samples = opus_packet_get_samples_per_frame(payload, fs);
  if (samples < 120 || samples > 5760) {
//      printf("sean haha Invalid payload duration %d(480<= samples <=5760)\n",samples);
    /* Invalid payload duration. */
    return 0;
  }
  return samples;
...
int WebRtcOpus_PacketHasFec(const uint8_t* payload,
  ...
  payload_length_ms = opus_packet_get_samples_per_frame(payload, fs) / (fs/1000);
...
//      printf("sean haha frame_data:%02x,n:%d, frames:%d\n",frame_data[0][0],n,frames);
      if (frame_data[0][0] & (0x80 >> ((n + 1) * (frames + 1) - 1))) {
//        printf("sean haha fec is true\n");
        return 1;
      }
  }
