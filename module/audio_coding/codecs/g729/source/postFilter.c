...
{
	int i,j;
	word16_t *residualSignal,*scaledResidualSignal;
	...
	correlationMax = (word32_t)MININT32;
	...
	if ((MULT16_16(correlationMaxWord16, correlationMaxWord16) < SHR(MULT16_16(residualSignalEnergyWord16, delayedResidualSignalEnergyWord16), 1)) /* eq82 */
		...
		if (correlationMax > delayedResidualSignalEnergy) {
			...
			//避免出现除0异常
			word32_t nDiv = 0;
			nDiv = (word32_t)ADD32(SHL32(delayedResidualSignalEnergyWord16,1), correlationMaxWord16);
			if(nDiv == 0) nDiv = 1;
			g1 = DIV32((word32_t)SHL32(correlationMaxWord16,15),nDiv); /* g1 in Q15 */
