...
/*                                                                           */
/*****************************************************************************/
int16_t initBcg729EncoderChannel(bcg729EncoderChannelContextStruct *encInst)
{
...
//	bcg729EncoderChannelContextStruct *encoderChannelContext = malloc(sizeof(bcg729EncoderChannelContextStruct));
	if(encInst)
	{
		...
		memset(encInst->signalBuffer, 0, (L_LP_ANALYSIS_WINDOW-L_FRAME)*sizeof(word16_t)); /* set to zero all the past signal */
		encInst->signalLastInputFrame = &(encInst->signalBuffer[L_LP_ANALYSIS_WINDOW-L_FRAME]); /* point to the last frame in the signal buffer */
		encInst->signalCurrentFrame = &(encInst->signalBuffer[L_LP_ANALYSIS_WINDOW-L_SUBFRAME-L_FRAME]); /* point to the current frame */
		memcpy(encInst->previousLSPCoefficients, previousLSPInitialValues, NB_LSP_COEFF*sizeof(word16_t)); /* reset the previous quantized and unquantized LSP vector with the same value */
		memcpy(encInst->previousqLSPCoefficients, previousLSPInitialValues, NB_LSP_COEFF*sizeof(word16_t));
		memset(encInst->weightedInputSignal, 0, MAXIMUM_INT_PITCH_DELAY*sizeof(word16_t)); /* set to zero values of previous weighted signal */
		memset(encInst->excitationVector, 0, L_PAST_EXCITATION*sizeof(word16_t)); /* set to zero values of previous excitation vector */
		memset(encInst->targetSignal, 0, NB_LSP_COEFF*sizeof(word16_t)); /* set to zero values filter memory for the targetSignal computation */
		encInst->lastQuantizedAdaptativeCodebookGain = O2_IN_Q14; /* quantized gain is initialized at his minimum value: 0.2 */

		/* initialisation of the differents blocs which need to be initialised */
		initPreProcessing(encInst);
		initLSPQuantization(encInst);
		initGainQuantization(encInst);	
	}
	else
	{
		return -1;
