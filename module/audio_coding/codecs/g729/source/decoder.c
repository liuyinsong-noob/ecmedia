...
/*                                                                           */
/*****************************************************************************/
int16_t initBcg729DecoderChannel(bcg729DecoderChannelContextStruct *decInst)
{
...
//	bcg729DecoderChannelContextStruct *decoderChannelContext = malloc(sizeof(bcg729DecoderChannelContextStruct));
	...
	if(decInst)
	{
		...
		memcpy(decInst->previousqLSP, previousqLSPInitialValues, NB_LSP_COEFF*sizeof(word16_t)); /* initialise the previousqLSP buffer */
		memset(decInst->excitationVector, 0, L_PAST_EXCITATION*sizeof(word16_t)); /* initialise the part of the excitationVector containing the past excitation */
		decInst->boundedAdaptativeCodebookGain = BOUNDED_PITCH_GAIN_MIN;
		decInst->pseudoRandomSeed = 21845; /* initialise pseudo Random seed according to spec 4.4.4 */
		decInst->adaptativeCodebookGain = 0; /* gains are initialised at 0 */
		decInst->fixedCodebookGain = 0;
		memset(decInst->reconstructedSpeech, 0, NB_LSP_COEFF*sizeof(word16_t)); /* initialise to zero all the values used from previous frame to get the current frame reconstructed speech */

		/* initialisation of the differents blocs which need to be initialised */
		initDecodeLSP(decInst);
		initDecodeAdaptativeCodeVector(decInst);
		initDecodeGains(decInst);
		initPostFilter(decInst);
		initPostProcessing(decInst);	
	}
	else
		return -1;
