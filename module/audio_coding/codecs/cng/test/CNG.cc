...
int main(int argc, char* argv[])
{
    ...
    CloopenWebRtcVad_Create(&vinst);
    CloopenWebRtcVad_Init(vinst);
...
#ifndef ASSIGN
    res=CloopenWebRtcCng_CreateEnc(&e_inst);
    if (res < 0) {
    ...
    res=CloopenWebRtcCng_CreateDec(&d_inst);
    if (res < 0) {
        /* exit if returned with error */
        errtype=CloopenWebRtcCng_GetErrorCodeDec(d_inst);
        fprintf(stderr,"\n\n Error in initialization: %d.\n\n", errtype);
        exit(EXIT_FAILURE);
    }
    ...
    res=CloopenWebRtcCng_InitEnc(e_inst,atoi(argv[3]),atoi(argv[4]),atoi(argv[5]));
    if (res < 0) {
        /* exit if returned with error */
        errtype=WebRtcCng_GetErrorCodeEnc(e_inst);
        fprintf(stderr,"\n\n Error in initialization: %d.\n\n", errtype);
    ...
    res=CloopenWebRtcCng_InitDec(d_inst);
    if (res < 0) {
        /* exit if returned with error */
        errtype=CloopenWebRtcCng_GetErrorCodeDec(d_inst);
        fprintf(stderr,"\n\n Error in initialization: %d.\n\n", errtype);
    ...
    while(fread(anaSpeech,2,frameLen,infile)==frameLen){
        ...
        //        if(!CloopenWebRtcVad_Process(vinst, fs, anaSpeech, frameLen)){
        ...
        if(1){ // Do CNG coding of entire file
            ...
            if(firstSilent){
                res = CloopenWebRtcCng_Encode(e_inst, anaSpeech, frameLen/2, SIDpkt,&size,1);
                if (res < 0) {
                    /* exit if returned with error */
                    errtype=WebRtcCng_GetErrorCodeEnc(e_inst);
                    fprintf(stderr,"\n\n Error in encoder: %d.\n\n", errtype);
                ...
                res=CloopenWebRtcCng_Encode(e_inst, &anaSpeech[frameLen/2], frameLen/2, SIDpkt,&size,1);
                if (res < 0) {
                    /* exit if returned with error */
                    errtype=WebRtcCng_GetErrorCodeEnc(e_inst);
                    fprintf(stderr,"\n\n Error in encoder: %d.\n\n", errtype);
                ...
                res=CloopenWebRtcCng_Encode(e_inst, anaSpeech, frameLen/2, SIDpkt,&size,0);
                if (res < 0) {
                    /* exit if returned with error */
                    errtype=WebRtcCng_GetErrorCodeEnc(e_inst);
                    fprintf(stderr,"\n\n Error in encoder: %d.\n\n", errtype);
                ...
                res=CloopenWebRtcCng_Encode(e_inst, &anaSpeech[frameLen/2], frameLen/2, SIDpkt,&size,0);
                if (res < 0) {
                    /* exit if returned with error */
                    errtype=WebRtcCng_GetErrorCodeEnc(e_inst);
                    fprintf(stderr,"\n\n Error in encoder: %d.\n\n", errtype);
            ...
            if(size>0){
                res=CloopenWebRtcCng_UpdateSid(d_inst,SIDpkt, size);
                if (res < 0) {
                    /* exit if returned with error */
                    errtype=CloopenWebRtcCng_GetErrorCodeDec(d_inst);
                    fprintf(stderr,"\n\n Error in decoder: %d.\n\n", errtype);
            ...
            res=CloopenWebRtcCng_Generate(d_inst,genSpeech, frameLen,0);
                if (res < 0) {
                    /* exit if returned with error */
                    errtype=CloopenWebRtcCng_GetErrorCodeDec(d_inst);
                    fprintf(stderr,"\n\n Error in decoder: %d.\n\n", errtype);
