...
ACMNetEQ::Init()
{
    ...
    for(WebRtc_Word16 idx = 0; idx < _numSlaves + 1; idx++)
    {
        ...
        {
            CloopenWebRtcVad_Free(_ptrVADInst[idx]);
    ...
    const WebRtc_Word16 idx)
{
    ...
    if (_instMem[idx] == NULL)
    {
        WEBRTC_TRACE(yuntongxunwebrtc::kTraceError, yuntongxunwebrtc::kTraceAudioCoding, _id,
    ...
    const WebRtc_Word16 idx)
{
    if(_ptrVADInst[idx] == NULL)
    {
        if(CloopenWebRtcVad_Create(&_ptrVADInst[idx]) < 0)
        {
            _ptrVADInst[idx] = NULL;
            WEBRTC_TRACE(yuntongxunwebrtc::kTraceError, yuntongxunwebrtc::kTraceAudioCoding, _id,
    ...
    if(WebRtcNetEQ_SetVADInstance(_inst[idx], _ptrVADInst[idx],
        (CloopenWebRtcNetEQ_VADInitFunction)    CloopenWebRtcVad_Init,
        (WebRtcNetEQ_VADSetmodeFunction) CloopenWebRtcVad_set_mode,
        (WebRtcNetEQ_VADFunction)        CloopenWebRtcVad_Process) < 0)
    {
    ...
    AudioFrame& audioFrame)
{
    ...
    else
    {
        ...
        {
            WEBRTC_TRACE(yuntongxunwebrtc::kTraceWarning, yuntongxunwebrtc::kTraceAudioCoding, _id,
...
void ACMNetEQ::RemoveNetEQSafe(int index) {
  ...
  if (_ptrVADInst[index] != NULL) {
    CloopenWebRtcVad_Free(_ptrVADInst[index]);
