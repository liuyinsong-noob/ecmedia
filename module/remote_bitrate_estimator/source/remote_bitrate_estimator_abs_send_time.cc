...
void RemoteBitrateEstimatorAbsSendTimeImpl::IncomingPacket(
  ...
  if (!remote_rate_->ValidEstimate() ||
    ...
    if (probes_.empty()) {
      LOG(LS_INFO) << "Probe packet received: send time=" << (unsigned long)send_time_ms
                   << " ms, recv time=" << (unsigned long)arrival_time_ms << " ms";
    } else {
      int send_delta_ms = send_time_ms - probes_.back().send_time_ms;
      int recv_delta_ms = arrival_time_ms - probes_.back().recv_time_ms;
      LOG(LS_INFO) << "Probe packet received: send time=" << (unsigned long)send_time_ms
                   << " ms, recv time=" << (unsigned long)arrival_time_ms
