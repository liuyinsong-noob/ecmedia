...
class WindowCapturerMac : public WindowCapturer {
 public:
    explicit WindowCapturerMac(scoped_refptr<FullScreenChromeWindowDetector>
  ...
  void Start(Callback* callback) override;
    ...
    bool GetWindowList(WindowList* sources) override;
...
bool WindowCapturerMac::GetWindowList(WindowList* sources) {
//    sources = [NSWindow windowNumbersWithOptions:NSWindowNumberListAllApplications];
//    return NSWindowList(sources, true);
    return yuntongxunwebrtc::GetWindowList(sources, true);
}
...
bool WindowCapturerMac::GetShareCaptureRect(int &width, int &height){
    if(!window_id_)
        return NO;
    width = [NSScreen mainScreen].frame.size.width * [NSScreen mainScreen].backingScaleFactor;
    height = [NSScreen mainScreen].frame.size.height * [NSScreen mainScreen].backingScaleFactor;
    return YES;
}
...
void WindowCapturerMac::Capture(const DesktopRegion &region){
    WindowCapturerMac::CaptureFrame();
}

void WindowCapturerMac::CaptureFrame() {
  if (!IsWindowValid(window_id_)) {
     callback_->OnCaptureCompleted(nullptr, kCapture_NoCaptureImage);
      return;
  }
  ...
  frame->mutable_updated_region()->SetRect(
    ...
    callback_->OnCaptureCompleted(frame.release(), kCapture_Ok);
...
//    return std::unique_ptr<DesktopCapturer> capture(new WindowCapturerMac(options.full_screen_chrome_window_detector()));
