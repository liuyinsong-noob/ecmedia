...
bool IsScreenValid(ScreenId screen, std::string& device_key) {
  ...
  if (enum_result)
  {
        std::string key((char*)device.DeviceKey);
        device_key = key;
  }
  return !!enum_result;
}
...
DesktopRect GetScreenRect(ScreenId screen, const std::string& device_key) {
  ...
  std::string DeviceKey((const char*)device.DeviceKey);
  if(device_key.compare(DeviceKey) != 0)
      return DesktopRect();
