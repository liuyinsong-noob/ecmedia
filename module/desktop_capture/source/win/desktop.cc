...
Desktop::~Desktop() {
  if (own_ && desktop_ != NULL) {
	...
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Failed to close the owned desktop handle: %d",GetLastError());
    }
  }
}
...
bool Desktop::GetName(std::wstring* desktop_name_out) const {
  ...
  if (!GetUserObjectInformationW(desktop_, UOI_NAME, &buffer[0],
	...
	WEBRTC_TRACE(kTraceError, kTraceAudioMixerServer, -1,
		"Failed to query the desktop name: %d",GetLastError());
    return false;
  }
...
bool Desktop::SetThreadDesktop() const {
  if (!::SetThreadDesktop(desktop_)) {
	WEBRTC_TRACE(kTraceError, kTraceAudioMixerServer, -1,
		"Failed to assign the desktop to the current thread: %d",GetLastError());
    return false;
  }
...
Desktop* Desktop::GetDesktop(const WCHAR* desktop_name) {
  ...
  if (desktop == NULL) {
	...
	WEBRTC_TRACE(kTraceError, kTraceAudioMixerServer, -1,
		"Failed to open the desktop : %s",desktop_name);
    return NULL;
  }
...
Desktop* Desktop::GetThreadDesktop() {
  ...
  if (desktop == NULL) {
	...
	WEBRTC_TRACE(kTraceError, kTraceAudioMixerServer, -1,
		"Failed to retrieve the handle of the desktop assigned to the current thread: %d",GetLastError());
    return NULL;
  }
