...
void ScreenCapturerWinMagnifier::Capture(const DesktopRegion& region) {
  ...
  if (!SetThreadExecutionState(ES_DISPLAY_REQUIRED | ES_SYSTEM_REQUIRED)) {
    if (!set_thread_execution_state_failed_) {
	  ...
	  WEBRTC_TRACE(kTraceError, kTraceAudioMixerServer, -1,
		  "Failed to make system & display power assertion: %d",GetLastError());
    }
  }
  ...
  if (!succeeded) {
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,"Switching to the fallback screen capturer");
  ...
  callback_->OnCaptureCompleted(frame, kCapture_Ok);
}
...
bool ScreenCapturerWinMagnifier::GetShareCaptureRect(int &width, int &height)
{
    DesktopRect rect = GetScreenRect(current_screen_id_, current_device_key_);
    width = rect.width();
    height = rect.height();
...
bool ScreenCapturerWinMagnifier::CaptureImage(const DesktopRect& rect) {
  ...
  if (!result) {
	  WEBRTC_TRACE(kTraceError, kTraceAudioMixerServer, -1,
		  "Failed to call SetWindowPos: left %d ,top %d right %d bottom %d",
  ...
  if (!result) {
	  WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		  "Failed to call MagSetWindowSource:: left %d ,top %d right %d bottom %d",
		  rect.left(),rect.top(), rect.right(),rect.bottom());
    return false;
  }
...
bool ScreenCapturerWinMagnifier::InitializeMagnifier() {
  ...
  if (!mag_initialize_func_ || !mag_uninitialize_func_ ||
      ...
      !set_image_scaling_callback_func_) {
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Failed to initialize ScreenCapturerWinMagnifier library functions missing");
    return false;
  }
  ...
  if (!result) {
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Failed to initialize ScreenCapturerWinMagnifier: %d",GetLastError());
    return false;
  }
  ...
  if (!magnifier_window_) {
	...
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Failed to initialize ScreenCapturerWinMagnifier,error from creating magnifier window: %d",GetLastError());
    return false;
  }
  ...
  if (!result) {
    mag_uninitialize_func_();
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Failed to initialize ScreenCapturerWinMagnifierr,error from MagSetImageScalingCallback: %d",GetLastError());
    return false;
  }
  ...
  if (excluded_window_) {
    ...
    if (!result) {
      mag_uninitialize_func_();
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Failed to initialize ScreenCapturerWinMagnifier,error from MagSetWindowFilterList: %d",GetLastError());
      return false;
    }
  }
...
void ScreenCapturerWinMagnifier::OnCaptured(void* data,
  ...
  if (header.format != GUID_WICPixelFormat32bppRGBA ||
	...
	WEBRTC_TRACE(kTraceWarning, kTraceAudioMixerServer, -1,
		"Output format does not match the captured width:%d height:%d stride:%d bpp:%d",
		header.width,header.height,header.stride,captured_bytes_per_pixel);
    return;
  }
