...
bool DesktopAndCursorComposer::GetShareCaptureRect(int &width, int &height)
{
    if (desktop_capturer_.get())
    {
        return desktop_capturer_->GetShareCaptureRect(width, height);
    }
    return false;
}
...
void DesktopAndCursorComposer::OnCaptureCompleted(DesktopFrame* frame, CaptureErrCode errCode , DesktopRect *window_rect /*= NULL*/) {
  if (frame && cursor_.get() && cursor_state_ == MouseCursorMonitor::INSIDE) {
    if(window_rect != NULL)
    {
        //Make sure the mouse is in the window.
        int x = cursor_position_.x() - window_rect->left();
        int y = cursor_position_.y() - window_rect->top();
        cursor_position_.set(x,y);
    }
  ...
  callback_->OnCaptureCompleted(frame, errCode);
}
