...
int AudioProcessingImpl::StartDebugRecordingForPlatformFile(
    PlatformFile handle) {
  FILE* stream = FdopenPlatformFileForWriting(handle);
...
int AudioProcessingImpl::set_num_channels(
	...
	int output_channels) {
		...
		return InitializeLocked(fwd_in_format_.rate(),
			fwd_out_format_.rate(),
			rev_in_format_.rate(),
			input_channels,
			fwd_out_format_.num_channels(),
...
int AudioProcessingImpl::ProcessStreamNoiseSuppression(AudioFrame* frame) {
	CriticalSectionScoped crit_scoped(crit_);
	int err = kNoError;
	...
	if (frame->num_channels_ != fwd_in_format_.num_channels())
	{
		return kBadNumberChannelsError;
	}
	if (frame->samples_per_channel_ != fwd_in_format_.samples_per_channel())
	...
	if (fwd_out_format_.num_channels() < fwd_in_format_.num_channels())
	{
		capture_audio_->Mix(fwd_out_format_.num_channels());
		frame->num_channels_ = fwd_out_format_.num_channels();
	}
	...
	err = high_pass_filter_->ProcessCaptureAudio(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}

	err = gain_control_->AnalyzeCaptureAudio(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}

	err = echo_cancellation_->ProcessCaptureAudio(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}
	...
	err = noise_suppression_->ProcessCaptureAudio(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}
	...
	err = voice_detection_->ProcessCaptureAudio(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}

	err = gain_control_->ProcessCaptureAudio(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}
	...
	// The level estimator operates on the recombined data.
	err = level_estimator_->ProcessStream(capture_audio_.get());
	if (err != kNoError) {
		return err;
	}
