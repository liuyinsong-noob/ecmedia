...
int osip_uri_parse_headers(osip_uri_t * url, const char *headers)
{
	...
	start_and = (char *)strchr(headers + 1, '&');
	...
	do {
		...
		if (start_and != NULL) {
			if (start_and - equal < 2) {
				osip_free(hname);
				return OSIP_SYNTAXERROR;
			}
			hvalue = (char *) osip_malloc(start_and - equal);
			if (hvalue == NULL) {
				osip_free(hname);
				return OSIP_NOMEM;
			}
			osip_strncpy(hvalue, equal + 1, start_and - equal - 1);
		...
		if (start_and == NULL)		/* we just set the last header */
			equal = NULL;
...
int
osip_uri_param_get_byname(osip_list_t * params, const char *pname,
						  osip_uri_param_t ** url_param)
{
	int pos = 0;
	size_t pname_len;
