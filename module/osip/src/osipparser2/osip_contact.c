...
int osip_message_replace_contact(osip_message_t * sip, const char *hvalue)
{
	int i;
	osip_contact_t *contact,*temp;

	if (hvalue == NULL || hvalue[0] == '\0' || sip == NULL)
		return OSIP_SUCCESS;
	...
	sip->message_property = 2;
	while(sip->contacts.nb_elt > 0) {	
		temp =(osip_contact_t*) osip_list_get(&sip->contacts,0);
		osip_list_remove(&sip->contacts,0);
		osip_contact_free(temp);
	}
	osip_list_add(&sip->contacts, contact, -1);
	return OSIP_SUCCESS;		/* ok */
}
