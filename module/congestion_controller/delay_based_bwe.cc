...
bool ReadTrendlineFilterExperimentParameters(size_t* window_size,
  ...
  DCHECK(TrendlineFilterExperimentIsEnabled());
  DCHECK(!MedianSlopeFilterExperimentIsEnabled());
  ...
  if (parsed_values == 3) {
    CHECK_GT(*window_size, 1) << "Need at least 2 points to fit a line.";
    CHECK(0 <= *smoothing_coef && *smoothing_coef <= 1)
        << "Coefficient needs to be between 0 and 1 for weighted average.";
    CHECK_GT(*threshold_gain, 0) << "Threshold gain needs to be positive.";
    return true;
  }
...
DelayBasedBwe::Result DelayBasedBwe::OnLongFeedbackDelay(
  ...
  DCHECK(rate_control_.ValidEstimate());
