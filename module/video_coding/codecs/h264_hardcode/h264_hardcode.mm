...
namespace yuntongxunwebrtc {
    ...
    int H264VideoToolboxEncoder::SetRates(uint32_t new_bitrate_kbit, uint32_t new_framerate) {
        ...
        if(compression_session_) {
            ...
            //VTCompressionSessionCompleteFrames((VTCompressionSessionRef)compression_session_, kCMTimeInvalid);
            ...
            CFRelease(ref);
            ret = VTSessionCopyProperty((VTCompressionSessionRef)compression_session_, kVTCompressionPropertyKey_AverageBitRate, kCFAllocatorDefault, &ref);
    ...
    void H264VideoToolboxEncoder::ConfigureCompressionSession(int width, int height)
    {
        dispatch_sync(aQueue, ^{
            ...
            SInt32 bitRate = (codec_.width*codec_.height*15)*2;
            ...
            status = VTSessionSetProperty(compression_session_, kVTCompressionPropertyKey_DataRateLimits, (__bridge CFArrayRef)@[@(bitRate / 8), @1]);
            NSLog(@"sean haha status %d", status);
