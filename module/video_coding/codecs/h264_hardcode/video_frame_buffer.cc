...
const uint8_t* I420Buffer::DataU() const {
  return data_ + stride_y_ * height_;
}
...
scoped_refptr<VideoFrameBuffer> I420Buffer::NativeToI420Buffer() {
  NOTREACHED();
  return nullptr;
}

scoped_refptr<I420Buffer> I420Buffer::Copy(
...
WrappedI420Buffer::WrappedI420Buffer(int width,
                                     ...
                                     int v_stride,
                                     const Callback0<void>& no_longer_used)
    : width_(width),
      height_(height),
      y_plane_(y_plane),
      u_plane_(u_plane),
...
scoped_refptr<VideoFrameBuffer> WrappedI420Buffer::NativeToI420Buffer() {
  NOTREACHED();
  return nullptr;
}

scoped_refptr<VideoFrameBuffer> ShallowCenterCrop(
    const scoped_refptr<VideoFrameBuffer>& buffer,
    int cropped_width,
    int cropped_height) {
