...
int VP8EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  configurations_[0].rc_undershoot_pct = 100; //undershoot percent
  configurations_[0].rc_overshoot_pct = 15; //overshoot percent
  configurations_[0].rc_buf_initial_sz = 500;
  configurations_[0].rc_buf_optimal_sz = 600;
  configurations_[0].rc_buf_sz = 1000; //decoding buffer size in ms.
...
int VP8EncoderImpl::InitAndSetControlSettings() {
  ...
  if (encoders_.size() > 1) {
    ...
    if (error) {
      ...
      vpx_codec_err_t rett = vpx_codec_enc_init(&encoders_[0],
                                                vpx_codec_vp8_cx(),
                                                &configurations_[0],
                                                flags);
    if (rett) {
  ...
  for (size_t i = 0; i < encoders_.size(); ++i) {
    vpx_codec_control(&(encoders_[i]), VP8E_SET_STATIC_THRESHOLD, 1000); //add by ylr
