...
int OpenH264DecoderImpl::PrepareRawImage(uint8_t** data, SBufferInfo *info)
{
	...
	uint8_t* dstBuffer = decoded_image_._buffer;
	uint8_t* srcBuffer;
	int offset = 0;
	int i=0;
	...
	decoded_image_._width = info->UsrData.sSystemBuffer.iWidth;
	decoded_image_._height = info->UsrData.sSystemBuffer.iHeight;
