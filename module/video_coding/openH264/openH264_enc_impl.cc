...
int OpenH264EncoderImpl::Encode(const RawImage& input_image,
...
int OpenH264EncoderImpl::SetChannelParameters(uint32_t /*packet_loss*/, int rtt) {
  // ffs
  return WEBRTC_VIDEO_CODEC_OK;
}

int OpenH264EncoderImpl::UpdateCodecFrameSize(const RawImage& input_image) {
...
void OpenH264EncoderImpl::SetSVCEncoderParameters(const VideoCodec *inst, SEncParamExt &sSvcParam)
{
	...
	sSvcParam.iMultipleThreadIdc		= 2;
...
void OpenH264EncoderImpl::InitializeSSPic(const RawImage& input_image,SSourcePicture &pic)
{
	memset(&pic,0,sizeof(SSourcePicture));
	pic.iPicWidth = input_image._width;
	pic.iPicHeight = input_image._height;
	pic.iColorFormat = videoFormatI420;
	pic.iStride[0] = pic.iPicWidth;
	pic.iStride[1] = pic.iStride[2] = pic.iPicWidth >> 1;

	pic.pData[0] = (uint8_t*)(input_image._buffer);
	pic.pData[1] = (uint8_t*)(pic.pData[0] + input_image._width*input_image._height);
	pic.pData[2] = (uint8_t*)(pic.pData[1] + (input_image._width*input_image._height>> 2));
	pic.uiTimeStamp = WELS_ROUND (frameIndx_ * (1000 / sSvcParam.fMaxFrameRate));
}

bool OpenH264EncoderImpl::CopyEncodedImage(RTPFragmentationHeader &fragment, SFrameBSInfo &frameBsInfo, const RawImage &input_image)
{
	int layer = 0;
