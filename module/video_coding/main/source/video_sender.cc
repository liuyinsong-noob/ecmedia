...
int32_t VideoSender::AddVideoFrame(const I420VideoFrame& videoFrame,
  ...
  CriticalSectionScoped cs(_sendCritSect);
  if (_encoder == NULL) {
    LOG(LS_ERROR) << " AddVideoFrame failed. encoder is NULL.";
    return VCM_UNINITIALIZED;
  }
...
int32_t VideoSender::RegisterEncoderRateObserver(VideoEncoderRateObserver *observer)
{
	if (_encoder != NULL)
	{
		return _encoder->RegisterEncoderRateObserver(observer);
	}
	return -1;
}
}  // namespace vcm
