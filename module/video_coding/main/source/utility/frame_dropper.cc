...
{
	/*int isDeltaframe = deltaFrame ? 1 : 0;
	WEBRTC_TRACE(kTraceError, kTraceVideoCoding, 0, "[Test-1]Update _accumulator, frameSizeBytes=%d, deltaFrame=%d",frameSizeBytes, isDeltaframe);*/
...
//	WEBRTC_TRACE(kTraceError, kTraceVideoCoding, 0, "[Test-1]Update _accumulator, _accumulator=%f, frameSizeKbits=%f",_accumulator, frameSizeKbits);
    CapAccumulator();
}
...
//	WEBRTC_TRACE(kTraceError, kTraceVideoCoding, 0, "[Test-1]Leak, inputFrameRate=%d, _targetBitRate=%f, _keyFrameRatio=%f, _keyFrameSizeAvgKbits=%f", inputFrameRate, _targetBitRate,_keyFrameRatio.filtered(), _keyFrameSizeAvgKbits.filtered());
...
//	WEBRTC_TRACE(kTraceError, kTraceVideoCoding, 0, "[Test-1]Update dropRaio, dropRatio=%f, _accumulator=%f, _accumulatorMax=%f", _dropRatio, _accumulator,_accumulatorMax);
}
...
FrameDropper::DropFrame()
{
    ...
    if (_dropRatio.filtered() >= 0.5f) // Drops per keep
    {
        ...
        if (_dropCount < limit)
        {
            ...
            _dropCount++;
//			 WEBRTC_TRACE(kTraceError, kTraceVideoCoding, 0, "[Test-1]drop frame,_dropCount++ = %d, dropRatio=%f, limit=%d", _dropCount, _dropRatio, limit);
            return true;
        }
        else
        {
