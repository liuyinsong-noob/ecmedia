...
class MediaOptimization {
 ...
 private:
  ...
  uint16_t codec_width_ GUARDED_BY(crit_sect_); //编码器的图像宽
  uint16_t codec_height_ GUARDED_BY(crit_sect_);  //编码器的图像高
  ...
  uint32_t avg_sent_bit_rate_bps_ GUARDED_BY(crit_sect_); //返回最近1秒内发送的帧的平均bitrate
  uint32_t avg_sent_framerate_ GUARDED_BY(crit_sect_);  //返回最近1秒内发送的帧数
