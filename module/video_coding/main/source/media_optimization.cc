...
void UpdateProtectionCallback(
  ...
  // TODO(Marco): Pass FEC protection values per layer. //video_protection_callback is vie_encoder
  video_protection_callback->ProtectionRequest(&delta_fec_params,
                                               &key_fec_params,
                                               video_rate_bps, //返回参数：rtp_sender_.VideoBitrateSent();
...
uint32_t MediaOptimization::SetTargetRates(
    uint32_t target_bitrate, //bitrate control计算的码率
    uint8_t fraction_lost, //last_fraction_loss_
    int64_t round_trip_time_ms, //last_round_trip_time_ms_
  ...
  if (enable_qm_ && qmsettings_callback) {
    ...
    bool select_qm = CheckStatusForQMchange(); //qm update interval default=10s
    if (select_qm) {
  ...
  if (encoded_length > 0) {
    ...
    if (max_payload_size_ > 0 && encoded_length > 0) {
      ...
      if (enable_qm_) {
        ...
        qm_resolution_->UpdateEncodedSize(encoded_length); //计算编码low_buffer_cnt_
      }
    }
...
bool MediaOptimization::QMUpdate(
  ...
  if (qm->change_resolution_spatial) {
      codec_width_ = (qm->codec_width % 4 == 0)?qm->codec_width:(qm->codec_width/4+1)*4;
...
//  qm_resolution_->UpdateCodecParameters(qm->frame_rate, codec_width_, codec_height_);
  qm_resolution_->UpdateCodecParameters(qm->frame_rate, qm->codec_width, qm->codec_height);
  return true;
}
