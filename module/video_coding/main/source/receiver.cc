...
namespace cloopenwebrtc {
    ...
    VCMEncodedFrame* VCMReceiver::FrameForDecoding(uint16_t max_wait_time_ms,
                                                   ...
                                                   bool render_timing) {
        complete_flag = false;
        decodable_flag = false; //decodable, may be incomplete. In the case, we should request key frame
        const int64_t start_time_ms = clock_->TimeInMilliseconds();
        uint32_t frame_timestamp = 0;
        // Exhaust wait time to get a complete frame for decoding.
        bool found_frame = jitter_buffer_.NextCompleteTimestamp(
                                                                max_wait_time_ms, &frame_timestamp);
        complete_flag = found_frame;
        ...
        if (next_render_time_ms < 0) {
            timing_error = true;
        } else if (std::abs((int)(next_render_time_ms - now_ms)) > max_video_delay_ms_) {
            int frame_delay = static_cast<int>(std::abs((int)(next_render_time_ms - now_ms)));
