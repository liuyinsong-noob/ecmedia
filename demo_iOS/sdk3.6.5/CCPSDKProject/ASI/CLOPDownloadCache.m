...
#import "CLOPDownloadCache.h"
#import "CLOPHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static CLOPDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface CLOPDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation CLOPDownloadCache

+ (void)initialize
{
	if (self == [CLOPDownloadCache class]) {
...
- (id)init
{
	...
	[self setDefaultCachePolicy:CLOPUseDefaultCachePolicy];
	...
	if (!sharedCache) {
		@synchronized(self) {
			if (!sharedCache) {
				...
				[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"CLOPHTTPRequestCache"]];
			}
		}
	}
	return sharedCache;
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:CLOPCacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (void)updateExpiryForRequest:(CLOPHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-CLOPHTTPRequest-Expires"];
...
{
  return [CLOPHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}
...
- (void)storeResponseForRequest:(CLOPHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & CLOPDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
	...
	// Create a special 'X-CLOPHTTPRequest-Expires' header
	...
	// We store this as a timestamp to make reading it eCLOPer as NSDateFormatter is quite expensive

	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-CLOPHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-CLOPHTTPRequest-Response-Status-Code"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(CLOPHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == CLOPCacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)isCachedDataCurrentForRequest:(CLOPHTTPRequest *)request
{
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-CLOPHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-CLOPHTTPRequest-Expires"];
		if (expires) {
...
{
	[[self accessLock] lock];
	CLOPCachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(CLOPCachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = CLOPAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(CLOPCacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == CLOPCacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/CLOPHTTPRequest/ is cached the same as http://allseeing-i.com/CLOPHTTPRequest
...
- (BOOL)canUseCachedDataForRequest:(CLOPHTTPRequest *)request
{
	...
	if ([request cachePolicy] & CLOPDoNotReadFromCacheCachePolicy) {
		return NO;

	// If we don't want to load the request whatever happens, always pretend we have cached data even if we don't
	} else if ([request cachePolicy] & CLOPDontLoadCachePolicy) {
		return YES;
	}
	...
	if ([request cachePolicy] & CLOPOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & CLOPFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & CLOPAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & CLOPAskServerIfModifiedCachePolicy) {
