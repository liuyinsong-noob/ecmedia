...
//  http://allseeing-i.com/CLOPHTTPRequest
//
//  Portions are based on the ImageClient example from Apple:
//  See: http://developer.apple.com/samplecode/ImageClient/listing37.html

#import "CLOPHTTPRequest.h"

#if TARGET_OS_IPHONE
#import "../Reachability/TReachability.h"
#import "CLOPAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "CLOPInputStream.h"
#import "CLOPDataDecompressor.h"
#import "CLOPDataCompressor.h"

// Automatically set on build
NSString *CLOPHTTPRequestVersion = @"v1.8.1-61 2011-09-19";

static NSString *defaultUserAgent = nil;

NSString* const SDK_NetworkRequestErrorDomain = @"CLOPHTTPRequestErrorDomain";

static NSString *CLOPHTTPRequestRunLoopMode = @"CLOPHTTPRequestRunLoopMode";
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((CLOPHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *CLOPRequestCancelledError;
static NSError *CLOPRequestTimedOutError;
static NSError *CLOPAuthenticationError;
static NSError *CLOPUnableToCreateRequestError;
static NSError *CLOPTooMuchRedirectionError;
...
unsigned long const CLOPWWANBandwidthThrottleAmount = 14800;
...
// When using CLOPAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <CLOPCacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [CLOPHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
+ (void)performInvocation:(NSInvocation *)invocation onTarget:(id *)target releCLOPngObject:(id)objectToRelease;
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(CLOPHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(CLOPBCLOPcBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(CLOPBCLOPcBlock)block;
#endif
...
@property (assign) CLOPAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) CLOPHTTPRequest *PACFileRequest;
...
@implementation CLOPHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [CLOPHTTPRequest class]) {
		...
		CLOPRequestTimedOutError = [[NSError alloc] initWithDomain:SDK_NetworkRequestErrorDomain code:CLOPRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
		CLOPAuthenticationError = [[NSError alloc] initWithDomain:SDK_NetworkRequestErrorDomain code:CLOPAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		CLOPRequestCancelledError = [[NSError alloc] initWithDomain:SDK_NetworkRequestErrorDomain code:CLOPRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		CLOPUnableToCreateRequestError = [[NSError alloc] initWithDomain:SDK_NetworkRequestErrorDomain code:CLOPUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		CLOPTooMuchRedirectionError = [[NSError alloc] initWithDomain:SDK_NetworkRequestErrorDomain code:CLOPTooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[CLOPHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CLOPCacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:CLOPUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CLOPCacheDelegate>)cache andCachePolicy:(CLOPCachePolicy)policy
{
	CLOPHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:CLOPNoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![CLOPDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [CLOPDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	CLOP_DEBUG_LOG(@"[STATUS] Request cancelled: %@",self);
	#endif
	...
	[self failWithError:CLOPRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [CLOPDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	CLOP_DEBUG_LOG(@"[STATUS] Starting synchronous request %@",self);
#endif
	[self setSynchronous:YES];
	[self setRunLoopMode:CLOPHTTPRequestRunLoopMode];
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	CLOP_DEBUG_LOG(@"[STATUS] Starting asynchronous request %@",self);
#endif
...
- (void)main
{
	@try {
		...
		if ([CLOPHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an CLOPNetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:CLOPUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an CLOPNetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (CLOPS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (CLOPAskServerIfModifiedWhenStaleCachePolicy|CLOPAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)applyAuthorizationHeader
{
	...
	if (![self shouldPresentCredentialsBeforeChallenge]) {
		...
		CLOP_DEBUG_LOG(@"[AUTH] Request %@ will not send credentials to the server until it asks for them",self);
		#endif
		return;
	}
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		// If we have bCLOPc authentication explicitly set and a username and password set on the request, add a bCLOPc auth header
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			[self addBCLOPcAuthenticationHeaderWithUsername:[self username] andPassword:[self password]];

			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BCLOPC. Will send credentials without waiting for an authentication challenge",self);	
			#endif
				...
				if (credentials) {
					...
					if ([credentials objectForKey:@"Authentication"]) {
						...
						if (CFHTTPMessageApplyCredentialDictionary(request, (CFHTTPAuthenticationRef)[credentials objectForKey:@"Authentication"], (CFDictionaryRef)[credentials objectForKey:@"Credentials"], NULL)) {
							...
							#if DEBUG_HTTP_AUTHENTICATION
							CLOP_DEBUG_LOG(@"[AUTH] Request %@ found cached credentials (%@), will reuse without waiting for an authentication challenge",self,[credentials objectForKey:@"AuthenticationScheme"]);
							#endif
						} else {
							[[self class] removeAuthenticationCredentialsFromSessionStore:[credentials objectForKey:@"Credentials"]];
							#if DEBUG_HTTP_AUTHENTICATION
							CLOP_DEBUG_LOG(@"[AUTH] Failed to apply cached credentials to request %@. These will be removed from the session store, and this request will wait for an authentication challenge",self);
							#endif
						}

					// If the Authentication key is not set, these credentials were stored after a username and password set on a previous request passed bCLOPc authentication
					// When this happens, we'll need to create the Authorization header ourselves
						...
						[self addBCLOPcAuthenticationHeaderWithUsername:[usernameAndPassword objectForKey:(NSString *)kCFHTTPAuthenticationUsername] andPassword:[usernameAndPassword objectForKey:(NSString *)kCFHTTPAuthenticationPassword]];
						#if DEBUG_HTTP_AUTHENTICATION
						CLOP_DEBUG_LOG(@"[AUTH] Request %@ found cached BCLOPC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
						#endif
					}
				}
			}
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [CLOPHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[CLOPInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[CLOPInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[CLOPInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[CLOPInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[NSMakeCollectable(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream])) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[CLOPHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				CLOP_DEBUG_LOG(@"[CONNECTION] Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] intValue]);
				#endif
				...
				#if DEBUG_PERSISTENT_CONNECTIONS
                CLOP_DEBUG_LOG(@"%@ - Not re-using connection #%i for request #%i because it is already used by request #%i",self,[[[self connectionInfo] objectForKey:@"id"] intValue],[[self requestID] intValue],[[[self connectionInfo] objectForKey:@"request"] intValue]);
				#endif
                [self setConnectionInfo:nil];
            }
		}
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
		CLOP_DEBUG_LOG(@"[CONNECTION] Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] intValue]);
		#endif
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("CLOPStreamID"), [[self connectionInfo] objectForKey:@"id"]);
	
	} else {
		#if DEBUG_PERSISTENT_CONNECTIONS
		CLOP_DEBUG_LOG(@"[CONNECTION] Request %@ will not use a persistent connection",self);
		#endif
	}
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[CLOPHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[CLOPHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:CLOPRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[CLOPHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
						
				#if DEBUG_REQUEST_STATUS
				if ([self totalBytesSent] == [self postLength]) {
					CLOP_DEBUG_LOG(@"[STATUS] Request %@ finished uploading data",self);
				}
				#endif
			}
		}
...
// Used by CLOPNetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (CLOPHTTPRequest *)HEADRequest
{
	CLOPHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CLOPHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CLOPHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[CLOPHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[CLOPHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[CLOPHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[CLOPHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[CLOPHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[CLOPHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[CLOPHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[CLOPHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[CLOPHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[CLOPHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[CLOPHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[CLOPHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[CLOPHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        SEL callback = @selector(performInvocation:onTarget:releCLOPngObject:);
        NSMethodSignature *cbSignature = [CLOPHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[CLOPHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	CLOP_DEBUG_LOG(@"[STATUS] Request finished: %@",self);
#endif
	if ([self error] || [self mainRequest]) {
		return;
	}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	CLOP_DEBUG_LOG(@"[STATUS] Request %@: %@",self,(theError == CLOPRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != CLOPAuthenticationErrorType && [theError code] != CLOPTooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
		CLOP_DEBUG_LOG(@"[CONNECTION] Request #%@ failed and will invalidate connection #%@",[self requestID],[[self connectionInfo] objectForKey:@"id"]);
		#endif
	...
	// If we have cached data, use it and ignore the error when using CLOPFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & CLOPFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	CLOPHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an CLOPNetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	#if DEBUG_REQUEST_STATUS
	if ([self totalBytesSent] == [self postLength]) {
		CLOP_DEBUG_LOG(@"[STATUS] Request %@ received response headers",self);
	}
	#endif		
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:CLOPHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:CLOPProxyAuthenticationNeeded];
	} else {
		#if DEBUG_HTTP_AUTHENTICATION
		if ([self authenticationScheme]) {
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ has passed %@ authentication",self,[self authenticationScheme]);
		}
		#endif
	}
	...
	if (![self authenticationNeeded]) {
		...
		// Did we get here without an authentication challenge? (which can happen when shouldPresentCredentialsBeforeChallenge is YES and bCLOPc auth was successful)
		if (!requestAuthentication && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic] && [self username] && [self password] && [self useSessionPersistence]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ passed BCLOPC authentication, and will save credentials in the session store for future use",self);
			#endif
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[CLOPHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		CLOPHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
			...
			if (![connectionHeader isEqualToString:@"close"]) {
				...
				if (keepAliveHeader) { 
					...
					if (max > 5) {
						...
						#if DEBUG_PERSISTENT_CONNECTIONS
							CLOP_DEBUG_LOG(@"[CONNECTION] Got a keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
						#endif					
					}
				...
				// Otherwise, we'll assume we can keep this connection open
					...
					[self setConnectionCanBeReused:YES];
					#if DEBUG_PERSISTENT_CONNECTIONS
						CLOP_DEBUG_LOG(@"[CONNECTION] Got no keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
					#endif
				}
			}
		}
...
- (BOOL)willRedirect
{
	...
	// Note that CLOPHTTPRequest does not currently support 305 Use Proxy
	...
	#if DEBUG_REQUEST_STATUS
	CLOP_DEBUG_LOG(@"[STATUS] Request will redirect (code: %i): %@",responseCode,self);
	#endif
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CLOPHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CLOPHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	CLOPHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an CLOPNetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [CLOPHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if (user && pass) {
		...
		#if DEBUG_HTTP_AUTHENTICATION
		CLOP_DEBUG_LOG(@"[AUTH] Request %@ will use credentials set on its url",self);
		#endif
		...
		if ([self mainRequest] && [[self mainRequest] username] && [[self mainRequest] password]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ will use credentials from its parent request",self);
			#endif
		...
		// Let's try to use the ones set in this object
			...
			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ will use username and password properties as credentials",self);
			#endif
		}		
	}
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [CLOPHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
			if (user && pass) {
				CLOP_DEBUG_LOG(@"[AUTH] Request %@ will use credentials from the keychain",self);
			}
			#endif
		}
	}
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		CLOP_DEBUG_LOG(@"[AUTH] Request %@ received credentials from its delegate or an CLOPAuthenticationDialog, will retry",self);
	#endif
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		CLOP_DEBUG_LOG(@"[AUTH] Request %@ had authentication cancelled by its delegate or an CLOPAuthenticationDialog",self);
	#endif
...
- (void)failAuthentication
{
	[self failWithError:CLOPAuthenticationError];
}
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[CLOPAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
	#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == CLOPProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (!requestAuthentication) {
		#if DEBUG_HTTP_AUTHENTICATION
		CLOP_DEBUG_LOG(@"[AUTH] Request %@ failed to read authentication information from response headers",self);
		#endif
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		...
		if ([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM || [self authenticationRetryCount] == 0) {
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ received 401 challenge and must authenticate using %@%@",self,[self authenticationScheme],realm);
		} else {
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ NTLM handshake step %i",self,[self authenticationRetryCount]+1);
		}
	#endif
	...
	if (!CFHTTPAuthenticationIsValid(requestAuthentication, &err)) {
		...
		if (err.domain == kCFStreamErrorDomainHTTP && (err.error == kCFStreamErrorHTTPAuthenticationBadUserName || err.error == kCFStreamErrorHTTPAuthenticationBadPassword)) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ had bad credentials, will remove them from the session store if they are cached",self);
			#endif
			...
			if ([self error] || [self isCancelled]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				CLOP_DEBUG_LOG(@"[AUTH] Request %@ failed or was cancelled while waiting to access credentials",self);
				#endif
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials && [self applyCredentials:[credentials objectForKey:@"Credentials"]]) {
					...
					#if DEBUG_HTTP_AUTHENTICATION
					CLOP_DEBUG_LOG(@"[AUTH] Request %@ will reuse cached credentials from the session (%@)",self,[credentials objectForKey:@"AuthenticationScheme"]);
					#endif
			...
			if ([self willAskDelegateForCredentials]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				CLOP_DEBUG_LOG(@"[AUTH] Request %@ will ask its delegate for credentials to use",self);
				#endif
			...
			if ([self showAuthenticationDialog]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				CLOP_DEBUG_LOG(@"[AUTH] Request %@ will ask CLOPAuthenticationDialog for credentials",self);
				#endif
		...
		#if DEBUG_HTTP_AUTHENTICATION
		CLOP_DEBUG_LOG(@"[AUTH] Request %@ has no credentials to present and must give up",self);
		#endif
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ has failed NTLM authentication",self);
			#endif
			...
			#if DEBUG_HTTP_AUTHENTICATION
			CLOP_DEBUG_LOG(@"[AUTH] Request %@ had credentials and they were not marked as bad, but we got a 401 all the same.",self);
			#endif

			[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
		...
		if (newCredentials) {
			...
			if ([self applyCredentials:newCredentials]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				CLOP_DEBUG_LOG(@"[AUTH] Request %@ failed to apply credentials",self);
				#endif
				[delegateAuthenticationLock unlock];
				[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
			}
...
- (void)addBCLOPcAuthenticationHeaderWithUsername:(NSString *)theUsername andPassword:(NSString *)thePassword
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"BCLOPc %@",[CLOPHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[CLOPDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[CLOPHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
...
#if DEBUG_REQUEST_STATUS
	CLOP_DEBUG_LOG(@"[STATUS] Request %@ finished downloading data (%qu bytes)",self, [self totalBytesRead]);
#endif
	[self setStatusTimer:nil];
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[CLOPDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
			...
			// Response should already have been inflated, move the temporary file to the destination path
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
	if ([self requestID]) {
		CLOP_DEBUG_LOG(@"[CONNECTION] Request #%@ finished using connection #%@",[self requestID], [[self connectionInfo] objectForKey:@"id"]);
	}
	#endif
	[[self connectionInfo] removeObjectForKey:@"request"];
	[[self connectionInfo] setObject:[NSDate dateWithTimeIntervalSinceNow:[self persistentConnectionTimeoutSeconds]] forKey:@"expires"];
	...
	// If request has asked delegate or CLOPAuthenticationDialog for credentials
...
- (void)markAsFinished
{
    ...
    BOOL wCLOPnProgress = inProgress;
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-CLOPHTTPRequest-Response-Status-Code"] intValue]];
...
- (BOOL)retryUsingNewConnection
{
	if ([self retryCount] == 0) {
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
			CLOP_DEBUG_LOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
		#endif
		[connectionsLock lock];
		[[self connectionInfo] removeObjectForKey:@"request"];
		[persistentConnectionsPool removeObject:[self connectionInfo]];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
		CLOP_DEBUG_LOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
	#endif	
	return NO;
}
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (CLOPHTTPRequest does not support file:// urls)
	...
	// Create an CLOPHTTPRequest to fetch the PAC file
	CLOPHTTPRequest *PACRequest = [CLOPHTTPRequest requestWithURL:[self PACurl]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
			CLOP_DEBUG_LOG(@"[CONNECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	CLOPHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[CLOPHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[CLOPHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[CLOPHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[CLOPHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[CLOPHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:CLOPCacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and CLOPHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
				...
				// We won't bother to check for systems prior to 10.4, since CLOPHTTPRequest only works on 10.5+
				OSErr err;
...
- (void)performThrottling
{
	...
	[CLOPHTTPRequest measureBandwidthUsage];
	if ([CLOPHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIsScheduled]) {
					[self unscheduleReadStream];
					#if DEBUG_THROTTLING
					CLOP_DEBUG_LOG(@"[THROTTLING] Sleeping request %@ until after %@",self,throttleWakeUpTime);
					#endif
				}
			} else {
				if (![self readStreamIsScheduled]) {
					[self scheduleReadStream];
					#if DEBUG_THROTTLING
					CLOP_DEBUG_LOG(@"[THROTTLING] Waking up request %@",self);
					#endif
				}
			}
		} 
...
+ (void)recordBandwidthUsage
{
	...
	#if DEBUG_THROTTLING
	CLOP_DEBUG_LOG(@"[THROTTLING] ===Used: %u bytes of bandwidth in last measurement period===",bandwidthUsedInLastSecond);
	#endif
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[CLOPHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an CLOPNetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[CLOPHTTPRequest throttleBandwidthForWWANUsingLimit:CLOPWWANBandwidthThrottleAmount];
	} else {
		[CLOPHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[CLOPHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[CLOPHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[CLOPHTTPRequest reachabilityChanged:nil];
}
...
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [CLOPHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <CLOPCacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
...
+ (id <CLOPCacheDelegate>)defaultCache
{
    @synchronized(self) {
...
+ (NSThread *)threadForRequest:(CLOPHTTPRequest *)request
{
...
+ (NSDate *)expiryDateForRequest:(CLOPHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [CLOPHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(CLOPBCLOPcBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(CLOPHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(CLOPBCLOPcBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(CLOPBCLOPcBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(CLOPProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(CLOPProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(CLOPSizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(CLOPSizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(CLOPDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(CLOPBCLOPcBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(CLOPBCLOPcBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(CLOPBCLOPcBlock)aRedirectBlock
{
