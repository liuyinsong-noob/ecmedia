...
#import "CLOPFormDataRequest.h"


// Private stuff
@interface CLOPFormDataRequest ()
...
@implementation CLOPFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:CLOPURLEncodedPostFormat];
...
- (void)addFile:(NSString *)filePath withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if (!fileExists || isDirectory) {
		[self failWithError:[NSError errorWithDomain:SDK_NetworkRequestErrorDomain code:CLOPInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",filePath],NSLocalizedDescriptionKey,nil]]];
	}
	...
	if (!contentType) {
		contentType = [CLOPHTTPRequest mimeTypeForFileAtPath:filePath];
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == CLOPURLEncodedPostFormat) {
...
#if DEBUG_FORM_DATA_REQUEST
	CLOP_DEBUG_LOG(@"%@",[self debugBodyString]);
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:CLOPMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	CLOPFormDataRequest *newRequest = [super copyWithZone:zone];
