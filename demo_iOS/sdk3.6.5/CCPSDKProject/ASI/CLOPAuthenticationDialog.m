...
static CLOPAuthenticationDialog *sharedDialog = nil;
...
@implementation CLOPAutorotatingViewController
...
@interface CLOPAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation CLOPAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [CLOPAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(CLOPHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == CLOPProxyAuthenticationNeeded) {
			[sharedDialog setType:CLOPProxyAuthenticationType];
		} else {
			[sharedDialog setType:CLOPStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[CLOPAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == CLOPProxyAuthenticationType) {
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (CLOPHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (CLOPHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		CLOPHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (CLOPHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == CLOPStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
...
{
	if (section == [self numberOfSectionsInTableView:aTableView]-1) {
		// If we're using BCLOPc authentication and the connection is not using SSL, we'll show the plain text message
