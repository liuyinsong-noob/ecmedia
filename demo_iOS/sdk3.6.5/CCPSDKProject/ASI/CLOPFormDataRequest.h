...
#import "CLOPHTTPRequestConfig.h"

typedef enum _CLOPPostFormat {
    CLOPMultipartFormDataPostFormat = 0,
    CLOPURLEncodedPostFormat = 1
	
} CLOPPostFormat;

@interface CLOPFormDataRequest : CLOPHTTPRequest <NSCopying> {
	...
	CLOPPostFormat postFormat;
	
	NSStringEncoding stringEncoding;
	
#if DEBUG_FORM_DATA_REQUEST
	// Will store a string version of the request body that will be printed to the console when CLOPHTTPREQUEST_DEBUG is set in GCC_PREPROCESSOR_DEFINITIONS
...
@property (assign) CLOPPostFormat postFormat;
