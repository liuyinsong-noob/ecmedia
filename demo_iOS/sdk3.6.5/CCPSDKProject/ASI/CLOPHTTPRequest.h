...
@class CLOPDataDecompressor;

extern NSString *CLOPHTTPRequestVersion;
...
typedef enum _CLOPAuthenticationState {
	CLOPNoAuthenticationNeededYet = 0,
	CLOPHTTPAuthenticationNeeded = 1,
	CLOPProxyAuthenticationNeeded = 2
} CLOPAuthenticationState;

typedef enum _CLOPNetworkErrorType {
    CLOPConnectionFailureErrorType = 1,
    CLOPRequestTimedOutErrorType = 2,
    CLOPAuthenticationErrorType = 3,
    CLOPRequestCancelledErrorType = 4,
    CLOPUnableToCreateRequestErrorType = 5,
    CLOPInternalErrorWhileBuildingRequestType  = 6,
    CLOPInternalErrorWhileApplyingCredentialsType  = 7,
	CLOPFileManagementError = 8,
	CLOPTooMuchRedirectionErrorType = 9,
	CLOPUnhandledExceptionError = 10,
	CLOPCompressionError = 11
	
} CLOPNetworkErrorType;

typedef enum {
    ERequestType_Init = 0,
    ERequestType_GetSipAddress,
    ERequestType_GetNetGroupId,
    ERequestType_GetCallback,
    ERequestType_UploadFileServer,
    ERequestType_DownloadIMChunkedFile,
    
    //实时对讲
    ERequestType_StartInterphone,
    ERequestType_ControlMic,
    ERequestType_ReleaseMic,
    ERequestType_GetInterphoneMember,
    
    //语音群聊
    ERequestType_StartChatroom,
    ERequestType_GetChatroomsList,
    ERequestType_InviteJoinChatroom,
    ERequestType_DismissChatroom,
    ERequestType_RemoveMemberFromChatroom,
    ERequestType_GetChatRoomMember,
    
    //视频会议
    ERequestType_StartVideoConference,
    ERequestType_GetVideoConferencesList,
    ERequestType_DismissVideoConference,
    ERequestType_RemoveMemberFromVideoConference,
    ERequestType_GetVideoConferenceMember,
    ERequestType_SwitchVideoRealScreen,
    ERequestType_SendPortraitToVideoConference,
    ERequestType_UploadPortraitMsg,
    ERequestType_GetPortraitFromVideoConference,
    ERequestType_DownloadPortraitOfVideoConference,
    
    //多路视频
    ERequestType_StartMultiVideo,
    ERequestType_PublishVideo,
    ERequestType_UnpublishVideo,
    
    //other
    ERequestType_SendMediaMsg,
    ERequestType_ConfirmDownloadMediaMessage,
    ERequestType_UploadMediaMsg,
    ERequestType_DownloadFile_Media,
    ERequestType_EditTestNum,
    ERequestType_CheckVoipOfflineMsg,
    ERequestType_GetVoipOfflineMsg,
    ERequestType_ConfirmOfflineMsg    
} ERequestType;

// The error domain that all errors generated by CLOPHTTPRequest use
extern NSString* const SDK_NetworkRequestErrorDomain;
...
extern unsigned long const CLOPWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^CLOPBCLOPcBlock)(void);
typedef void (^CLOPHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^CLOPSizeBlock)(long long size);
typedef void (^CLOPProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^CLOPDataBlock)(NSData *data);
#endif

@interface CLOPHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the CLOPHTTPRequestDelegate protocol
	id <CLOPHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but CLOPNetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <CLOPHTTPRequestDelegate, CLOPProgressDelegate> queue;
	...
	// Automatically set to true in CLOPFormDataRequests when using setFile:forKey:
	...
	// Set to true when CLOPHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = CLOPConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <CLOPProgressDelegate> uploadProgressDelegate;
	...
	id <CLOPProgressDelegate> downloadProgressDelegate;
	...
	// Authentication scheme (BCLOPc, Digest, NTLM)
	// If you are using BCLOPc authentication and want to force CLOPHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, CLOPHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, CLOPHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	// The dialog will not be shown if your delegate responds to proxyAuthenticationNeededForRequest:
	// Default is YES (bCLOPcally, because most people won't want the hassle of adding support for authenticating proxies to their apps)
	...
	// Authentication scheme for the proxy (BCLOPc, Digest, NTLM)
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - CLOPHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	CLOPHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in CLOPNetworkQueue.h
	...
	// Tells CLOPHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let CLOPHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// CLOPHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See CLOPAuthenticationState values above. 0 == default == No authentication needed yet
	CLOPAuthenticationState authenticationNeeded;
	
	// When YES, CLOPHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Default is YES
	// For requests using BCLOPc authentication, set authenticationScheme to (NSString *)kCFHTTPAuthenticationSchemeBasic, and credentials can be sent on the very first request when shouldPresentCredentialsBeforeChallenge is YES
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops CLOPHTTPRequest cleaning up a temporary post body)
	...
	// Will be CLOPHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [CLOPHTTPRequest setDefaultCache:cache] to configure a default cache
	id <CLOPCacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See CLOPCacheDelegate.h for possible values
	CLOPCachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See CLOPCacheDelegate.h for possible values
	CLOPCacheStoragePolicy cacheStoragePolicy;
	...
	CLOPDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using CLOPHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	CLOPHTTPRequest *PACFileRequest;
	...
	CLOPBCLOPcBlock startedBlock;

	//block to execute when headers are received
	CLOPHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	CLOPBCLOPcBlock completionBlock;

	//block to execute when request fails
	CLOPBCLOPcBlock failureBlock;

	//block for when bytes are received
	CLOPProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	CLOPProgressBlock bytesSentBlock;

	//block for when download size is incremented
	CLOPSizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	CLOPSizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	CLOPDataBlock dataReceivedBlock;

	//block for handling authentication
	CLOPBCLOPcBlock authenticationNeededBlock;

	//block for handling proxy authentication
	CLOPBCLOPcBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    CLOPBCLOPcBlock requestRedirectedBlock;
	#endif
}
...
// Construct a bCLOPc authentication header from the username and password supplied, and add it to the request headers
...
// Used internally to record bandwidth use, and by CLOPInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, CLOPHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
// Used for generating Authorization header when using bCLOPc authentication when shouldPresentCredentialsBeforeChallenge is true
// And also by CLOPS3Request
...
@property (assign) CLOPHTTPRequest *mainRequest;
...
@property (assign, readonly) CLOPAuthenticationState authenticationNeeded;
...
@property (assign) id <CLOPCacheDelegate> downloadCache;
@property (assign) CLOPCachePolicy cachePolicy;
@property (assign) CLOPCacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) CLOPDataDecompressor *dataDecompressor;
