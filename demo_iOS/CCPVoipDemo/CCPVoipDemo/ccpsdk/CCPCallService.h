...
/*
 * 查具体离线消息
 * @param sender 消息发送者，如果是群消息则是群id
 * @param strTimestamp yyyy-MM-dd hh:mm:ss格式
 * @param andLimit 获取消息数量，confirmOfflineMsgWithMsgIds接口确认过的消息，下次不会获取
 */
- (void) getVoipOfflineMsgWithSender:(NSString*) sender andTimestamp:(NSString*) strTimestamp andLimit:(int)limit;

//确认消息已经获取，确认后通过getVoipOfflineMsgWithSender获取不到，传入参数是ConfirmOfflineMsg类型
