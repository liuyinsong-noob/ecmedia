...
- (void)viewDidLoad
{
    ...
    UILabel *fecTitle = [[UILabel alloc]initWithFrame:CGRectMake(24.0f+10, 270.0f+20+30, 150.0f, 37.0f)];
    fecTitle.text = @"OPUS FEC Switch";
    fecTitle.font = [UIFont systemFontOfSize:13.0f];
    [self.scrollView  addSubview:fecTitle];
    UISwitch *fecSwitch = [[UISwitch alloc]initWithFrame:CGRectMake(24.0f+170.0, 270.0f+20+30, 100.0f, 37.0f)];
    [fecSwitch addTarget:self action:@selector(switchMethod:) forControlEvents:UIControlEventValueChanged];
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"FecSet"] isEqualToString:@"on"]) {
        [fecSwitch setOn:TRUE];
        [self.modelEngineVoip setOpusFec:TRUE];
    }
    else
    {
        [fecSwitch setOn:FALSE];
    ...
    packetLossTitle.font = [UIFont systemFontOfSize:13.0f];
    [self.scrollView addSubview:packetLossTitle];
    UITextField *packetLossValue = [[UITextField alloc] initWithFrame:CGRectMake(24.0f+170.0, 270.0f+20+30+30, 120.0f, 37.0f)];
    packetLossValue.delegate = self;
    packetLossValue.backgroundColor = [UIColor clearColor];
    packetLossValue.placeholder = @"Actual Loss Rate:";
    packetLossValue.keyboardType = UIKeyboardTypeNumberPad;
    packetLossValue.returnKeyType = UIReturnKeyDone;
    packetLossValue.clearButtonMode = UITextFieldViewModeWhileEditing;
    packetLossValue.tag = 1002;
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"loss"]) {
        ...
        packetLossValue.text = [NSString stringWithFormat:@"%d", loss];
    }
...
- (void)switchMethod:(id)switchbutton
{
    ...
    [self.modelEngineVoip setOpusFec:tmp.isOn];
    if (tmp.isOn) {
        [[NSUserDefaults standardUserDefaults] setObject:@"on" forKey:@"FecSet"];
    }
    else
    {
        [[NSUserDefaults standardUserDefaults] setObject:@"off" forKey:@"FecSet"];
    }
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    else if (textField.tag == 1002)
    {
     //set Fec packet loss
        ...
        [self.modelEngineVoip setLoss: textField.text.intValue];
        NSLog(@"Packet Loss Rate: %@",textField.text);
