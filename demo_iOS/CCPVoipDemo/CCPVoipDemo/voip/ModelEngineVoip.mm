...
//    [self.VoipCallService setPrivateCloudWithCompanyID:@"qixin" andRestAddr:[NSString stringWithFormat:@"%@:%ld",self.serverIP,self.serverPort] andNativeCheck:NO];
...
- (NSInteger)acceptCall:(NSString*)callid
{
    ...
    [self.VoipCallService acceptCall:callid];
//    NSString *dumpFileName = [[NSString stringWithUTF8String:filename_path] stringByAppendingPathComponent:callid];
//    [self startRtpDump:callid andMediaType:0 andFileName:dumpFileName andDirection:0];
    return 0;
}
...
{
    [theAppDelegate printLog:[NSString stringWithFormat:@"接听网络电话 callid=%@",callid]];
    [self.VoipCallService acceptCall:callid withType:callType];
//    NSString *dumpFileName = [[NSString stringWithUTF8String:filename_path] stringByAppendingPathComponent:callid];
//    [self startRtpDump:callid andMediaType:0 andFileName:dumpFileName andDirection:0];
    return 0;
}
...
-(NSInteger)setAudioConfigEnabledWithType:(EAudioType) type andEnabled:(BOOL) enabled andMode:(NSInteger) mode{
    return [self.VoipCallService setAudioConfigEnabledWithType:type andEnabled:enabled andMode:4];
}
...
- (void)onConnected
{
    ...
    flag = NO;
    if ([userDefaults integerForKey:Codec_OPUS8K_KEY] == 0)
    {
        flag = YES;
    }
    [self.VoipCallService setCodecEnabledWithCodec:Codec_OPUS8 andEnabled:flag];
    flag = NO;
    if ([userDefaults integerForKey:Codec_OPUS16K_KEY] == 0)
    {
        flag = YES;
    }
    [self.VoipCallService setCodecEnabledWithCodec:Codec_OPUS16 andEnabled:flag];

    flag = NO;
    if ([userDefaults integerForKey:Codec_OPUS48K_KEY] == 0)
    {
        flag = YES;
    }

    [self.VoipCallService setCodecEnabledWithCodec:Codec_OPUS48 andEnabled:flag];
    ...
    [self.VoipCallService checkVoipOfflineMsgs];
}

-(void)onCheckVoipOfflineMsgsWithReason:(CloopenReason *)reason andReceiveMsgs:(NSArray *)receiveMsgs
{
    if (reason.reason ==0) {
        for (CheckVoipOfflineMsg *offlineCount in receiveMsgs) {
//            NSDateFormatter *dateFormatter = [[[NSDateFormatter alloc] init] autorelease];
//            [dateFormatter setDateFormat:@"yyyy-MM-dd hh:mm:ss"];
//            NSString *currentDateStr = [dateFormatter stringFromDate:[NSDate date]];
            [self.VoipCallService getVoipOfflineMsgWithSender:offlineCount.sender andTimestamp:@"" andLimit:50];
        }
    }
}

-(void)onGetVoipOfflineMsgWithReason:(CloopenReason *)reason andMsg:(OfflineMsg *) offlineMsg
{
    if (reason.reason == 0) {
        NSMutableArray* confirmIMArray = [[NSMutableArray alloc] init];
        for (InstanceMsg *msg in offlineMsg.msgs) {
            [self parseIMMsg:msg];
            ConfirmOfflineMsg* confirmOfflineMsg = [[ConfirmOfflineMsg alloc] init];
             if ([msg isKindOfClass:[IMTextMsg class]])
             {
                 confirmOfflineMsg.msgId = ((IMTextMsg*)msg).sender;
                 confirmOfflineMsg.type = @"0";
             }
             else
             {
                 confirmOfflineMsg.msgId = ((IMTextMsg*)msg).sender;
                 confirmOfflineMsg.type = @"1";
             }
            [confirmIMArray addObject:confirmOfflineMsg];
        }
        [self.VoipCallService confirmOfflineMsgWithMsgIds:confirmIMArray];
        [confirmIMArray release];
    }
}

-(void)onConfirmOfflineMsgWithReason:(CloopenReason *)reason
{
...
- (void)onCallReleased:(NSString *)callid
{
    ...
    [self stopRtpDump:callid andMediaType:0 andDirection:0];
}
...
{
    [self.VoipCallService setOpusFec:enable];
}
