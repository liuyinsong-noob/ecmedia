...
class WEBRTC_DLLEXPORT ViECapture {
 public:
  ...
  virtual int AllocateCaptureDevice(const char* unique_idUTF8,
                                    ...
                                    int& capture_id,
									CaptureCapability* settings=NULL) = 0;
  ...
  // Sets local video preview window.
  virtual int SetLocalVideoWindow(const int capture_id, void* window) = 0;
 ...
 protected:
  ...
  //---begin
  public:
	  virtual int RegisterFrameCallback(int capture_id, ViEFrameCallback* callbackObject)=0;
