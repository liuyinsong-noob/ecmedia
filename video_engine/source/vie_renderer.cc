...
int ViERenderer::ChangeWindow(void *video_window) {
    return render_module_.ChangeWindow(video_window);
}
...
void ViERenderer::DeliverFrame(int id,
	...
	/*sean 20130402*/
	if (_frame_width!=video_frame->width() || _frame_height != video_frame->height()) {
		_frame_width = video_frame->width();
		_frame_height = video_frame->height();
		if(return_video_width_height) {
			return_video_width_height(_frame_width,_frame_height,render_id_);
		}
	}
	/*sean*/
