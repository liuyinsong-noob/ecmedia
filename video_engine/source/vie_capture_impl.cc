...
int ViECaptureImpl::RegisterFrameCallback(int capture_id, ViEFrameCallback* callbackObject)
{
	ViEInputManagerScoped is(*(shared_data_->input_manager()));
	ViECapturer* vie_capture = is.Capture(capture_id);
	if (!vie_capture) {
	...
	if (vie_capture->IsFrameCallbackRegistered(callbackObject)) {
		shared_data_->SetLastError(kViECaptureDeviceObserverNotRegistered);
		return -1;
	}

	if (vie_capture->RegisterFrameCallback(capture_id, callbackObject) != 0) {
		shared_data_->SetLastError(kViECaptureDeviceUnknownError);
		return -1;
	}
	return 0;
...
int ViECaptureImpl::DeregisterFrameCallback(int capture_id, ViEFrameCallback* callbackObject)
{
	ViEInputManagerScoped is(*(shared_data_->input_manager()));
	ViECapturer* vie_capture = is.Capture(capture_id);
	if (!vie_capture) {
	...
	if (vie_capture->DeregisterFrameCallback(callbackObject) != 0) {
		shared_data_->SetLastError(kViECaptureDeviceUnknownError);
		return -1;
	}
	return 0;
...
int ViECaptureImpl::SetCaptureSettings(const int capture_id, CaptureCapability settings)
{
	ViEInputManagerScoped is(*(shared_data_->input_manager()));
	ViECapturer* vie_capture = is.Capture(capture_id);
	if (!vie_capture) {
...
int ViECaptureImpl::SetLocalVideoWindow(const int capture_id,
                                        void* window) {
    WEBRTC_TRACE(kTraceApiCall, kTraceVideo, ViEId(shared_data_->instance_id()),
                 "%s(local video window %p)", __FUNCTION__, window);
    ...
    if (vie_capture->SetLocalVieoWindow(window) != 0) {
        shared_data_->SetLastError(kViECaptureDeviceUnknownError);
        return -1;
    }
    return 0;
...
int ViECaptureImpl::UpdateLossRate(const int capture_id, int lossRate)
{
    ...
    if (vie_capture->UpdateLossRate(lossRate) != 0) {
        shared_data_->SetLastError(kViECaptureDeviceUnknownError);
        return -1;
    }
    return 0;
