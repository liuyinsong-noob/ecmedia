...
int ViERTP_RTCPImpl::SetRTPKeepAliveStatus(
                                      ...
                                      const unsigned int deltaTransmitTimeSeconds)
    {
        ...
        // Get the channel
        ViEChannelManagerScoped cs(*(shared_data_->channel_manager()));
        ViEChannel* vie_channel = cs.Channel(videoChannel);
        if (!vie_channel) {
            WEBRTC_TRACE(kTraceError, kTraceVideo,
                         ViEId(shared_data_->instance_id(), videoChannel),
        ...
        if (vie_channel->SetKeepAliveStatus(enable, unknownPayloadType,
...
int ViERTP_RTCPImpl::GetRTPKeepAliveStatus(const int videoChannel, bool& enabled, char& unknownPayloadType,
                                      unsigned int& deltaTransmitTimeSeconds)
    {
        ...
        int retVal;
        retVal = vie_channel->GetKeepAliveStatus(enabled, (int8_t&)unknownPayloadType,
                                                     deltaTimeMs);
