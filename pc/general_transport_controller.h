...
#ifndef GENERAL_TRANSPORT_CONTROLLER_H_
#define GENERAL_TRANSPORT_CONTROLLER_H_
...
class GeneralTransportController : public sigslot::has_slots<> {
 public:
  ...
  GeneralTransportController(rtc::Thread* signaling_thread,
                          rtc::Thread* network_thread,
                          cricket::PortAllocator* port_allocator,
                          AsyncResolverFactory* async_resolver_factory,
                          Config config);
  virtual ~GeneralTransportController();
 ...
 /* void MaybeStartGathering();
  ...
  GenerateOrGetLastMediaTransportOffer();
	...
	bool RegisterRtpDemuxerSink(const RtpDemuxerCriteria& criteria,
            webrtc::RtpPacketSinkInterface* sink);  // add by dxf
 ...
 private:
  ...
  cricket::GeneralTransportDescription CreateGeneralTransportDescription(
  ...
  const cricket::GeneralTransport* GetGeneralTransportForMid(
      const std::string& mid) const;
  cricket::GeneralTransport* GetGeneralTransportForMid(const std::string& mid);

  // Get the JsepTransport without considering the BUNDLE group. Return nullptr
  // if the JsepTransport is destroyed.
  const cricket::GeneralTransport* GetGeneralTransportByName(
      const std::string& mid) const;
  cricket::GeneralTransport* GetGeneralTransportByName(
      const std::string& transport_name);

  const cricket::GeneralTransportChannel* GetGeneralTransportChannelByMid(
      const std::string& mid) const;
  cricket::GeneralTransportChannel* GetGeneralTransportChannelByMid(
      const std::string& mid);
  bool RemoveGeneralTransportChannelByMid(const std::string& mid);
  void RemoveAllGeneralTransportChannel();
  ...
  RTCError MaybeCreateGeneralTransport(
      bool local,
      const cricket::ContentInfo& content_info,
      const cricket::SessionDescription& description);
  ...
  std::unique_ptr<webrtc::MediaTransportInterface> MaybeCreateMediaTransport(
      ...
      const cricket::SessionDescription& description,
      bool local);
  void MaybeDestroyGeneralTransport(const std::string& mid);
  void DestroyAllGeneralTransports_n();
  ...
  // JsepTransports. GeneralTransportController can iterate all the DtlsTransports
 ...
 void OnTransportRoleConflict_n(cricket::IceTransportInternal* transport);
  ...
  std::map<std::string, std::unique_ptr<cricket::GeneralTransport>>
      general_transports_by_name_;
  // This keeps track of the mapping between media section
  // (BaseChannel/SctpTransport) and the JsepTransport underneath.
  std::map<std::string, cricket::GeneralTransport*> mid_to_transport_;
  ...
  //add by dxf
  //cricket::GeneralTransportChannel* generalTransportChannel_ = nullptr;
  //std::map<std::string, std::unique_ptr<cricket::GeneralTransport>>
  std::map<std::string, cricket::GeneralTransportChannel*>
      general_transport_channels_by_mid_;

  RTC_DISALLOW_COPY_AND_ASSIGN(GeneralTransportController);
};
