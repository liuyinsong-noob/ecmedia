...
GeneralTransportController::~GeneralTransportController() {
  ...
  network_thread_->Invoke<void>(
      RTC_FROM_HERE,
      rtc::Bind(&GeneralTransportController::DestroyAllGeneralTransports_n,
                this));
}

/*RTCError GeneralTransportController::SetLocalDescription(
    SdpType type,
...
RTCError GeneralTransportController::SetRemoteDescription(
    SdpType type,
    const cricket::SessionDescription* description) {
  if (!network_thread_->IsCurrent()) {
    return network_thread_->Invoke<RTCError>(
...
RtpTransportInternal* GeneralTransportController::GetRtpTransport(
    const std::string& mid) const {
  auto jsep_transport = GetGeneralTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
  }
...
MediaTransportInterface* GeneralTransportController::GetMediaTransport(
    const std::string& mid) const {
  auto jsep_transport = GetGeneralTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
...
MediaTransportState GeneralTransportController::GetMediaTransportState(
    const std::string& mid) const {
  auto jsep_transport = GetGeneralTransportForMid(mid);
  if (!jsep_transport) {
...
cricket::DtlsTransportInternal* GeneralTransportController::GetDtlsTransport(
...
GeneralTransportController::GetRtcpDtlsTransport(const std::string& mid) const {
  auto jsep_transport = GetGeneralTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
  }
...
GeneralTransportController::LookupDtlsTransportByMid(const std::string& mid) {
  auto jsep_transport = GetGeneralTransportForMid(mid);
  if (!jsep_transport) {
    return nullptr;
  }
...
void GeneralTransportController::SetIceConfig(
    const cricket::IceConfig& config) {
  if (!network_thread_->IsCurrent()) {
...
/*void GeneralTransportController::SetNeedsIceRestartFlag() {
  for (auto& kv : general_transports_by_name_) {
    kv.second->SetNeedsIceRestartFlag();
  }
}

bool GeneralTransportController::NeedsIceRestart(
    const std::string& transport_name) const {
  const cricket::GeneralTransport* transport =
      GetGeneralTransportByName(transport_name);
  if (!transport) {
    return false;
  }
...
absl::optional<rtc::SSLRole> GeneralTransportController::GetDtlsRole(
    const std::string& mid) const {
  if (!network_thread_->IsCurrent()) {
  ...
  const cricket::GeneralTransport* t = GetGeneralTransportForMid(mid);
  if (!t) {
...
bool GeneralTransportController::SetLocalCertificate(
...
GeneralTransportController::GetLocalCertificate(
    const std::string& transport_name) const {
  if (!network_thread_->IsCurrent()) {
  ...
  const cricket::GeneralTransport* t =
      GetGeneralTransportByName(transport_name);
  if (!t) {
    return nullptr;
  }
...
GeneralTransportController::GetRemoteSSLCertChain(
    const std::string& transport_name) const {
  if (!network_thread_->IsCurrent()) {
  ...
  // fingerprint.
  auto jsep_transport = GetGeneralTransportByName(transport_name);
  if (!jsep_transport) {
    return nullptr;
  }
...
void GeneralTransportController::MaybeStartGathering() {
  if (!network_thread_->IsCurrent()) {
...
RTCError GeneralTransportController::AddRemoteCandidates(
  ...
  auto jsep_transport = GetGeneralTransportByName(transport_name);
  if (!jsep_transport) {
    RTC_LOG(LS_WARNING) << "Not adding candidate because the GeneralTransport "
...
RTCError GeneralTransportController::RemoveRemoteCandidates(
    const cricket::Candidates& candidates) {
  if (!network_thread_->IsCurrent()) {
    return network_thread_->Invoke<RTCError>(
  ...
  for (const auto& kv : candidates_by_transport_name) {
    ...
    cricket::GeneralTransport* jsep_transport =
        GetGeneralTransportByName(transport_name);
    if (!jsep_transport) {
      RTC_LOG(LS_WARNING) << "Not removing candidate because the "
                             "GeneralTransport doesn't exist.";
      continue;
    }
    for (const cricket::Candidate& candidate : candidates) {
...
bool GeneralTransportController::GetStats(const std::string& transport_name,
  ...
  cricket::GeneralTransport* transport =
      GetGeneralTransportByName(transport_name);
  if (!transport) {
    return false;
  }
...
void GeneralTransportController::SetActiveResetSrtpParams(
  ...
  RTC_LOG(INFO) << "Updating the active_reset_srtp_params for "
...
GeneralTransportController::CreateIceTransport(const std::string transport_name,
                                               bool rtcp) {
  ...
  if (config_.external_transport_factory) {
    ...
    std::unique_ptr<cricket::GeneralTransportChannel> generalChannel = nullptr;
    generalChannel = absl::make_unique<cricket::GeneralTransportChannel>(
        transport_name, component, port_allocator_, async_resolver_factory_,
        config_.event_log);
    general_transport_channels_by_mid_[transport_name] = generalChannel.get();
    return generalChannel;
  }
}

std::unique_ptr<cricket::DtlsTransportInternal>
GeneralTransportController::CreateDtlsTransport(
  ...
  dtls->SignalWritableState.connect(
      this, &GeneralTransportController::OnTransportWritableState_n);
  dtls->SignalReceivingState.connect(
      this, &GeneralTransportController::OnTransportReceivingState_n);
 /* dtls->SignalDtlsHandshakeError.connect(
      this, &GeneralTransportController::OnDtlsHandshakeError);
  dtls->ice_transport()->SignalGatheringState.connect(
      this, &GeneralTransportController::OnTransportGatheringState_n);
  dtls->ice_transport()->SignalCandidateGathered.connect(
      this, &GeneralTransportController::OnTransportCandidateGathered_n);
  dtls->ice_transport()->SignalCandidatesRemoved.connect(
      this, &GeneralTransportController::OnTransportCandidatesRemoved_n);
  dtls->ice_transport()->SignalRoleConflict.connect(
      this, &GeneralTransportController::OnTransportRoleConflict_n);
  dtls->ice_transport()->SignalStateChanged.connect(
      this, &GeneralTransportController::OnTransportStateChanged_n);
...
GeneralTransportController::CreateUnencryptedRtpTransport(
    const std::string& transport_name,
...
GeneralTransportController::CreateSdesTransport(
    const std::string& transport_name,
...
GeneralTransportController::CreateDtlsSrtpTransport(
    const std::string& transport_name,
    cricket::DtlsTransportInternal* rtp_dtls_transport,
    cricket::DtlsTransportInternal* rtcp_dtls_transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
...
//  dtls_srtp_transport->SignalDtlsStateChange.connect(
 //     this, &GeneralTransportController::UpdateAggregateStates_n);
  return dtls_srtp_transport;
}

std::vector<cricket::DtlsTransportInternal*>
GeneralTransportController::GetDtlsTransports() {
  std::vector<cricket::DtlsTransportInternal*> dtls_transports;
  for (auto it = general_transports_by_name_.begin();
       it != general_transports_by_name_.end(); ++it) {
...
/*RTCError GeneralTransportController::ApplyDescription_n(
    bool local,
    SdpType type,
    const cricket::SessionDescription* description) {
  RTC_DCHECK(network_thread_->IsCurrent());
  ...
  for (const cricket::ContentInfo& content_info : description->contents()) {
    ...
    error = MaybeCreateGeneralTransport(local, content_info, *description);
    if (!error.ok()) {
      return error;
    }
  }
  ...
  for (size_t i = 0; i < description->contents().size(); ++i) {
    ...
    cricket::GeneralTransport* transport =
        GetGeneralTransportForMid(content_info.name);
    RTC_DCHECK(transport);

    SetIceRole_n(DetermineIceRole(transport, transport_info, type, local));

    cricket::GeneralTransportDescription jsep_description =
        CreateGeneralTransportDescription(content_info, transport_info,
                                          extension_ids,
                                          rtp_abs_sendtime_extn_id);
    if (local) {
      error = transport->SetLocalGeneralTransportDescription(jsep_description,
                                                             type);
    } else {
      error = transport->SetRemoteGeneralTransportDescription(jsep_description,
                                                              type);
    }
...
/*RTCError GeneralTransportController::ValidateAndMaybeUpdateBundleGroup(
    bool local,
    SdpType type,
    const cricket::SessionDescription* description) {
  RTC_DCHECK(description);
...
RTCError GeneralTransportController::ValidateContent(
...
void GeneralTransportController::HandleRejectedContent(
    const cricket::ContentInfo& content_info,
    const cricket::SessionDescription* description) {
  ...
  MaybeDestroyGeneralTransport(content_info.name);
}

bool GeneralTransportController::HandleBundledContent(
    const cricket::ContentInfo& content_info) {
  auto jsep_transport = GetGeneralTransportByName(*bundled_mid());
  RTC_DCHECK(jsep_transport);
...
bool GeneralTransportController::SetTransportForMid(
    const std::string& mid,
    cricket::GeneralTransport* jsep_transport) {
  RTC_DCHECK(jsep_transport);
...
void GeneralTransportController::RemoveTransportForMid(const std::string& mid) {
...
cricket::GeneralTransportDescription
GeneralTransportController::CreateGeneralTransportDescription(
...
std::vector<int> GeneralTransportController::GetEncryptedHeaderExtensionIds(
    const cricket::ContentInfo& content_info) {
  const cricket::MediaContentDescription* content_desc =
      static_cast<const cricket::MediaContentDescription*>(
          content_info.description);
...
std::vector<int>
GeneralTransportController::MergeEncryptedHeaderExtensionIdsForBundle(
    const cricket::SessionDescription* description) {
  RTC_DCHECK(description);
...
int GeneralTransportController::GetRtpAbsSendTimeHeaderExtensionId(
    const cricket::ContentInfo& content_info) {
  if (!config_.enable_external_auth) {
    return -1;
  }
...
const cricket::GeneralTransport*
GeneralTransportController::GetGeneralTransportForMid(
    const std::string& mid) const {
  auto it = mid_to_transport_.find(mid);
  return it == mid_to_transport_.end() ? nullptr : it->second;
}

cricket::GeneralTransport*
GeneralTransportController::GetGeneralTransportForMid(const std::string& mid) {
  auto it = mid_to_transport_.find(mid);
  return it == mid_to_transport_.end() ? nullptr : it->second;
}

const cricket::GeneralTransport*
GeneralTransportController::GetGeneralTransportByName(
    const std::string& transport_name) const {
  auto it = general_transports_by_name_.find(transport_name);
  return (it == general_transports_by_name_.end()) ? nullptr : it->second.get();
}
...
GeneralTransportController::MaybeCreateMediaTransport(
    const cricket::ContentInfo& content_info,
    const cricket::SessionDescription& description,
    bool local) {
...
RTCError GeneralTransportController::MaybeCreateGeneralTransport(
    bool local,
    const cricket::ContentInfo& content_info,
    const cricket::SessionDescription& description) {
  RTC_DCHECK(network_thread_->IsCurrent());
  cricket::GeneralTransport* transport = GetGeneralTransportByName(content_info.name);
  if (transport) {
    return RTCError::OK();
  }
  ...
  if (config_.rtcp_mux_policy != PeerConnectionInterface::kRtcpMuxPolicyRequire && content_info.type == cricket::MediaProtocolType::kRtp) {
  ...
  if (config_.disable_encryption) {
    ...
    unencrypted_rtp_transport = CreateUnencryptedRtpTransport(content_info.name, rtp_dtls_transport.get(), rtcp_dtls_transport.get());
  ...
  std::unique_ptr<cricket::GeneralTransport> jsep_transport = absl::make_unique<cricket::GeneralTransport>(
 ...
 /* jsep_transport->SignalRtcpMuxActive.connect(
  ...
  jsep_transport->SignalMediaTransportStateChanged.connect(
      this, &GeneralTransportController::OnMediaTransportStateChanged_n);
  SetTransportForMid(content_info.name, jsep_transport.get());
  general_transports_by_name_[content_info.name] = std::move(jsep_transport);
  UpdateAggregateStates_n();*/ //lzm-----------
...
void GeneralTransportController::MaybeDestroyGeneralTransport(
    const std::string& mid) {
  auto jsep_transport = GetGeneralTransportByName(mid);
  if (!jsep_transport) {
    return;
  }
...
void GeneralTransportController::DestroyAllGeneralTransports_n() {
  RTC_DCHECK(network_thread_->IsCurrent());

  for (const auto& jsep_transport : general_transports_by_name_) {
...
cricket::IceRole GeneralTransportController::DetermineIceRole(
    cricket::GeneralTransport* jsep_transport,
    const cricket::TransportInfo& transport_info,
    SdpType type,
    bool local) {
  ...
  if (local) {
    ...
    // SetLocalDescription in GeneralTransportController.
    ...
    if (config_.redetermine_role_on_ice_restart &&
        ...
        cricket::generalIceCredentialsChanged(
...
void GeneralTransportController::OnTransportWritableState_n(
...
void GeneralTransportController::OnTransportReceivingState_n(
    rtc::PacketTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
//  UpdateAggregateStates_n();
}

void GeneralTransportController::OnTransportGatheringState_n(
    cricket::IceTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
 // UpdateAggregateStates_n();
}

/*void GeneralTransportController::OnTransportCandidateGathered_n(
    cricket::IceTransportInternal* transport,
...
void GeneralTransportController::OnTransportCandidatesRemoved_n(
    cricket::IceTransportInternal* transport,
    const cricket::Candidates& candidates) {
  invoker_.AsyncInvoke<void>(
...
/*void GeneralTransportController::OnTransportRoleConflict_n(
    cricket::IceTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
...
void GeneralTransportController::OnTransportStateChanged_n(
    cricket::IceTransportInternal* transport) {
  RTC_DCHECK(network_thread_->IsCurrent());
...
void GeneralTransportController::OnMediaTransportStateChanged_n() {
  SignalMediaTransportStateChanged();
  UpdateAggregateStates_n();
}

void GeneralTransportController::UpdateAggregateStates_n() {
  RTC_DCHECK(network_thread_->IsCurrent());
...
void GeneralTransportController::OnDtlsHandshakeError(
    rtc::SSLHandshakeError error) {
  SignalDtlsHandshakeError(error);
}

absl::optional<cricket::SessionDescription::MediaTransportSetting>
GeneralTransportController::GenerateOrGetLastMediaTransportOffer() {
...
bool GeneralTransportController::ReleaseUdpConnection(const std::string& mid) {
...
bool GeneralTransportController::setUdpConnection(
    const std::string& mid,
  ...
  cricket::GeneralTransportChannel* general_channel = GetGeneralTransportChannelByMid(mid);
  if (general_channel) {
    general_channel->setUdpConnection(socketFactory, addrLocal, addrRemote);
  }
  return false;
}

bool GeneralTransportController::RegisterRtpDemuxerSink(
...
const cricket::GeneralTransportChannel* GeneralTransportController::GetGeneralTransportChannelByMid(
    const std::string& mid) const {
  auto it = general_transport_channels_by_mid_.find(mid);
  return it == general_transport_channels_by_mid_.end() ? nullptr : it->second;
}

cricket::GeneralTransportChannel* GeneralTransportController::GetGeneralTransportChannelByMid(
    const std::string& mid) {
  auto it = general_transport_channels_by_mid_.find(mid);
  return it == general_transport_channels_by_mid_.end() ? nullptr : it->second;
}

bool GeneralTransportController::RemoveGeneralTransportChannelByMid(
    const std::string& mid) {
  auto it = general_transport_channels_by_mid_.find(mid);
  if (it != general_transport_channels_by_mid_.end()) {
    general_transport_channels_by_mid_.erase(it);
    return true;
  }
  return false;
...
void GeneralTransportController::RemoveAllGeneralTransportChannel() {
  general_transport_channels_by_mid_.clear();
}
