...
#include "pc/general_transport.h"
...
static bool VerifyIceParams(const GeneralTransportDescription& jsep_description) {
...
GeneralTransportDescription::GeneralTransportDescription() {}
...
GeneralTransportDescription::GeneralTransportDescription(
    const GeneralTransportDescription& from)
...
GeneralTransportDescription::~GeneralTransportDescription() = default;

GeneralTransportDescription& GeneralTransportDescription::operator=(
    const GeneralTransportDescription& from) {
  if (this == &from) {
    return *this;
  }
...
webrtc::RTCError GeneralTransport::SetLocalGeneralTransportDescription(
    const GeneralTransportDescription& jsep_description,
    SdpType type) {
  ...
  bool ice_restarting =
      ...
      generalIceCredentialsChanged(local_description_->transport_desc.ice_ufrag,
  ...
  local_description_.reset(new GeneralTransportDescription(jsep_description));
...
webrtc::RTCError GeneralTransport::SetRemoteGeneralTransportDescription(
    const GeneralTransportDescription& jsep_description,
    webrtc::SdpType type) {
  webrtc::RTCError error;
  ...
  remote_description_.reset(new GeneralTransportDescription(jsep_description));
...
webrtc::RTCError GeneralTransport::AddRemoteCandidates(
...
void GeneralTransport::SetNeedsIceRestartFlag() {
  rtc::CritScope scope(&accessor_lock_);
...
absl::optional<rtc::SSLRole> GeneralTransport::GetDtlsRole() const {
  RTC_DCHECK_RUN_ON(network_thread_);
  rtc::CritScope scope(&accessor_lock_);
  RTC_DCHECK(rtp_dtls_transport_);
  RTC_DCHECK(rtp_dtls_transport_->internal());
...
bool GeneralTransport::GetStats(TransportStats* stats) {
  RTC_DCHECK_RUN_ON(network_thread_);
  rtc::CritScope scope(&accessor_lock_);
...
webrtc::RTCError GeneralTransport::VerifyCertificateFingerprint(
...
void GeneralTransport::SetActiveResetSrtpParams(bool active_reset_srtp_params) {
  RTC_DCHECK_RUN_ON(network_thread_);
  rtc::CritScope scope(&accessor_lock_);
...
void GeneralTransport::SetLocalIceParameters(IceTransportInternal* ice_transport) {
  RTC_DCHECK_RUN_ON(network_thread_);
...
void GeneralTransport::SetRemoteIceParameters(
...
webrtc::RTCError GeneralTransport::SetNegotiatedDtlsParameters(
...
bool GeneralTransport::SetRtcpMux(bool enable,
                               webrtc::SdpType type,
...
void GeneralTransport::ActivateRtcpMux() {
  {
...
bool GeneralTransport::SetSdes(const std::vector<CryptoParams>& cryptos,
...
webrtc::RTCError GeneralTransport::NegotiateAndSetDtlsParameters(
...
webrtc::RTCError GeneralTransport::NegotiateDtlsRole(
    SdpType local_description_type,
...
bool GeneralTransport::GetTransportStats(DtlsTransportInternal* dtls_transport,
...
void GeneralTransport::OnStateChanged(webrtc::MediaTransportState state) {
