...
class GeneralConnectionRequest;
...
class Port : public PortInterface,
 ...
 private:
  ...
  friend class GeneralConnection;
};
...
class GeneralConnection : public CandidatePairInterface,
					public rtc::MessageHandler,
                   public sigslot::has_slots<> {
 public:
  struct SentPing {
  ...
  bool setUdpSocket(rtc::AsyncPacketSocket* udpSocket, const rtc::SocketAddress& addrRemote);
  // The local port where this connection sends and receives packets.
  Port* port() { return port_; }
  const Port* port() const { return port_; }
  ...
  sigslot::signal1<GeneralConnection*> SignalStateChange;

  // Sent when the connection has decided that it is no longer of value.  It
  // will delete itself immediately after this call.
  sigslot::signal1<GeneralConnection*> SignalDestroyed;
  ...
  // This signal will be fired if this connection is nominated by the
  // controlling side.
  sigslot::signal1<GeneralConnection*> SignalNominated;

  // Invoked when GeneralConnection receives STUN error response with 487 code.
  void HandleRoleConflictFromPeer();
  ...
  //modify by dxf
    // Constructs a new connection to the given remote port.
  GeneralConnection(Port* port,  size_t index, const Candidate& candidate);
 ...
 protected:
  ...
  // Callbacks from ConnectionRequest
  virtual void OnConnectionRequestResponse(GeneralConnectionRequest* req,
                                           StunMessage* response);
  void OnConnectionRequestErrorResponse(GeneralConnectionRequest* req,
                                        StunMessage* response);
  void OnConnectionRequestTimeout(GeneralConnectionRequest* req);
  void OnConnectionRequestSent(GeneralConnectionRequest* req);
  ...
  rtc::RateTracker recv_rate_tracker_;
  rtc::RateTracker send_rate_tracker_;
  rtc::AsyncPacketSocket* udpSocket_ = nullptr;//add by dxf
  rtc::SocketAddress addrRemote_;//add by dxf
 private:
  // Update the local candidate based on the mapped address attribute.
  // If the local candidate changed, fires SignalStateChange.
  void MaybeUpdateLocalCandidate(ConnectionRequest* request,
  ...
  friend class Port;
  friend class GeneralConnectionRequest;
};
