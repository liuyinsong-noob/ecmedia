...
#include "p2p/base/general_transport_channel.h"
...
bool generalIceCredentialsChanged(const std::string& old_ufrag,
...
GeneralTransportChannel::GeneralTransportChannel(
    const std::string& transport_name,
    int component,
    PortAllocator* allocator)
    : GeneralTransportChannel(transport_name, component, allocator, nullptr) {}
...
GeneralTransportChannel::~GeneralTransportChannel() {
  ...
  resolvers_.clear();
  if (general_connection_) {
	delete general_connection_;
    general_connection_ = nullptr;
  }
...
// are still active.
void GeneralTransportChannel::AddAllocatorSession(
    std::unique_ptr<PortAllocatorSession> session) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());

  session->set_generation(static_cast<uint32_t>(allocator_sessions_.size()));
  session->SignalPortReady.connect(this, &GeneralTransportChannel::OnPortReady);
  session->SignalPortsPruned.connect(this,
                                     &GeneralTransportChannel::OnPortsPruned);
  session->SignalCandidatesReady.connect(
      this, &GeneralTransportChannel::OnCandidatesReady);
  session->SignalCandidatesRemoved.connect(
      this, &GeneralTransportChannel::OnCandidatesRemoved);
  session->SignalCandidatesAllocationDone.connect(
      this, &GeneralTransportChannel::OnCandidatesAllocationDone);
...
void GeneralTransportChannel::AddConnection(Connection* connection) {
  ...
  connection->SignalReadPacket.connect(this,
                                       &GeneralTransportChannel::OnReadPacket);
  connection->SignalReadyToSend.connect(
      this, &GeneralTransportChannel::OnReadyToSend);
  connection->SignalStateChange.connect(
      this, &GeneralTransportChannel::OnConnectionStateChange);
  connection->SignalDestroyed.connect(
      this, &GeneralTransportChannel::OnConnectionDestroyed);
  connection->SignalNominated.connect(this,
                                      &GeneralTransportChannel::OnNominated);
...
bool GeneralTransportChannel::ShouldSwitchSelectedConnection(
...
bool GeneralTransportChannel::MaybeSwitchSelectedConnection(
    Connection* new_connection,
  ...
  if (missed_receiving_unchanged_threshold &&
    ...
    invoker_.AsyncInvokeDelayed<void>(
        ...
        rtc::Bind(&GeneralTransportChannel::SortConnectionsAndUpdateState, this,
                  reason_to_sort),
        config_.receiving_switching_delay_or_default());
  }
  return false;
...
void GeneralTransportChannel::SetIceRole(IceRole ice_role) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
IceRole GeneralTransportChannel::GetIceRole() const {
  return ice_role_;
}

void GeneralTransportChannel::SetIceTiebreaker(uint64_t tiebreaker) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
IceTransportState GeneralTransportChannel::GetState() const {
  return state_;
}

webrtc::IceTransportState GeneralTransportChannel::GetIceTransportState()
    const {
  return standardized_state_;
}

const std::string& GeneralTransportChannel::transport_name() const {
  return transport_name_;
}

int GeneralTransportChannel::component() const {
  return component_;
}

bool GeneralTransportChannel::writable() const {
  return writable_;
}

bool GeneralTransportChannel::receiving() const {
  return receiving_;
}

IceGatheringState GeneralTransportChannel::gathering_state() const {
  return gathering_state_;
}

absl::optional<int> GeneralTransportChannel::GetRttEstimate() {
  if (selected_connection_ != nullptr &&
...
IceTransportState GeneralTransportChannel::ComputeState() const {
  if (!had_connection_) {
...
webrtc::IceTransportState GeneralTransportChannel::ComputeIceTransportState()
    const {
...
void GeneralTransportChannel::SetIceParameters(
...
void GeneralTransportChannel::SetRemoteIceParameters(
    const IceParameters& ice_params) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
  ...
  for (GeneralRemoteCandidate& candidate : remote_candidates_) {
...
void GeneralTransportChannel::SetRemoteIceMode(IceMode mode) {
...
void GeneralTransportChannel::SetIceConfig(const IceConfig& config) {
...
const IceConfig& GeneralTransportChannel::config() const {
  return config_;
}

// TODO(qingsi): Add tests for the config validation starting from
// PeerConnection::SetConfiguration.
RTCError GeneralTransportChannel::ValidateIceConfig(const IceConfig& config) {
...
const Connection* GeneralTransportChannel::selected_connection() const {
  return selected_connection_;
}

int GeneralTransportChannel::check_receiving_interval() const {
  return std::max(MIN_CHECK_RECEIVING_INTERVAL,
                  config_.receiving_timeout_or_default() / 10);
}

void GeneralTransportChannel::MaybeStartGathering() {
  // delete by dxf
  ...
  if (allocator_sessions_.empty() ||
    ...
    std::unique_ptr<PortAllocatorSession> pooled_session =
        allocator_->TakePooledSession(transport_name(), component(), "dxf",
                                      "dxf");
...
void GeneralTransportChannel::OnPortReady(PortAllocatorSession* session,
                                          PortInterface* port) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
  ...
  port->SignalUnknownAddress.connect(
      this, &GeneralTransportChannel::OnUnknownAddress);
  port->SignalDestroyed.connect(this,
                                &GeneralTransportChannel::OnPortDestroyed);

  port->SignalRoleConflict.connect(this,
                                   &GeneralTransportChannel::OnRoleConflict);
  port->SignalSentPacket.connect(this, &GeneralTransportChannel::OnSentPacket);

  // Attempt to create a connection from this new port to all of the remote
  // candidates that we were given so far.

  std::vector<GeneralRemoteCandidate>::iterator iter;
...
void GeneralTransportChannel::OnCandidatesReady(
...
void GeneralTransportChannel::OnCandidatesAllocationDone(
    PortAllocatorSession* session) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
  if (config_.gather_continually()) {
    RTC_LOG(LS_INFO) << "GeneralTransportChannel: " << transport_name()
                     << ", component " << component()
...
// Handle stun packets
void GeneralTransportChannel::OnUnknownAddress(
    PortInterface* port,
  ...
  // candidates in GeneralTransportChannel::GetStats. The difficulty of
  // preventing creating the prflx from the beginning is that we do not have a
  // reliable way to claim two candidates are identical without the address
  // information. If we always pause the addition of a prflx candidate when
  // there is ongoing name resolution and dedup after we have a resolved
  // address, we run into the risk of losing/delaying the addition of a
  // non-identical candidate that could be the only way to have a connection, if
  // the resolution never completes or is significantly delayed.
  ...
  if (!remote_candidate_is_new) {
    ...
    if (!priority_attr) {
      RTC_LOG(LS_WARNING) << "GeneralTransportChannel::OnUnknownAddress - "
...
void GeneralTransportChannel::OnRoleConflict(PortInterface* port) {
  SignalRoleConflict(this);  // STUN ping will be sent when SetRole is called
                             // from Transport.
}

const IceParameters* GeneralTransportChannel::FindRemoteIceFromUfrag(
...
void GeneralTransportChannel::OnNominated(Connection* conn) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
void GeneralTransportChannel::ResolveHostnameCandidate(
  ...
  resolver->SignalDone.connect(this,
                               &GeneralTransportChannel::OnCandidateResolved);
...
void GeneralTransportChannel::AddRemoteCandidate(const Candidate& candidate) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
GeneralTransportChannel::CandidateAndResolver::CandidateAndResolver(
    const Candidate& candidate,
...
void GeneralTransportChannel::OnCandidateResolved(
    rtc::AsyncResolverInterface* resolver) {
  auto p =
...
void GeneralTransportChannel::AddRemoteCandidateWithResolver(
    Candidate candidate,
    rtc::AsyncResolverInterface* resolver) {
...
void GeneralTransportChannel::FinishAddingRemoteCandidate(
...
void GeneralTransportChannel::RemoveRemoteCandidate(
...
void GeneralTransportChannel::RemoveAllRemoteCandidates() {
...
// the origin port.
bool GeneralTransportChannel::CreateConnections(
...
bool GeneralTransportChannel::CreateConnection(
    PortInterface* port,
    const Candidate& remote_candidate,
    PortInterface* origin_port) {
...
bool GeneralTransportChannel::FindConnection(Connection* connection) const {
  return absl::c_linear_search(connections_, connection);
}

uint32_t GeneralTransportChannel::GetRemoteCandidateGeneration(
    const Candidate& candidate) {
...
bool GeneralTransportChannel::IsDuplicateRemoteCandidate(
    const Candidate& candidate) {
...
void GeneralTransportChannel::RememberRemoteCandidate(
    const Candidate& remote_candidate,
    PortInterface* origin_port) {
  ...
  remote_candidates_.push_back(
      GeneralRemoteCandidate(remote_candidate, origin_port));
}

// Set options on ourselves is simply setting options on all of our available
// port objects.
int GeneralTransportChannel::SetOption(rtc::Socket::Option opt, int value) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
bool GeneralTransportChannel::GetOption(rtc::Socket::Option opt, int* value) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
int GeneralTransportChannel::GetError() {
  return error_;
}

// Send data to the other side, using our selected connection.
int GeneralTransportChannel::SendPacket(const char* data,
                                        size_t len,
  ...
  int sent = -1; 
  if (selected_connection_ == nullptr && general_connection_) {
    rtc::PacketOptions modified_options(options);
    modified_options.info_signaled_after_sent.packet_type =
        rtc::PacketType::kData;
    sent = general_connection_->Send(data, len, modified_options);
    return sent;
  }
...
bool GeneralTransportChannel::GetStats(
...
absl::optional<rtc::NetworkRoute> GeneralTransportChannel::network_route()
    const {
  return network_route_;
}

rtc::DiffServCodePoint GeneralTransportChannel::DefaultDscpValue() const {
...
void GeneralTransportChannel::UpdateConnectionStates() {
...
void GeneralTransportChannel::RequestSortAndStateUpdate(
    const std::string& reason_to_sort) {
  if (!sort_dirty_) {
    invoker_.AsyncInvoke<void>(
        RTC_FROM_HERE, thread(),
...
void GeneralTransportChannel::MaybeStartPinging() {
  if (started_pinging_) {
    return;
  }
  ...
  if (absl::c_any_of(connections_, [this, now](const Connection* c) {
    ...
    invoker_.AsyncInvoke<void>(
        RTC_FROM_HERE, thread(),
        rtc::Bind(&GeneralTransportChannel::CheckAndPing, this));
    regathering_controller_->Start();
    started_pinging_ = true;
  }
}

int GeneralTransportChannel::CompareCandidatePairNetworks(
    const Connection* a,
    const Connection* b,
...
// states.
int GeneralTransportChannel::CompareConnectionStates(
    const Connection* a,
    const Connection* b,
    absl::optional<int64_t> receiving_unchanged_threshold,
    bool* missed_receiving_unchanged_threshold) const {
...
int GeneralTransportChannel::CompareConnectionCandidates(
    const Connection* a,
...
bool GeneralTransportChannel::IsPortPruned(const Port* port) const {
  return !absl::c_linear_search(ports_, port);
}

bool GeneralTransportChannel::IsRemoteCandidatePruned(
    const Candidate& cand) const {
  return !absl::c_linear_search(remote_candidates_, cand);
}

int GeneralTransportChannel::CompareConnections(
    const Connection* a,
    const Connection* b,
    absl::optional<int64_t> receiving_unchanged_threshold,
    bool* missed_receiving_unchanged_threshold) const {
...
bool GeneralTransportChannel::PresumedWritable(const Connection* conn) const {
...
void GeneralTransportChannel::SortConnectionsAndUpdateState(
    const std::string& reason_to_sort) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
GeneralTransportChannel::GetBestConnectionByNetwork() const {
...
GeneralTransportChannel::GetBestWritableConnectionPerNetwork() const {
...
void GeneralTransportChannel::PruneConnections() {
...
void GeneralTransportChannel::SwitchSelectedConnection(Connection* conn) {
...
void GeneralTransportChannel::UpdateState() {
  ...
  if (state_ != state) {
    ...
    // GeneralTransportChannel does not (yet) implement a direct mapping of the
...
void GeneralTransportChannel::MaybeStopPortAllocatorSessions() {
  if (!IsGettingPorts()) {
    return;
  }
...
void GeneralTransportChannel::HandleAllTimedOut() {
  for (Connection* connection : connections_) {
    connection->Destroy();
  }
}

bool GeneralTransportChannel::weak() const {
  return !selected_connection_ || selected_connection_->weak();
}

bool GeneralTransportChannel::ReadyToSend(Connection* connection) const {
...
void GeneralTransportChannel::CheckAndPing() {
  ...
  invoker_.AsyncInvokeDelayed<void>(
      RTC_FROM_HERE, thread(),
      rtc::Bind(&GeneralTransportChannel::CheckAndPing, this), delay);
}

// A connection is considered a backup connection if the channel state
// is completed, the connection is not the selected connection and it is active.
bool GeneralTransportChannel::IsBackupConnection(const Connection* conn) const {
...
bool GeneralTransportChannel::IsPingable(const Connection* conn,
                                         int64_t now) const {
  return true;  // add by dxf
...
bool GeneralTransportChannel::WritableConnectionPastPingInterval(
...
int GeneralTransportChannel::CalculateActiveWritablePingInterval(
    const Connection* conn,
    int64_t now) const {
...
Connection* GeneralTransportChannel::FindNextPingableConnection() {
  int64_t now = rtc::TimeMillis();
...
void GeneralTransportChannel::MarkConnectionPinged(Connection* conn) {
...
void GeneralTransportChannel::PingConnection(Connection* conn) {
...
uint32_t GeneralTransportChannel::GetNominationAttr(Connection* conn) const {
  return (conn == selected_connection_) ? nomination_ : 0;
}

// Nominate a connection based on the NominationMode.
bool GeneralTransportChannel::GetUseCandidateAttr(Connection* conn,
...
// unusable.
void GeneralTransportChannel::OnConnectionStateChange(Connection* connection) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
// connection.
void GeneralTransportChannel::OnConnectionDestroyed(Connection* connection) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
// connection attempts.
void GeneralTransportChannel::OnPortDestroyed(PortInterface* port) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
void GeneralTransportChannel::OnPortsPruned(
    PortAllocatorSession* session,
...
void GeneralTransportChannel::OnCandidatesRemoved(
    PortAllocatorSession* session,
    const std::vector<Candidate>& candidates) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
...
void GeneralTransportChannel::PruneAllPorts() {
  pruned_ports_.insert(pruned_ports_.end(), ports_.begin(), ports_.end());
  ports_.clear();
}

bool GeneralTransportChannel::PrunePort(PortInterface* port) {
...
void GeneralTransportChannel::OnReadPacket(Connection* connection,
                                           const char* data,
                                           size_t len,
...
void GeneralTransportChannel::OnSentPacket(const rtc::SentPacket& sent_packet) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());

  SignalSentPacket(this, sent_packet);
}

void GeneralTransportChannel::OnReadyToSend(Connection* connection) {
...
Connection* GeneralTransportChannel::FindOldestConnectionNeedingTriggeredCheck(
    int64_t now) {
...
Connection* GeneralTransportChannel::MostLikelyToWork(Connection* conn1,
                                                      Connection* conn2) {
...
Connection* GeneralTransportChannel::LeastRecentlyPinged(Connection* conn1,
                                                         Connection* conn2) {
...
Connection* GeneralTransportChannel::MorePingable(Connection* conn1,
                                                  Connection* conn2) {
...
void GeneralTransportChannel::SetWritable(bool writable) {
...
void GeneralTransportChannel::SetReceiving(bool receiving) {
...
void GeneralTransportChannel::LogCandidatePairConfig(
    Connection* conn,
...
bool GeneralTransportChannel::setUdpConnection(
    rtc::BasicPacketSocketFactory* socketFactory,
    const rtc::SocketAddress& addrLocal,
    const rtc::SocketAddress& addrRemote) {
  if (!general_connection_) {
    general_connection_ = new GeneralConnection(nullptr, 0, Candidate());
    //general_connection_->SignalReadPacket.connect(this, &GeneralTransportChannel::OnReadPacket);
    general_connection_->SignalReadPacket.connect(
        this, &GeneralTransportChannel::OnReadPacket);
    general_connection_->SignalReadyToSend.connect(
        this, &GeneralTransportChannel::OnReadyToSend);
    general_connection_->SignalStateChange.connect(
        this, &GeneralTransportChannel::OnConnectionStateChange);
    general_connection_->SignalDestroyed.connect(
        this, &GeneralTransportChannel::OnConnectionDestroyed);
  ...
  if (general_connection_ && socketFactory) {
   // rtc::AsyncPacketSocket* udpSocket =
    udpSocket_ =
        socketFactory->CreateUdpSocket(addrLocal, 0, 0);
...
// When a GeneralConnection's state changes, we need to figure out who to use as
// the selected connection again.  It could have become usable, or become
// unusable.
void GeneralTransportChannel::OnConnectionStateChange(
    GeneralConnection* connection) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());

  if (connection == nullptr || connection->port() == nullptr)
    return;
  // May stop the allocator session when at least one connection becomes
  // strongly connected after starting to get ports and the local candidate of
  // the connection is at the latest generation. It is not enough to check
...
// When a connection is removed, edit it out, and then update our best
// connection.
void GeneralTransportChannel::OnConnectionDestroyed(GeneralConnection* connection) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());

  if (!general_connection_ || !connection)
    return;
...
// We data is available, let listeners know
void GeneralTransportChannel::OnReadPacket(GeneralConnection* connection,
                                           const char* data,
                                           size_t len,
                                           int64_t packet_time_us) {
  RTC_DCHECK(network_thread_ == rtc::Thread::Current());
