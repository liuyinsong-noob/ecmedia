...
inline bool generalTooManyFailures(
    const std::vector<cricket::GeneralConnection::SentPing>&
...
// Determines whether we have gone too long without seeing any response.
inline bool generalTooLongWithoutResponse(
    const std::vector<cricket::GeneralConnection::SentPing>&
        pings_since_last_response,
    int64_t maximum_time,
    int64_t now) {
...
///////////////////////////////////GeneralConnectionRequest////////////////////////////////////////////
// A ConnectionRequest is a simple STUN ping used to determine writability.
class GeneralConnectionRequest : public StunRequest {
 public:
  explicit GeneralConnectionRequest(GeneralConnection* connection)
      : StunRequest(new IceMessage()), connection_(connection) {}
...
GeneralConnection::GeneralConnection(Port* port,
                       size_t index,
                       const Candidate& remote_candidate)
    : id_(rtc::CreateRandomId()),
      port_(port),
      ...
      use_candidate_attr_(false),
      remote_ice_mode_(ICEMODE_FULL),
      requests_(port != nullptr ? port->thread() : nullptr),
      rtt_(DEFAULT_RTT),
      last_ping_sent_(0),
      last_ping_received_(0),
      last_data_received_(0),
  ...
  // TODO(mallinath) - Start connections from STATE_FROZEN.
  // Wire up to send stun packets
  requests_.SignalSendPacket.connect(this, &GeneralConnection::OnSendStunPacket);
  RTC_LOG(LS_INFO) << ToString() << ": GeneralConnection created";
}

GeneralConnection::~GeneralConnection() {}

const Candidate& GeneralConnection::local_candidate() const {
  RTC_DCHECK(local_candidate_index_ < port_->Candidates().size());
  return port_->Candidates()[local_candidate_index_];
}

const Candidate& GeneralConnection::remote_candidate() const {
  return remote_candidate_;
}

uint64_t GeneralConnection::priority() const {
  uint64_t priority = 0;
  // RFC 5245 - 5.7.2.  Computing Pair Priority and Ordering Pairs
  // Let G be the priority for the candidate provided by the controlling
  // agent.  Let D be the priority for the candidate provided by the
...
void GeneralConnection::set_write_state(WriteState value) {
  WriteState old_value = write_state_;
  write_state_ = value;
  if (value != old_value) {
    RTC_LOG(LS_VERBOSE) << ToString() << ": set_write_state from: " << old_value
...
void GeneralConnection::UpdateReceiving(int64_t now) {
  bool receiving;
  if (last_ping_sent() < last_ping_response_received()) {
    // We consider any candidate pair that has its last connectivity check
    // acknowledged by a response as receiving, particularly for backup
...
void GeneralConnection::set_state(IceCandidatePairState state) {
  IceCandidatePairState old_state = state_;
  state_ = state;
  if (state != old_state) {
    RTC_LOG(LS_VERBOSE) << ToString() << ": set_state";
...
void GeneralConnection::set_connected(bool value) {
  bool old_value = connected_;
  connected_ = value;
  if (value != old_value) {
    RTC_LOG(LS_VERBOSE) << ToString() << ": Change connected_ to " << value;
...
void GeneralConnection::set_use_candidate_attr(bool enable) {
  use_candidate_attr_ = enable;
}

int GeneralConnection::unwritable_timeout() const {
  return unwritable_timeout_.value_or(CONNECTION_WRITE_CONNECT_TIMEOUT);
}

int GeneralConnection::unwritable_min_checks() const {
  return unwritable_min_checks_.value_or(CONNECTION_WRITE_CONNECT_FAILURES);
}

int GeneralConnection::inactive_timeout() const {
  return inactive_timeout_.value_or(CONNECTION_WRITE_TIMEOUT);
}

int GeneralConnection::receiving_timeout() const {
  return receiving_timeout_.value_or(WEAK_CONNECTION_RECEIVE_TIMEOUT);
}

void GeneralConnection::OnSendStunPacket(const void* data,
                                  size_t size,
                                  StunRequest* req) {
  rtc::PacketOptions options(port_->StunDscpValue());
  options.info_signaled_after_sent.packet_type =
...
void GeneralConnection::OnReadPacket(const char* data,
                              size_t size,
                              int64_t packet_time_us) {
  std::unique_ptr<IceMessage> msg;
  std::string remote_ufrag;
  ...
  if (!port_->GetStunMessage(data, size, addr, &msg, &remote_ufrag)) {
    ...
    if (!pruned_ && (write_state_ == STATE_WRITE_TIMEOUT)) {
      RTC_LOG(LS_WARNING)
          << "Received a data packet on a timed-out GeneralConnection. "
             "Resetting state to STATE_WRITE_INIT.";
      set_write_state(STATE_WRITE_INIT);
    }
  } else if (!msg) {
...
void GeneralConnection::HandleBindingRequest(IceMessage* msg) {
  // This connection should now be receiving.
  ReceivedPing();
  if (webrtc::field_trial::IsEnabled("WebRTC-ExtraICEPing") &&
      last_ping_response_received_ == 0) {
...
void GeneralConnection::OnReadyToSend() {
  SignalReadyToSend(this);
}

void GeneralConnection::Prune() {
  if (!pruned_ || active()) {
    RTC_LOG(LS_INFO) << ToString() << ": GeneralConnection pruned";
    pruned_ = true;
    requests_.Clear();
    set_write_state(STATE_WRITE_TIMEOUT);
  }
...
void GeneralConnection::Destroy() {
  // TODO(deadbeef, nisse): This may leak if an application closes a
  // PeerConnection and then quickly destroys the PeerConnectionFactory (along
  // with the networking thread on which this message is posted). Also affects
  // tests, with a workaround in
  // AutoSocketServerThread::~AutoSocketServerThread.
  RTC_LOG(LS_VERBOSE) << ToString() << ": GeneralConnection destroyed";
  port_->thread()->Post(RTC_FROM_HERE, this, MSG_DELETE);
  LogCandidatePairConfig(webrtc::IceCandidatePairConfigType::kDestroyed);
}

void GeneralConnection::FailAndDestroy() {
  set_state(IceCandidatePairState::FAILED);
  Destroy();
}

void GeneralConnection::FailAndPrune() {
  set_state(IceCandidatePairState::FAILED);
  Prune();
}

void GeneralConnection::PrintPingsSinceLastResponse(std::string* s, size_t max) {
  rtc::StringBuilder oss;
  if (pings_since_last_response_.size() > max) {
    for (size_t i = 0; i < max; i++) {
      const SentPing& ping = pings_since_last_response_[i];
...
void GeneralConnection::UpdateState(int64_t now) {
  int rtt = ConservativeRTTEstimate(rtt_);
  ...
  if ((write_state_ == STATE_WRITABLE) &&
      generalTooManyFailures(pings_since_last_response_, unwritable_min_checks(), rtt,
                      now) &&
      generalTooLongWithoutResponse(pings_since_last_response_, unwritable_timeout(),
                             now)) {
    uint32_t max_pings = unwritable_min_checks();
    RTC_LOG(LS_INFO) << ToString() << ": Unwritable after " << max_pings
                     << " ping failures and "
  ...
  if ((write_state_ == STATE_WRITE_UNRELIABLE ||
       write_state_ == STATE_WRITE_INIT) &&
      generalTooLongWithoutResponse(pings_since_last_response_, inactive_timeout(),
                             now)) {
    RTC_LOG(LS_INFO) << ToString() << ": Timed out after "
                     << now - pings_since_last_response_[0].sent_time
                     << " ms without a response, rtt=" << rtt;
...
void GeneralConnection::Ping(int64_t now) {
  last_ping_sent_ = now;
  GeneralConnectionRequest* req = new GeneralConnectionRequest(this);
  // If not using renomination, we use "1" to mean "nominated" and "0" to mean
  // "not nominated". If using renomination, values greater than 1 are used for
  // re-nominated pairs.
  int nomination = use_candidate_attr_ ? 1 : 0;
...
void GeneralConnection::ReceivedPing() {
  last_ping_received_ = rtc::TimeMillis();
  UpdateReceiving(last_ping_received_);
}

void GeneralConnection::ReceivedPingResponse(int rtt, const std::string& request_id) {
  RTC_DCHECK_GE(rtt, 0);
  // We've already validated that this is a STUN binding response with
  // the correct local and remote username for this connection.
  // So if we're not already, become writable. We may be bringing a pruned
...
bool GeneralConnection::dead(int64_t now) const {
  if (last_received() > 0) {
    // If it has ever received anything, we keep it alive until it hasn't
    // received anything for DEAD_CONNECTION_RECEIVE_TIMEOUT. This covers the
    // normal case of a successfully used connection that stops working. This
...
bool GeneralConnection::stable(int64_t now) const {
  // A connection is stable if it's RTT has converged and it isn't missing any
  // responses.  We should send pings at a higher rate until the RTT converges
  // and whenever a ping response is missing (so that we can detect
  // unwritability faster)
...
std::string GeneralConnection::ToDebugId() const {
  return rtc::ToHex(reinterpret_cast<uintptr_t>(this));
}

uint32_t GeneralConnection::ComputeNetworkCost() const {
  // TODO(honghaiz): Will add rtt as part of the network cost.
  return 0;  // port()->network_cost() + remote_candidate_.network_cost();
}

std::string GeneralConnection::ToString() const {
  return std::string();
  //const absl::string_view CONNECT_STATE_ABBREV[2] = {
  //    "-",  // not connected (false)
  //    "C",  // connected (true)
...
std::string GeneralConnection::ToSensitiveString() const {
  return ToString();
}

const webrtc::IceCandidatePairDescription& GeneralConnection::ToLogDescription() {
  if (log_description_.has_value()) {
    return log_description_.value();
  }
  const Candidate& local = local_candidate();
...
void GeneralConnection::LogCandidatePairConfig(
    webrtc::IceCandidatePairConfigType type) {
  if (ice_event_log_ == nullptr) {
    return;
  }
...
void GeneralConnection::LogCandidatePairEvent(webrtc::IceCandidatePairEventType type,
                                       uint32_t transaction_id) {
  if (ice_event_log_ == nullptr) {
    return;
  }
...
void GeneralConnection::OnConnectionRequestResponse(GeneralConnectionRequest* request,
                                             StunMessage* response) {
  // Log at LS_INFO if we receive a ping response on an unwritable
  // connection.
  //rtc::LoggingSeverity sev = !writable() ? rtc::LS_INFO : rtc::LS_VERBOSE;
...
void GeneralConnection::OnConnectionRequestErrorResponse(GeneralConnectionRequest* request,
                                                  StunMessage* response) {
  int error_code = response->GetErrorCodeValue();
  RTC_LOG(LS_WARNING) << ToString() << ": Received STUN error response id="
                      << rtc::hex_encode(request->id())
...
void GeneralConnection::OnConnectionRequestTimeout(GeneralConnectionRequest* request) {
  // Log at LS_INFO if we miss a ping on a writable connection.
  rtc::LoggingSeverity sev = writable() ? rtc::LS_INFO : rtc::LS_VERBOSE;
  RTC_LOG_V(sev) << ToString() << ": Timing-out STUN ping "
                 << rtc::hex_encode(request->id()) << " after "
...
void GeneralConnection::OnConnectionRequestSent(GeneralConnectionRequest* request) {
  // Log at LS_INFO if we send a ping on an unwritable connection.
  rtc::LoggingSeverity sev = !writable() ? rtc::LS_INFO : rtc::LS_VERBOSE;
  RTC_LOG_V(sev) << ToString()
                 << ": Sent STUN ping, id=" << rtc::hex_encode(request->id())
...
void GeneralConnection::HandleRoleConflictFromPeer() {
  port_->SignalRoleConflict(port_);
}

void GeneralConnection::MaybeSetRemoteIceParametersAndGeneration(
    const IceParameters& ice_params,
    int generation) {
  if (remote_candidate_.username() == ice_params.ufrag &&
      remote_candidate_.password().empty()) {
...
void GeneralConnection::MaybeUpdatePeerReflexiveCandidate(
    const Candidate& new_candidate) {
  if (remote_candidate_.type() == PRFLX_PORT_TYPE &&
      new_candidate.type() != PRFLX_PORT_TYPE &&
      remote_candidate_.protocol() == new_candidate.protocol() &&
...
void GeneralConnection::OnMessage(rtc::Message* pmsg) {
  RTC_DCHECK(pmsg->message_id == MSG_DELETE);
  RTC_LOG(LS_INFO) << "GeneralConnection deleted with number of pings sent: "
                   << num_pings_sent_;
  SignalDestroyed(this);
  delete this;
}

int64_t GeneralConnection::last_received() const {
  return std::max(last_data_received_,
                  std::max(last_ping_received_, last_ping_response_received_));
}

ConnectionInfo GeneralConnection::stats() {
  stats_.recv_bytes_second = round(recv_rate_tracker_.ComputeRate());
  stats_.recv_total_bytes = recv_rate_tracker_.TotalSampleCount();
  stats_.sent_bytes_second = round(send_rate_tracker_.ComputeRate());
  stats_.sent_total_bytes = send_rate_tracker_.TotalSampleCount();
...
void GeneralConnection::MaybeUpdateLocalCandidate(ConnectionRequest* request,
                                           StunMessage* response) {
  // RFC 5245
  // The agent checks the mapped address from the STUN response.  If the
  // transport address does not match any of the local candidates that the
  ...
  if (!addr) {
    RTC_LOG(LS_WARNING)
        << "GeneralConnection::OnConnectionRequestResponse - "
           "No MAPPED-ADDRESS or XOR-MAPPED-ADDRESS found in the "
           "stun response message";
    return;
  }
  ...
  for (size_t i = 0; i < port_->Candidates().size(); ++i) {
    if (port_->Candidates()[i].address() == addr->GetAddress()) {
      if (local_candidate_index_ != i) {
        ...
        local_candidate_index_ = i;
        // SignalStateChange to force a re-sort in P2PTransportChannel as this
        // GeneralConnection's local candidate has changed.
        SignalStateChange(this);
      }
      return;
    }
  ...
  const StunUInt32Attribute* priority_attr =
      request->msg()->GetUInt32(STUN_ATTR_PRIORITY);
  if (!priority_attr) {
    RTC_LOG(LS_WARNING) << "GeneralConnection::OnConnectionRequestResponse - "
                           "No STUN_ATTR_PRIORITY found in the "
                           "stun response message";
    return;
  }
  ...
  // Change the local candidate of this GeneralConnection to the new prflx candidate.
  RTC_LOG(LS_INFO) << ToString() << ": Updating local candidate type to prflx.";
  local_candidate_index_ = port_->AddPrflxCandidate(new_local_candidate);
...
void GeneralConnection::CopyCandidatesToStatsAndSanitizeIfNecessary() {
  auto get_sanitized_copy = [](const Candidate& c) {
    bool use_hostname_address = c.type() == LOCAL_PORT_TYPE;
    bool filter_related_address = c.type() == STUN_PORT_TYPE;
    return c.ToSanitizedCopy(use_hostname_address, filter_related_address);
...
bool GeneralConnection::rtt_converged() const {
  return rtt_samples_ > (RTT_RATIO + 1);
}

bool GeneralConnection::missing_responses(int64_t now) const {
  if (pings_since_last_response_.empty()) {
    return false;
  }
...
bool GeneralConnection::setUdpSocket(rtc::AsyncPacketSocket* udpSocket,
                                     const rtc::SocketAddress& addrRemote) {
  //udpSocket_.
  udpSocket_ = udpSocket;
  addrRemote_ = addrRemote;
  return true;
}

int GeneralConnection::Send(const void* data,
                          size_t size,
                          const rtc::PacketOptions& options) {
  stats_.sent_total_packets++;
  int sent = 0;
...
int GeneralConnection::GetError() {
  return error_;
}


void GeneralConnection::OnReadPacket(rtc::AsyncPacketSocket* socket,
                                      const char* data,
                                      size_t size,
                                      const rtc::SocketAddress& remote_addr,
