...
// GeneralTransportChannel wraps up the state management of the connection between
...
//
#ifndef GENERAL_BASE_P2P_TRANSPORT_CHANNEL_H_
#define GENERAL_BASE_P2P_TRANSPORT_CHANNEL_H_
...
enum class GeneralRestartState { CONNECTING, CONNECTED, DISCONNECTED, MAX_VALUE };
...
class GeneralRemoteCandidate : public Candidate {
 public:
  GeneralRemoteCandidate(const Candidate& c, PortInterface* origin_port)
...
// GeneralTransportChannel manages the candidates and connection process to keep
// two P2P clients connected to each other.
class RTC_EXPORT GeneralTransportChannel : public IceTransportInternal {
 public:
  // For testing only.
  // TODO(zstein): Remove once AsyncResolverFactory is required.
  GeneralTransportChannel(const std::string& transport_name,
                      int component,
  ...
  ~GeneralTransportChannel() override;
  ...
  // Public for unit tests.
  const std::vector<GeneralRemoteCandidate>& remote_candidates() const {
  ...
  bool setUdpConnection(rtc::BasicPacketSocketFactory* socketFactory, const rtc::SocketAddress& addrLocal, const rtc::SocketAddress& addrRemote);
  bool ReleaseUdpConnection();
  rtc::AsyncPacketSocket* udpSocket_; 
 private:
  ...
  //add by dxf begin
    void OnConnectionStateChange(GeneralConnection* connection);
  void OnReadPacket(GeneralConnection* connection,
                    const char* data,
                    size_t len,
                    int64_t packet_time_us);
  void OnReadyToSend(GeneralConnection* connection);
  ...
  GeneralConnection* general_connection_ = nullptr;

  std::vector<GeneralRemoteCandidate> remote_candidates_;
  ...
  RTC_DISALLOW_COPY_AND_ASSIGN(GeneralTransportChannel);
};

}  // namespace cricket

#endif  // GENERAL_BASE_P2P_TRANSPORT_CHANNEL_H_
