...
int ServiceCore::SetAudioKeepAlive(SerPhoneCall *call, bool enable, int interval)
{
    ...
    {
#ifdef VIDEO_ENABLED
        return ECMedia_createLiveStream();
#endif
    }
    
    int ServiceCore::configLiveVideoStream(void *handle, LiveVideoStreamConfig config) {
#ifdef VIDEO_ENABLED
        return ECMedia_ConfigLiveVideoStream(handle, config);
#endif
    }
    ...
    int ServiceCore::liveStream_SelectCamera(void *handle, int index)
    {
#ifdef VIDEO_ENABLED
        //	CameraCapability cap;
        //	cap.width = width;
        //	cap.height = height;
        //	cap.maxfps = fps;
        return ECMedia_SwitchLiveCamera(handle, index);
#endif
        return 0;
    }
...
int ServiceCore::startSendRtpPacket(int &channel, const char *ip, int rtp_port)
{
	...
	//setSendDestination
	ECMedia_audio_set_send_destination(channel, rtp_port, ip, 0, rtp_port+1, ip);
	//startRecord
	ECMedia_audio_start_record();
	//startSend
