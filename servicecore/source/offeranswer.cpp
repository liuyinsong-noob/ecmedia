	...
	for(i=0; i<SAL_CRYPTO_ALGO_MAX; i++) {
		...
		for(j=0; j<SAL_CRYPTO_ALGO_MAX; j++) {
            ...
            /* We're answering an SDP offer. Supply our master key, associated with the remote supplied tag */
				if (use_local_key) {
					strncpy(result->master_key, local[j].master_key, 65);//sean 20130512 41
...
//                printf("use_local_key not\n");
					strncpy(result->master_key, remote[i].master_key, 65);//sean 20130512 41
					result->tag = local[j].tag;
                    *choosen_local_tag = local[j].tag;
				}
				result->master_key[64] = '\0';  //40
//                printf("result->master_key = %s\n",result->master_key);
				return TRUE;
			}
		}
	}
...
//        if (!match_crypto_algo(local_cap->crypto, remote_offer->crypto, &result->crypto[0], &result->crypto_local_tag, FALSE))
			result->port = 0;
...
//	for(i=0,j=0;i<local_offer->nstreams;++i){
	...
	result->nstreams=result->n_total_streams;
	result->bandwidth=remote_answer->bandwidth;
	strcpy(result->addr,remote_answer->addr);
	return 0;
}
