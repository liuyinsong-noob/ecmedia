...
static void add_payload(sdp_message_t *msg, int line, const PayloadType *pt, bool_t strip_well_known_rtpmaps, bool_t nack_support)
{
	char attr[256];
	...
	if(nack_support)
	{
		snprintf(attr, sizeof(attr),  "%i nack",  payload_type_get_number(pt));
...
static void add_line(sdp_message_t *msg, int lineno, const SalStreamDescription *desc, bool usermode = false){
	const char *mt=NULL;
	const MSList *elem;
	const char *rtp_addr;
	...
	if (desc->payloads){
		for(elem=desc->payloads;elem!=NULL;elem=elem->next){
			add_payload(msg, lineno, (PayloadType*)elem->data,strip_well_known_rtpmaps, desc->nack_support);
		}
...
sdp_message_t *media_description_to_sdp(const SalMediaDescription *desc){
	...
	for(i=0;i<desc->n_total_streams;++i){
		add_line(msg,i,&desc->streams[i],desc->usermode);
	}
	return msg;
}
...
int sdp_to_media_description(sdp_message_t *msg, SalMediaDescription *desc, bool_t isreinvite = 0){
//    printf("%s called\n",__FUNCTION__);
	int i,j;
	...
	for (i=0; !sdp_message_endof_media (msg, i) && i<SAL_MEDIA_DESCRIPTION_MAX_STREAMS; i++)
	{
		...
		if (addr != NULL) {
            ...
            strncpy(stream->rtp_addr, addr, sizeof(stream->rtp_addr));
        }
		if (port)
        {
			stream->port=atoi(port);
            stream->rtp_port = stream->port;
        }
		...
		stream->dir=(SalStreamDir)_sdp_message_get_mline_dir(msg,i);
        if (stream->rtp_port == 0) {
            stream->dir = (SalStreamDir)SalStreamInactive;
        }
		...
		for (j=0;((number=sdp_message_m_payload_get (msg, i,j)) != NULL); j++){
			...
			const char *rtcpfb;
			rtcpfb = sdp_message_a_attr_value_get_with_pt(msg, i, ptn, "rtcp-fb");
			if(rtcpfb  && !strcmp(rtcpfb, "nack"))
			{
				stream->nack_support = true;
			}
		}
				...
				if (keywordcmp("crypto",attr->a_att_field)==0 && attr->a_att_value!=NULL){
					...
					if (nb == 3) {
						if (strcmp(tmp, "AES_CM_128_HMAC_SHA1_80") == 0)
							stream->crypto[valid_count].algo = CCPAES_128_SHA1_80;
						else if (strcmp(tmp, "AES_CM_128_HMAC_SHA1_32") == 0)
							stream->crypto[valid_count].algo = CCPAES_128_SHA1_32;
                        else if (strcmp(tmp, "AES_CM_256_HMAC_SHA1_80") == 0)
                            stream->crypto[valid_count].algo = CCPAES_256_SHA1_80;
                        else if (strcmp(tmp, "AES_CM_256_HMAC_SHA1_32") ==  0)
                            stream->crypto[valid_count].algo = CCPAES_256_SHA1_32;
						else {
							PrintConsole("Failed to parse crypto-algo: '%s'\n", tmp);
							stream->crypto[valid_count].algo = (enum ccportp_srtp_crypto_suite_t)0;
						}
...
//							stream->crypto[valid_count].master_key[40] = '\0';
		...
		for (j = 0; (attr = sdp_message_attribute_get(msg, i, j)) != NULL; j++) {
			if ((keywordcmp("nortpproxy", attr->a_att_field) == 0) && (attr->a_att_value != NULL)) {
                if (keywordcmp("yes", attr->a_att_value)==0) {
                    iceCompleted = true;
                }
            }
			if ((keywordcmp("candidate", attr->a_att_field) == 0) && (attr->a_att_value != NULL)) {
                ...
                if (1 == candidate->componentID) {
                    strcpy(stream->rtp_addr,candidate->addr);
                    stream->rtp_addr[strlen(candidate->addr)] = '\0';
                    stream->rtp_port = candidate->port;
                   if (iceCompleted) {
                        strcpy(stream->addr, candidate->addr);
                        stream->addr[strlen(stream->rtp_addr)] = '\0';
                       stream->port = candidate->port;
                    }
                }
                if (2 == candidate->componentID) {
                    strcpy(stream->rtcp_addr, candidate->addr);
                    stream->rtcp_addr[strlen(candidate->addr)] = '\0';
                    stream->rtcp_port = candidate->port;
                }
        ...
        //If ice, add c attribute as a ice candidate
//        if (nb_ice_candidates>0 && isreinvite == 0) {
//            const char *raddr_local = stream->ice_candidates[nb_ice_candidates-1].raddr;
//            int rport_local = stream->ice_candidates[nb_ice_candidates-1].rport;
//            
//            //rtp
//            SalIceCandidate *candidate = &stream->ice_candidates[nb_ice_candidates++];
//            IceCandidate tempCandidate;
//            tempCandidate.type = ICT_RelayedCandidate;
//            tempCandidate.componentID = 1;
//            ice_compute_candidate_priority(&tempCandidate);
//            
//            
//            strcpy(candidate->foundation, "3");
//            candidate->componentID = 1;
//            candidate->priority = tempCandidate.priority;
//            strcpy(candidate->addr, msg->c_connection->c_addr);
//            candidate->port = atoi(port);
//            strcpy(candidate->type, "relay");
//            strcpy(candidate->raddr, raddr_local);
//            candidate->rport = rport_local;
//            
//            //rtcp
//            tempCandidate.componentID = 2;
//            ice_compute_candidate_priority(&tempCandidate);
//            candidate = &stream->ice_candidates[nb_ice_candidates++];
//            strcpy(candidate->foundation, "3");
//            candidate->componentID = 2;
//            candidate->priority = tempCandidate.priority;
//            strcpy(candidate->addr, msg->c_connection->c_addr);
//            candidate->port = atoi(port)+1;
//            strcpy(candidate->type, "relay");
//            strcpy(candidate->raddr, raddr_local);
//            candidate->rport = rport_local;
//            
