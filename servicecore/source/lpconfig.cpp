...
LpItem * lp_item_new(const char *key, const char *value){
	...
	item->key=ser_strdup(key);
	item->value=ser_strdup(value);
	return item;
}

LpSection *lp_section_new(const char *name){
	LpSection *sec=lp_new0(LpSection,1);
	sec->name=ser_strdup(name);
	return sec;
}
					...
					if (pos2-pos1>=0){
						...
						if (cur!=NULL){
							...
							if (item==NULL){
								...
								ms_free((void **)&item->value);
                                item->value = NULL;
#if defined(WIN32)//added by zdm
...
LpConfig * lp_config_new(const char *filename){
	...
	if (filename!=NULL){
		lpconfig->filename=ser_strdup(filename);
#if !defined(_WIN32) 
		lpconfig->file=fopen(filename,"rw");   //这种模式在windows调试模式下不支持
#else
		...
		if (lpconfig->file!=NULL){
			...
			if (_chmod(filename, _S_IREAD | _S_IWRITE) == -1)        //added by zdm
#else
			/* make existing configuration files non-group/world-accessible */
