...
static void ice_send_stun_server_binding_request(void* base, const struct sockaddr *server, socklen_t addrlen, IceStunServerCheck *check,WebRtc_Word32 isRTCP=0, WebRtc_Word32 isVideo=0, int channel = -1);
...
void ice_session_destroy(IceSession *session)
{
	if (session != NULL) {
		...
		if (session->local_ufrag)
        {
            ms_free((void **)&session->local_ufrag);
        }
		if (session->local_pwd)
        {
            ms_free((void **)&session->local_pwd);
        }
		if (session->remote_ufrag)
        {
            ms_free((void **)&session->remote_ufrag);
        }
		if (session->remote_pwd)
        {
            ms_free((void **)&session->remote_pwd);
        }
		ms_list_free(session->streams);
		ms_free((void **)&session);
	}
}
...
static IceCandidatePair *ice_pair_new(IceCheckList *cl, IceCandidate* local_candidate, IceCandidate *remote_candidate)
{
	...
	pair->role = cl->session111->role;
	ice_compute_pair_priority(pair, &cl->session111->role);
	return pair;
}

static void ice_free_stun_server_check_transaction(IceStunServerCheckTransaction **transaction)
{
	ms_free((void **)transaction);
}
...
static void ice_free_stun_server_check(IceStunServerCheck **check)
{
	...
	//hubin resolve memeory leak
	ms_list_free((MSList*)((*check)->transactions));
...
void ice_check_list_destroy(IceCheckList **cl)
{
	if ((*cl)->remote_ufrag)
    {
        ms_free((void **)&(*cl)->remote_ufrag);
    }
	if ((*cl)->remote_pwd)
    {
        ms_free((void **)&(*cl)->remote_pwd);
    }
...
const char * ice_check_list_local_ufrag(const IceCheckList* cl)
{
	...
	return cl->session111->local_ufrag;
}
...
bool_t ice_check_list_remote_credentials_changed(IceCheckList *cl, const char *ufrag, const char *pwd)
{
	...
	if ((cl->remote_ufrag == NULL) || (cl->remote_pwd == NULL)) {
		if (cl->remote_ufrag == NULL) old_ufrag = cl->session111->remote_ufrag;
		else old_ufrag = cl->remote_ufrag;
		if ((strlen(ufrag) != strlen(old_ufrag)) || (strcmp(ufrag, old_ufrag) != 0)) return TRUE;
		if (cl->remote_pwd == NULL) old_pwd = cl->session111->remote_pwd;
...
{
	ms_list_for_each2((*cl)->pairs, (void (*)(void*,void*))ice_compute_pair_priority, &(*cl)->session111->role);
}
...
//    yuntongxunwebrtc::VoEBase* base = NULL;
	...
	if ((cl->rtp_session != NULL) && (cl->gathering_candidates == FALSE) && (cl->state != ICL_Completed) && (ice_check_list_candidates_gathered(cl) == FALSE)) {
        ...
        {
            base = rtp_session_get_voeBase(cl->rtp_session);
        }
        else if (IceNetworkVideo == cl->rtp_session->session_type)
        {
            base = rtp_session_get_vieNetwork(cl->rtp_session);
        }
        else
        {
...
//            for rtp
            ...
            check->sock = rtp_session_get_local_port(cl->rtp_session);//reuse to get right check when receive response
            check->base = base;
            int tempPort = rtp_session_get_local_port(cl->rtp_session);
			check->srcport = tempPort;//rtp_session_get_local_port(cl->rtp_session);
			if (si->index == 0) {
				check->next_transmission_time = ice_add_ms(curtime, ICE_DEFAULT_RTO_DURATION);
                ice_send_stun_server_binding_request(base, (struct sockaddr *)&cl->session111->ss, cl->session111->ss_len, check,0,cl->rtp_session->session_type,cl->rtp_session->channel);
			} else {
...
//            for rtcp
            check = (IceStunServerCheck *)ms_new0(IceStunServerCheck, 1);
            check->sock = rtp_session_get_local_port(cl->rtp_session)+1;//reuse to get right check when receive response
            check->base = base;
			check->srcport = rtp_session_get_local_port(cl->rtp_session) + 1;
            if (si->index == 0) {
				check->next_transmission_time = ice_add_ms(curtime, ICE_DEFAULT_RTO_DURATION);
                ice_send_stun_server_binding_request(base, (struct sockaddr *)&cl->session111->ss, cl->session111->ss_len, check, 1,cl->rtp_session->session_type,cl->rtp_session->channel);
			} else {
...
void ice_session_gather_candidates(IceSession *session, struct sockaddr_storage ss, socklen_t ss_len)
{
	...
	si.session = session;
	si.index = 0;
	ortp_get_cur_time(&session->gathering_start_ts);
 ...
 *****************************************************************************/
/*update begin------------------Sean someday----------for video ice------------*/
//static void ice_send_stun_server_binding_request(ortp_socket_t sock, const struct sockaddr *server, socklen_t addrlen, IceStunServerCheck *check)
static void ice_send_stun_server_binding_request(void* base, const struct sockaddr *server, socklen_t addrlen, IceStunServerCheck *check,WebRtc_Word32 isRTCP,WebRtc_Word32 isVideo, int channel)
/*update end--------------------Sean someday----------for video ice------------*/
{
	StunMessage msg;
...
//        printf("ice: Send STUN binding request from port %u [%s]\n", check->srcport, tr_id_str);
        ...
        if (IceNetworkAudio == isVideo) {
            ...
            //    PrintConsole("WARNING: ice video send bind packet error!\n");
            //}
        }
...
//    PrintConsole("remote_ufrag = %s\n",ice_check_list_remote_ufrag(cl));
	...
	if ((cl->session111->role == IR_Controlling) && (pair->use_candidate == TRUE)) {
	...
	switch (cl->session111->role) {
		case IR_Controlling:
//            PrintConsole("cl->session->role is IR_Controlling\n");
			msg.hasIceControlling = TRUE;
			msg.iceControlling.value = cl->session111->tie_breaker;
			break;
		case IR_Controlled:
//            PrintConsole("cl->session->role is IR_Controlled\n");
			msg.hasIceControlled = TRUE;
			msg.iceControlled.value = cl->session111->tie_breaker;
			break;
	}
	...
	if (len > 0) {
        ...
        //if (IceNetworkAudio == cl->rtp_session->session_type) {
        //    base->SendRaw(cl->rtp_session->channel, (const WebRtc_Word8 *)buf, len, pair->local->taddr.port==rtp_session_get_local_port(rtp_session)?0:1,dest.port,inet_ntoa(sin_addr));
        //}
        //if (IceNetworkVideo == cl->rtp_session->session_type) {
        //    int transmitted_bytes;
        //    network->SendUDPPacket(cl->rtp_session->channel, (const WebRtc_Word8 *)buf, len, transmitted_bytes, pair->local->taddr.port==rtp_session_get_local_port(rtp_session)?0:1,dest.port,inet_ntoa(sin_addr));
        //}
...
static void ice_send_binding_response(const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *dest)
{
    ...
    if (IceNetworkAudio == rtp_session->session_type) {
        base = rtp_session->VoEBase;
    }
    else if (IceNetworkVideo == rtp_session->session_type)
    {
        base = rtp_session->ViENetwork;
    }
...
{
	if (msg->hasUseCandidate && (cl->session111->role == IR_Controlled)) {
		switch (pair->state) {
			case ICP_Succeeded:
				pair->is_nominated = TRUE;
				break;
...
static IceCandidatePair * ice_construct_valid_pair(IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data, IceCandidate *candidate, IceCandidatePair *succeeded_pair)
{
	...
	if (elem == NULL) {
		...
		if (elem != NULL) {
			...
			valid_pair->selected = TRUE;
			if (ice_session_nb_losing_pairs(cl->session111) == 0) {
...
static void ice_handle_received_binding_response(IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr, void *data)
{
	...
	ortp_socket_t sock;
	int componentID;
	const struct sockaddr_in *servaddr = (const struct sockaddr_in *)&cl->session111->ss;
	bool_t stun_server_response = FALSE;
    
    struct in_addr temp;
    temp.s_addr = remote_addr->addr;
	...
	if (cl->gathering_candidates == TRUE) {
		if ((htonl(remote_addr->addr) == servaddr->sin_addr.s_addr) && (htons(remote_addr->port) == servaddr->sin_port)) {
            sock = evt_data->info.socket_type == OrtpRTPSocket ? rtp_session_get_local_port(rtp_session):rtp_session_get_local_port(rtp_session)+1;
			...
			if (ms_list_find_custom(cl->stun_server_checks, (MSCompareFunc)ice_find_non_responded_stun_server_check, NULL) == NULL) {
				...
				if (ms_list_find_custom(cl->session111->streams, (MSCompareFunc)ice_find_check_list_gathering_candidates, NULL) == NULL) {
					...
					ev = ortp_event_new(ORTP_EVENT_ICE_GATHERING_FINISHED);
					ortp_event_get_data(ev)->info.ice_processing_successful = TRUE;
					cl->session111->gathering_end_ts = evt_data->ts;
                    
                    SerPhoneCall *call = (SerPhoneCall*)data;
                    call->core->handle_ice_events(call,ev,data);
					//resolve memory leak
	...
	if (ice_check_received_binding_response_addresses(rtp_session, evt_data, succeeded_pair, remote_addr) < 0)
    {
//        PrintConsole("ice_check_received_binding_response_addresses\n");
        return;
    }
    else
    {
//        PrintConsole("ice_check_received_binding_response_addresses else\n");
        
    }
    
	if (ice_check_received_binding_response_attributes(msg, remote_addr) < 0)
    {
//        PrintConsole("ice_check_received_binding_response_attributes\n");
        return;
    }
    else
    {
//        PrintConsole("ice_check_received_binding_response_attributes else\n");
    }
...
void ice_handle_stun_packet(IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data, void *data)
{
	StunMessage msg;
	...
	if (cl->session111 == NULL) return;
    
	memset(&msg, 0, sizeof(msg));
//    printf("Sean        Here is response\n");
//    unsigned char *traceChar = (unsigned char *)mp->b_rptr;
//    for (int i=0; i<mp->b_wptr-mp->b_rptr; i++) {
//        printf("%02X ",*(traceChar+i));
//    }
//    printf("\n");
...
void ice_compute_candidate_priority(IceCandidate *candidate)
{
...
static void ice_prune_candidate_pairs(IceCheckList *cl)
{
	...
	if (nb_pairs > cl->session111->max_connectivity_checks) {
		nb_pairs_to_remove = nb_pairs - cl->session111->max_connectivity_checks;
		list = cl->check_list;
...
static void ice_continue_processing_on_next_check_list(IceCheckList *cl, RtpSession *rtp_session)
{
	...
	if (elem == NULL) {
		...
		elem = ms_list_find_custom(cl->session111->streams, (MSCompareFunc)ice_find_unsuccessful_check_list, NULL);
		if (elem == NULL) {
		...
		cl->session111->event_time = ice_add_ms(ice_current_time(), 1000);
		cl->session111->event_value = ORTP_EVENT_ICE_SESSION_PROCESSING_FINISHED;
		cl->session111->send_event = TRUE;
	} else {
...
static void ice_conclude_processing(IceCheckList *cl, RtpSession *rtp_session)
{
	...
	if (cl->state == ICL_Running) {
		if (cl->session111->role == IR_Controlling) {
...
{
	if ((*cl)->remote_ufrag) { ms_free((void **)&(*cl)->remote_ufrag);}
	if ((*cl)->remote_pwd) { ms_free((void **)&(*cl)->remote_pwd);}
	(*cl)->remote_ufrag = (*cl)->remote_pwd = NULL;
...
{
	if (session->local_ufrag) { ms_free((void **)&session->local_ufrag);}
	if (session->local_pwd) { ms_free((void **)&session->local_pwd);}
	if (session->remote_ufrag) { ms_free((void **)&session->remote_ufrag);}
	if (session->remote_pwd) { ms_free((void **)&session->remote_pwd);}
...
static bool_t ice_check_gathering_timeout(IceCheckList *cl, RtpSession *rtp_session, MSTimeSpec curtime)
{
	...
	tb.time = curtime;
	tb.result = FALSE;
	ms_list_for_each2(cl->session111->streams, (void (*)(void*,void*))ice_check_gathering_timeout_of_check_list, &tb);
	if (tb.result == TRUE) {
		ms_list_for_each(cl->session111->streams, (void (*)(void**))ice_check_list_stop_gathering);
...
//            ice_send_stun_server_binding_request(base, (struct sockaddr *)&cl->session->ss, cl->session->ss_len, check,0,cl->rtp_session->session_type,cl->rtp_session->channel);
            ice_send_stun_server_binding_request(base, (struct sockaddr *)&cl->session111->ss, cl->session111->ss_len, check,check->sock%2,cl->rtp_session->session_type,cl->rtp_session->channel);
/*update end--------------------Sean20130723----------for video ice------------*/
...
//        printf("pair->state == ICP_InProgress ice_compare_time\n");
		ice_send_binding_request(params->cl, pair, params->rtp_session);
	}
    else
    {
//        printf("pair->state == ICP_InProgress ice_compare_time not\n");
    }
}
...
void ice_check_list_process(IceCheckList *cl, RtpSession *rtp_session)
{
	...
	if ((cl->session111->send_event == TRUE) && (ice_compare_time(curtime, cl->session111->event_time) >= 0)) {
		OrtpEvent *ev;
		cl->session111->send_event = FALSE;
		ev = ortp_event_new(cl->session111->event_value);
		ortp_event_get_data(ev)->info.ice_processing_successful = (cl->session111->state == IS_Completed);
		rtp_session_dispatch_event(rtp_session, ev);
	}

	if ((cl->session111->state == IS_Stopped) || (cl->session111->state == IS_Failed)) return;

	switch (cl->state) {
		case ICL_Completed:
			/* Handle keepalive. */
			if (ice_compare_time(curtime, cl->keepalive_time) >= (cl->session111->keepalive_timeout * 1000)) {
			...
			if (ice_compare_time(curtime, cl->ta_time) < cl->session111->ta) return;
			cl->ta_time = curtime;
		...
		case ICL_Running:
			...
			if ((cl->nomination_delay_running == TRUE) && (ice_compare_time(curtime, cl->nomination_delay_start_time) >= ICE_NOMINATION_DELAY)) {
				ice_conclude_processing(cl, rtp_session);
				if (cl->session111->state == IS_Completed) return;
			}
			/* Check if some retransmissions are needed. */
			ice_check_list_retransmit_connectivity_checks(cl, rtp_session, curtime);
			if (ice_compare_time(curtime, cl->ta_time) < cl->session111->ta) return;
...
static int32_t ice_compare_time(MSTimeSpec ts1, MSTimeSpec ts2)
{
//    printf("ts1.tv_sec = %lld,ts2.tv_sec = %lld,ts1.tv_nsec = %lld, ts2.tv_nsec = %lld\n",ts1.tv_sec,ts2.tv_sec,ts1.tv_nsec,ts2.tv_nsec);
...
static char * ice_inet_ntoa(struct sockaddr *addr, int addrlen, char *dest, int destlen)
{
	...
	dest[0] = '\0';
#ifdef ANDROID
    sockaddr_in *tempaddr = (sockaddr_in *)addr;
    char *tempdest;
    tempdest = inet_ntoa(tempaddr->sin_addr);
...
static void ice_set_credentials(char **ufrag, char **pwd, const char *ufrag_str, const char *pwd_str)
{
	...
	if (*ufrag) {ms_free((void **)ufrag);}
	if (*pwd) {ms_free((void **)pwd);}
...
int ice_find_host_candidate(const IceCandidate *candidate, const uint16_t *componentID)
{
...
{
    if (cm == NULL || cm->cl == NULL || cm->cl->valid_list == NULL) {
        return;
    }
...
void ice_get_local_addr_and_ports_from_valid_pairs(const IceCheckList *cl, char *rtp_addr, int *rtp_port, char *rtcp_addr, int *rtcp_port, int addr_len)
{
	Addr_Ports addr_ports;
	MSList *ice_pairs = ice_get_valid_pairs(cl);
	addr_ports.rtp_addr = rtp_addr;
