/** 
 ...
 protocols and implementations, for example SIP, JINGLE...
**/
...
#define SAL_MEDIA_DESCRIPTION_MEDIA_DECREASED 0x05
#define SAL_MEDIA_DESCRIPTION_MEDIA_INCREASED 0x06
...
typedef void (*SalOnCallProceeding)(SalOp *op);
...
typedef void (*SalOnTextReceived)(Sal *sal, const char *from, const char *to, const char *msgid, const char *msg, const char *date);
typedef void (*SalOnTextReport)(Sal*sal,const char *msgid,const char*date,int status);
...
typedef void (*SalOnStunPacket)(Sal *sal, const char*call_id, OrtpEvent *stun_packet, bool_t is_video);
typedef void (*SalOnOptionReply)(SalOp *salop, int status);

typedef struct SalCallbacks{
	SalOnCallReceived call_received;
	SalOnCallRinging call_ringing;
	SalOnCallAccepted call_accepted;
	SalOnCallProceeding call_proceeding;
	...
	SalOnTextReport text_send_report;
	SalOnNotify notify;
    ...
    SalOnStunPacket stun_packet;
	SalOnOptionReply option_reply;
}SalCallbacks;
...
/*Messaging */
char *sal_text_send(SalOp *op, const char *from, const char *to, const char *text);
