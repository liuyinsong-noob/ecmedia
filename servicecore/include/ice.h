...
typedef struct _IceCheckList {
	IceSession *session111;	/**< Pointer to the ICE session */
 ...
 * This function is called from the audiostream or the videostream and is NOT to be called by the user.
 */
//void ice_handle_stun_packet(IceCheckList* cl, RtpSession* rtp_session, const char* buf, int bufLen);
...
/**
 * Get the local address, RTP port and RTCP port to use to send the stream once the ICE process has finished successfully.
 *
 * @param cl A pointer to a check list
 * @param rtp_addr A pointer to the buffer to use to store the remote RTP address
 * @param rtp_port A pointer to the location to store the RTP port to
 ...
 * This function will usually be called from within the success callback defined while creating the ICE check list with ice_check_list_new().
 */
MS2_PUBLIC void ice_get_local_addr_and_ports_from_valid_pairs(const IceCheckList *cl, char *rtp_addr, int *rtp_port, char *rtcp_addr, int *rtcp_port, int addr_len);
...
MS2_PUBLIC int ice_find_host_candidate(const IceCandidate *candidate, const uint16_t *componentID);
    MS2_PUBLIC void ice_compute_candidate_priority(IceCandidate *candidate);
#ifdef __cplusplus
}
#endif
