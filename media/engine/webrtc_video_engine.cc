...
VideoSendParameters WebRtcVideoChannel::GetSendParameters() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  return send_params_;
}

VideoRecvParameters WebRtcVideoChannel::GetRecvParameters() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  return recv_params_;
}
...
bool WebRtcVideoChannel::SendKeyframe() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  for (auto it : send_streams_) {
    it.second->SendKeyframe();
  }
  return true;
}

bool WebRtcVideoChannel::SetRequestKeyframeCallback(
    const int channelId,
...
//ytx_begin
bool WebRtcVideoChannel::RequestRemoteSsrc( const int channelId,int flag,int32_t ssrc, uint32_t local_ssrc) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  for (auto it : receive_streams_)
    it.second->RequestRemoteSsrc(flag,ssrc, local_ssrc);
...
bool WebRtcVideoChannel::WebRtcVideoSendStream::SendKeyframe() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  if (stream_) {
    stream_->SendKeyframe();
    return true;
  }
  return false;
}
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::SetRequestKeyframeCallback(
    const int channelId,
    OnRequestKeyFrameCallback cb) {
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::RequestRemoteSsrc(int flag,
    int32_t ssrc, uint32_t local_ssrc) {
  stream_->RequestRemoteSsrc(flag,ssrc, local_ssrc);
}
