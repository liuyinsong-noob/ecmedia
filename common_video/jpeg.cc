...
WebRtc_Word32
JpegEncoder::Encode(const webrtc::VideoFrame& inputImage)
{
	...
	WebRtc_UWord8 * inputYPlane = (WebRtc_UWord8 *)inputImage.video_frame_buffer().get()->GetI420().get()->DataY();
	WebRtc_UWord8 * inputUPlane = (WebRtc_UWord8 *)inputImage.video_frame_buffer().get()->GetI420().get()->DataU();
	WebRtc_UWord8 * inputVPlane = (WebRtc_UWord8 *)inputImage.video_frame_buffer().get()->GetI420().get()->DataV();
	...
	for (int i = 0; i < height / 2; i++)
	{
		...
		uPlane += width / 2;
		inputUPlane += (width + 1) / 2;

		memcpy(vPlane, inputVPlane, width / 2);
		vPlane += width / 2;
		inputVPlane += (width + 1) / 2;
	}
...
/*WebRtc_Word32
JpegEncoder::Encode(const VideoFrame& inputImage, EncodedImage& outputImage)
{
